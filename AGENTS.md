...
│       ├── human-capital-management/     (HCM — governance scaffold mevcut; production service yok)
│       ├── talent-ecosystem-platform/    (TEP — governance scaffold mevcut; production service yok)
...
**HCM / TEP notu:** `execution/domains/human-capital-management/` ve
`execution/domains/talent-ecosystem-platform/` governance scaffold olarak mevcuttur. Bu bootstrap
`services/Diten.TalentEcosystemService/` yalnız DCP-010 scaffold authorization kapsamında boş/minimal service scaffold
olarak oluşturulmuştur; native TEP business runtime implementation hâlâ DCP-002 canonicalization / CAND-CAP kararları,
ilgili domain module pack `ready-for-dev` durumu ve açık kullanıcı onayı sonrası ele alınabilir.
`services/Diten.HumanCapitalService/` yalnız DCP-009 scaffold authorization kapsamında boş/minimal service scaffold
olarak oluşturulmuştur; native HR/HCM business runtime implementation hâlâ DCP-002 canonicalization / CAND-CAP kararları,
ilgili domain module pack `ready-for-dev` durumu ve açık kullanıcı onayı sonrası ele alınabilir.
...
dotnet build services/Diten.TalentEcosystemService/src/Diten.TalentEcosystemService.Api/Diten.TalentEcosystemService.Api.csproj -c Debug
...
- `domain-kısa`: `mdm` | `pss` | `deven` | `hcm` | `tep` | `esbp`
...
- `feature/hcm/{module-id}-{slug}`
- `feature/tep/{module-id}-{slug}`
...
| **Domain** | `execution/domains/{name}/` — bir iş alanı (DEVEN, PSS; MDM, HCM, TEP governance scaffold; ESBP planlı) |

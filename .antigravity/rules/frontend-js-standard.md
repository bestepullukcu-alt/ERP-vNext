...
7.1. **Bulk/Row Action Dispatcher (ZORUNLU):** Standart DataTable sayfalarında checkbox selection, bulk action bar ve row action dropdown elle bağlanmaz. Aşağıdaki shared helper'lardan biri kullanılmalıdır:
   ...
   - **Row-action dropdown ikon/label hizalama SSOT (ZORUNLU):** Dropdown içindeki action item'larının ikon boyutu, ikon↔label boşluğu ve dikey ortalaması `renderActions` tarafından otomatik basılan `.dt-action-item` (flex + `align-items:center` + `gap` + `line-height`) ve `.dt-action-icon` (ikon `font-size`) sınıflarıyla yönetilir; bu sınıfların tanımı `backbone-custom.css` içindedir. Action item markup'ına `d-flex`, `align-items-center`, `me-*` gibi utility class veya inline style ile hizalama/ikon-boyutu eklemek YASAKTIR. Boyut/boşluk/hizalama değişikliği yalnızca `backbone-custom.css` içindeki bu tek SSOT bloğunda yapılır; transform/`translateY` hack'i kullanılmaz (gerekli dikey ortalama saf flex ile sağlanır).

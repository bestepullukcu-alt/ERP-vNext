...
7. **Artifact Retention (Eserlerin Korunması - ZORUNLU):** Planlama (Plan.md), gereksinim (PRD), module pack ve denetim raporları (/docs/records/audits/*) görev tamamlandıktan sonra KESİNLİKLE SİLİNMEZ.
...
10. **Product Backlog Kapısı (ZORUNLU):** Bilinçli ertelenen / go-live kapsamı dışı özellikler [`docs/roadmap/backlog/product-backlog.md`](../../docs/roadmap/backlog/product-backlog.md) dosyasında tutulur. Kod yazmadan önce bir talebin bu backlog'daki bir maddeye (ör. BL-001 Corporate Action Workspace, BL-002 Filing Calendar/Inbox, BL-003 LE governance/approval workflow, BL-004 LE evidence/belge toplama) denk gelip gelmediğini KONTROL ET. Denk geliyorsa: onaylı bir module pack maddeyi backlog'dan açıkça çıkarmadıkça **inşa etme** — maddeyi göster, `/prepare-module-pack` kapısına yönlendir. Bir özellik bu görüşmede bilinçli ertelendiyse, tamamlanmış saymadan önce backlog'a **yeni madde olarak ekle** (ne / neden ertelendi / yapım tetikleyicisi / ilgili modül). Backlog maddesi ancak teslim edilince kaldırılır. Diğer developer'lar ve ajanlar bu dosyayı ortak ertelenen-iş kaydı olarak kullanır.
    ...
    - **DOSYA İZİ ZORUNLU:** Bu madde bir kutu işaretlemekle değil, `git diff --name-only <base>..HEAD -- docs/roadmap/backlog/product-backlog.md` çıktısıyla doğrulanır. Boş çıktı + "yazıldı ✓" işareti **ihlaldir**.
...
>
> ⛔ **İşaret yeterli değildir, dosya izi gerekir.** Ölçüldü (2026-07-31): son 30 günde 92 commit ve 1123 kod dosyası değişirken `docs/reference/modules/platform/**/api.md` ve `user-manual.md` dosyalarının **hiçbiri** değişmedi — 21 dosyanın 20'si 2026-05-20'den beri donmuş. Dokümantasyon kutusu 72 gündür işaretleniyor ya da atlanıyor, ve `workflows/add-module.md:166`'daki ⛔ blocker bunu durdurmuyor. Sebebi: o blocker **kutu** istiyor, **dosya** değil. Dokümantasyon ve Backlog maddeleri için `git diff --name-only` çıktısı raporda yer alır; çıktı boşsa madde işaretlenmiş sayılmaz.

...
- **Bileşik Index (Compound Index):** Neredeyse tüm tenant-owned sorgular `TenantId` üzerinden yapılacağı için, tenant-owned index'ler her zaman `TenantId` ile başlamalıdır.
  - *Doğru Index:* `{ TenantId: 1, CountryCode: 1 }`
  - *Yanlış Index:* `{ CountryCode: 1 }`
- Eğer tenant-owned bir alan benzersiz (Unique) olacaksa, bu benzersizlik sadece o Tenant'ın içinde geçerli olmalıdır (Tenant-Scoped Unique Index). Platform global katalog istisnasında global unique index kullanılacaksa module pack acceptance criteria bunu açıkça test eder.
...
2. MongoDB sürücüsü (C#) üzerinden Fluent API veya Attribute'lar ile gerekli TenantId, IsDeleted ve performans Index'lerini yaz. Onaylı `GlobalEntity` kataloglarında TenantId index'i yazma; bunun yerine global unique/query indexleri ve `IsDeleted` davranışını doğrula.

...
| **Faz 2a-rest → next** | ⚠ The Task Center's complete dialog does NOT collect closure fields yet: WCN completes through the generic `WorkItemActionPayloadDto` (shared with MOD-0023), which carries no field values. Until this lands, a type with a REQUIRED closure field cannot be completed from the Task Center (400 `TASK_CLOSURE_FIELD_REQUIRED`). Scope: payload field + dispatcher pass-through + dialog rendering with the existing field renderer + reason-code label in 7 languages | Additive contract | Next |

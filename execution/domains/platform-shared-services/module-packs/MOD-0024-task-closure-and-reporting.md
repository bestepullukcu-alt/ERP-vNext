...
| **Faz 2a → done** | `TaskItem.ClosureNote` (≤ 4000) written on complete/cancel and copied to the transition's `Reason` (it was measured as silently discarded: `reason: null`); `TaskFieldDefinition.Stage` (Entry/Closure, default Entry); CLOSURE fields validated server-side (`TASK_CLOSURE_FIELD_REQUIRED`) and merged into `FieldValues`; the generic create/update path and the task form see Entry fields only; projection `closure { note, fields[], deliverables[] }` referencing ATT-1 attachments (no new container); detail-panel closure card; Stage on the field-definition screen; 7 languages. `deliverables[]` was NOT rebuilt — ATT-1 already provides it | Yes, additive | **Delivered 2026-09-13** (WP-PSS-MOD0024-CLOSURE-ENVELOPE-2A-01) |
| **Faz 2a-rest → done** | The Task Center's complete dialog now collects CLOSURE fields: `WorkItemActionPayloadDto.ClosureFieldValues` (neutral `WorkItemFieldValueDto`, omitted from the wire when null), `TaskWorkItemActionDispatcher` passes it for `complete` only, `HttpWorkItemActionDispatcher` strips it unconditionally so MOD-0023 receives a byte-identical body; the dialog fetches the catalogue only for `complete`, filters with the task form's own applicability rule and draws with `TaskForm.renderCustomFields` (no second renderer); `TASK_CLOSURE_FIELD_REQUIRED` labelled in 7 languages | Additive contract | **Delivered 2026-09-13** (WP-PSS-MOD0024-CLOSURE-ENVELOPE-2A-REST-01) |
| **Faz 2b** | `followUps[]` (next-step task), work report columns (note / deliverables / evidence), structured challenge codes | Yes, additive | Later |
...
| **Dilim 1e → export** | `GET /api/v1/tasks/work-report/export?format=csv\|json` and a download menu beside the filter badge. **The file is the report's own rows**: `ExportAsync` calls the same `ReadAsync` the numbers use, and `WorkReportTally.Export` marks each row with the cells `Select` puts it in — summing a column gives that cell's number. Same `WorkReportRead` key, same scope source, same five filters, same scope preference. More than 50 000 rows is **refused** (`WORK_REPORT_EXPORT_TOO_LARGE`), never cut. English column names (the audit CSV's rule). **Audit trail (BL-347, `5681eaac`):** every export writes one tenant-owned DataExport record through `IDataExportAuditWriter` before the file is returned — actor type from the token's `actor_type`, an unknown type refuses the export, `IsPlatformGlobal=false`; if the record cannot be written the file is not handed out (503 `DATA_EXPORT_AUDIT_NOT_RECORDED`) — live check pending | Read-only | **⚠️ Code 2026-09-10 — live check pending (BL-346)** |

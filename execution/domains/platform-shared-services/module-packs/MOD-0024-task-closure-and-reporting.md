...
| **Dilim 1e → export** | `GET /api/v1/tasks/work-report/export?format=csv\|json` and a download menu beside the filter badge. **The file is the report's own rows**: `ExportAsync` calls the same `ReadAsync` the numbers use, and `WorkReportTally.Export` marks each row with the cells `Select` puts it in — summing a column gives that cell's number. Same `WorkReportRead` key, same scope source, same five filters, same scope preference. More than 50 000 rows is **refused** (`WORK_REPORT_EXPORT_TOO_LARGE`), never cut. English column names (the audit CSV's rule). **Open:** no tenant-side export audit record — the audit export's `AuditMetaAuditWriter` stamps `PlatformAdministrator`/`IsPlatformGlobal` and would misattribute a tenant user (BL-347) | Read-only | **⚠️ Code 2026-09-10 — live check pending (BL-346)** |

...
- ✅ **E4 authenticated (2026-09-10, güncelleme):** projenin kendi token formatında dev-token (actor_type=tenant_user, tenant_id=97c5, `permission` claim'leri, local dev secret) ile seed'li 358'e karşı **HTTP 200**: `citations:resolve` by=uid → tam citation item'ları (citable=false, blockedReason=Draft, source=MasterRegister, unresolved omit); `citations/search?term=Mast` → term-filtreli, blocked görünür/citable=false. **Tam uçtan uca kanıtlı.** (Salt-okuma; gerçek RBAC grant + gerçek login ops tarafı — dev-token izinleri assert etti.)

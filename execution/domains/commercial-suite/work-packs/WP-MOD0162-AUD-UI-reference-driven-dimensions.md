...
NE (yalnız frontend Diten.Web):
 1) AudienceProfile formuna (tax-only-profile) dimension builder: "eksen ekle/çıkar" satırları. Axis = single select2 (account-type/contact-type/medical-specialty/custom). Values = multi-select2 (reference axis→ o setin published-values'ından, adla-seç; custom→ serbest AxisCode input + tag Values). Dimensions opsiyonel (0+ satır).

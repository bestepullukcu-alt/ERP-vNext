...
- ✅ **GATEWAY ROUTE GAP → ÇÖZÜLDÜ (SCMM-12-API-GW, 2026-09-13):** ocelot'a `/api/crm/content-composition/{everything}` bloğu eklendi (knowledge route birebir aynası: `localhost:5061`, GET/POST/PUT/OPTIONS, http, auth downstream). **CANLI DOĞRULANDI (gateway 5000 üzerinden, dev-token):** unauth→401 (route var, 404 değil) · authenticated GET→200 · POST create→201. Fleet restart (gateway+CrmService) sonrası ocelot+controller yüklendi; test claim temizlendi; fleet sağlıklı. Config değişikliği (ben ekledim, break-glass); JSON geçerli (160 route), knowledge ile downstream/scheme/method/port birebir.

---
id: CAND-CAP-0030
name: Learning / Training Records
...
branch: feature/hcm/cand-cap-0030-learning-training-records
started: 2026-09-07
target: 2026-12-15
form_field_count: 0
...
candidate_identity: CAND-CAP-0030
legacy_excel_id: MOD-0309
...
runtime_owner_key: hcm.learning-training
permission_namespace:
  - hcm.learning-training.read
  - hcm.learning-training.manage
  - hcm.learning-training.evaluate
  - hcm.learning-training.audit.read
runtime_repo_scope: services/Diten.HumanCapitalService/**
---

# CAND-CAP-0030 - Learning / Training Records

> Status: draft. This pack records the first metadata-only backend/API
> learning/training readiness contract slice under
...
> gateway (`5080` -> HCM `5059`). Production learning/training workflow, course
> catalog, enrollment, completion tracking, certification, assessment scoring,
> automated decision behavior, manager/employee learning UX, learning content /
> LMS integration, notification/document integration, and sensitive persistence
> remain closed. `CAND-CAP-0030` remains a governance/documentation identity
> only and must not be written into runtime literals. `MOD-0309` remains blocked
> for HCM use until future EA canonical MOD assignment.

## 1. Module Summary

`CAND-CAP-0030` reserves the temporary DCP-002 candidate identity for the native
Human Capital Management R3-C capability named `Learning / Training Records`.

The source roadmap is
`execution/portfolio/delivery-capability-packs/DCP-008-hr-tep-delivery-roadmap.md`.
DCP-008 proposed legacy Excel ID `MOD-0309`, but DCP-002 blocks that ID because
it is not Blueprint-backed and has no canonical registry row. EA/registry owner
reservation records `CAND-CAP-0030` as a temporary governance identity pending
future canonical MOD allocation.

This pack records the first metadata-only backend/API learning/training
readiness contract slice, mirroring the completed Competency & Skills Assessment
slice (`CAND-CAP-0028` / `MOD-0307`) exactly. Learning/training workflow, course
catalog, enrollment, completion tracking, certification, assessment scoring,
automated decision behavior, manager/employee learning UX, learning content /
LMS integration, notification/document integration, and sensitive persistence
remain closed.
...
- HCM-native learning/training records governance boundary.
- Sequencing after completed HCM employee, governance, position, offboarding,
  ...
  employment change, performance review, competency/skills, and development
  plan foundation modules.
- Candidate identity reservation and blocked legacy ID rationale.
- Metadata-only learning/training readiness backend/API contract.
- Golden-compact readiness CRUD frontend and gateway exposure limited to the
  metadata-only readiness contract.
- Explicit exclusion of learning/training workflow execution.
- Explicit exclusion of course catalog, enrollment, completion tracking,
  certification, assessment scoring, model output, and automated decision
  behavior.
- Explicit exclusion of manager-facing and employee-facing learning UX beyond
  the readiness-metadata CRUD (those fields are reserved and out of scope).
- Explicit exclusion of free-text learning notes, training transcript,
  attachment payload, raw provider/LMS payload,
  credential/token/secret/password, and PII-heavy persistence.

Not owned by this pack:

- Real learning/training execution, course delivery, enrollment submissions,
  completion records, transcripts, certifications, assessment scores, or model
  output.
- Learning content / LMS ownership beyond dependency/context references.
...
- Development plan, competency/skills, performance review, succession,
...
| Object | Type | Purpose |
|---|---|---|
| LearningTrainingRecordsBoundary | Governance boundary | Defines what a future HCM learning/training module may own. |
| LearningTrainingReadinessBoundary | Governance boundary | Separates readiness metadata from learning/training workflow execution. |
| CourseEnrollmentBoundary | Governance boundary | Blocks course catalog, enrollment, and completion tracking execution. |
| CertificationAssessmentBoundary | Governance boundary | Blocks certification, assessment score, and model-output persistence. |
| LearningUxBoundary | Governance boundary | Blocks manager and employee learning UX beyond readiness-metadata CRUD. |
| LearningSensitiveDataBoundary | Governance boundary | Blocks transcript, attachment, credential, and PII-heavy persistence. |
| LearningDependencyBoundary | Governance boundary | Keeps learning-content/LMS/notification/document dependencies deferred and out of scope. |

Deferred runtime objects:

| Object | Type | Purpose |
|---|---|---|
| LearningTrainingWorkflow | Deferred runtime | Learning/training workflow execution; not authorized. |
| CourseEnrollmentCompletionEngine | Deferred runtime | Course catalog, enrollment, and completion tracking; not authorized. |
| CertificationAssessmentEngine | Deferred runtime | Certification, assessment scoring, and model output; not authorized. |
| ManagerEmployeeLearningExperience | Deferred frontend/runtime | Manager/employee learning UX beyond readiness CRUD; not authorized. |
| LearningContentLmsIntegration | Deferred dependency | Learning content / LMS integration; out of scope. |
| LearningDocumentIntegration | Deferred dependency | Controlled/external document use; out of scope. |
| LearningNotificationIntegration | Deferred dependency | Notification and reminder delivery; out of scope. |

Approved runtime/frontend objects (metadata-only readiness slice):

| Object | Type | Purpose |
|---|---|---|
| LearningTrainingReadinessMetadata | Domain entity | Metadata-only readiness contract; `Domain/Entities/LearningTrainingReadinessMetadata.cs`. |
| LearningTrainingReadinessState | Domain enum | Readiness state; `Domain/Enums/LearningTrainingReadinessState.cs` (Draft=0, Ready=1, Deferred=2, Blocked=3, NotRequired=4, Archived=5). |
| ILearningTrainingReadinessMetadataRepository | Domain repository | Tenant-aware repository contract; `Domain/Repositories/ILearningTrainingReadinessMetadataRepository.cs`. |
| MongoLearningTrainingReadinessMetadataRepository | Persistence repository | Mongo-backed repository; collection `hcm_learning_training_readiness`. |
| LearningTraining Application features | Application (CQRS/MediatR) | `Application/Features/LearningTraining/**` (Models/Guard/Commands/Queries/Handlers). |
| LearningTrainingController | API controller | Thin controller; `Api/Controllers/Hcm/LearningTrainingController.cs`. |
| LearningTraining golden-compact CRUD | Frontend | Controller + `Views/HumanCapital/LearningTraining/**` + `wwwroot/assets/js/HumanCapital/LearningTraining/**` + Resources + nav entry under `frontend/Diten.Web/**`. |
...
`LearningTrainingReadinessState`):

- `Code`
- `DisplayName`
- `LearningTrainingReadinessState`
- `CourseCatalogBoundaryState`
- `EnrollmentWorkflowBoundaryState`
- `CompletionTrackingBoundaryState`
- `CertificationBoundaryState`
- `AssessmentScoringBoundaryState`
- `AutomatedDecisionBoundaryState`
- `LearningContentDependencyState`
...
- `LastEvaluatedAt`
- `LearningTrainingReadinessVersion`
- `DeferredReason`
- `IsDeleted`
- `DeletedAt`
- `CreatedAt`
...
- Manager-facing learning fields (manager assignment, manager review, manager
  action UX state) are reserved and out of scope.
- Employee-facing learning fields (employee enrollment, employee completion,
  employee self-service UX state) are reserved and out of scope.
...
- Learning/training workflow body, enrollment submission payload, completion
  payload, transcript payload, certification body, approval decision body,
  action evidence, or completed course content.
- Assessment score, rating value, rank, certification outcome, model output,
  automated decision result, recommendation output, or evaluator scoring
  payload.
- Free-text learning notes, training narrative, instructor comments, manager
  comments, employee comments, HR notes, or sensitive learning body.
- Attachment payload, controlled document body, external document content, raw
  evidence, raw provider/LMS payload, or uploaded file metadata.
- Salary amount, compensation amount, course cost, benefits election, payroll
  details, tax details, bank details, or payment instruction fields.
...
- `execution/domains/human-capital-management/module-packs/CAND-CAP-0030-learning-training-records.md`
...
- `frontend/Diten.Web/**` (Controller, `Views/HumanCapital/LearningTraining/**`,
  `wwwroot/assets/js/HumanCapital/LearningTraining/**`, Resources, nav entry).
...
- `frontend/**` outside the owned LearningTraining CRUD objects listed in §3/§5.
- `gateway/**` outside the owned LearningTraining readiness route.
- `services/Diten.TalentEcosystemService/**`
- `services/Diten.Platform/**`
- `services/Diten.PssService/**`
- global `tests/**`
- notification/document/LMS module packs listed as out of scope.
...
- Runtime owner/key: `hcm.learning-training`.
...
- Thin controller (`Api/Controllers/Hcm/LearningTrainingController.cs`),
...
- Mongo-backed tenant-aware repository; collection
  `hcm_learning_training_readiness`.
...
- Course catalog, enrollment, completion tracking, certification.
- Assessment scoring/model-output persistence.
- Automated decision behavior.
- Manager/employee learning UX beyond readiness-metadata CRUD.
- Learning content / LMS, notification, and document integrations.
...
- Tenant shell nav entry for learning/training readiness.
...
- Manager learning UX beyond readiness-metadata CRUD.
- Employee learning/self-service UX beyond readiness-metadata CRUD.
- Course catalog, enrollment, completion, certification, assessment scoring, or
  model-output UI.
- Document, notification, LMS, export, upload, or attachment UI.
...
- `GET /api/learning-training` - list readiness metadata.
- `GET /api/learning-training/{id}` - get readiness metadata by id.
- `POST /api/learning-training` - create readiness metadata.
- `POST /api/learning-training/{id}/evaluate` - fail-closed evaluation.
- `DELETE /api/learning-training/{id}` - soft-delete.
- `GET /api/learning-training/{id}/audit-metadata` - audit metadata read.

The first slice must stay limited to:

- Create/list/get learning/training readiness metadata.
- Fail-closed evaluation/deferred metadata.
- Audit metadata read endpoint.
- Soft delete.
- Tenant-aware persistence and active-code uniqueness.
- No learning/training workflow, course catalog, enrollment, completion
  tracking, certification, assessment scoring, model-output persistence,
  automated decision behavior, learning content / LMS integration,
...
- Metadata-only learning/training readiness state.
- Boundary-state metadata for course catalog, enrollment, completion tracking,
  certification, assessment scoring, automated decision, consent, minimization,
  retention, and evidence.
- Dependency-state metadata for learning content, skill taxonomy, document, and
  notification dependencies.
- Dependency/precondition metadata (`DependencyStates` dictionary).

Forbidden:

- Learning/training workflow payload.
- Course/enrollment/completion/certification execution payload.
- Assessment score/model output.
- Automated decision result.
- Free-text learning notes or training narrative.
- Training transcript payload.
- Attachment payload.
- Raw provider/LMS payload.
- Credential/token/secret/password values.
- PII-heavy employee/profile details.
- Compensation/payroll/benefits/course-cost payload.
...
- Learning data minimization fail-closed behavior.
- Retention/evidence/audit/legal-hold/deletion as local/deferred metadata.
- Exclusion of learning narrative, transcript, scoring payloads, and PII-heavy
  data.
...
- Learning content / LMS integration (no canonical MOD; deferred).

No integration implementation, LMS/provider payload persistence, or
...
2. AC-02: Candidate identity is `CAND-CAP-0030`.
3. AC-03: Legacy Excel ID `MOD-0309` remains blocked/unsafe until EA canonical
   MOD assignment.
4. AC-04: Runtime owner/key is `hcm.learning-training`.
5. AC-05: Permission namespace is limited to `hcm.learning-training.read`,
   `hcm.learning-training.manage`, `hcm.learning-training.evaluate`, and
   `hcm.learning-training.audit.read`.
...
8. AC-08: `TenantId` is not accepted in request DTOs; tenant is resolved
   server-side.
9. AC-09: Mongo persistence (collection `hcm_learning_training_readiness`)
...
13. AC-13: Learning/training workflow, course catalog, enrollment, completion
    tracking, certification, assessment scoring, model output, and automated
    decision behavior remain unauthorized.
14. AC-14: Manager/employee learning fields remain reserved/out of scope; no
    money field is present.
15. AC-15: Free-text learning notes, training transcript, attachment payload,
    raw provider/LMS payload, credential/token/secret/password, and PII-heavy
    persistence remain unauthorized.
16. AC-16: Learning content / LMS, notification, and document dependencies
...
19. AC-19: Golden-compact parity and list-DTO to JS column parity hold with no
    drift.
20. AC-20: Runtime literal scan for `CAND-CAP-0030|MOD-0309` passes under
...
- Run targeted application tests with `LearningTraining` filter
  (`Application.Tests/LearningTrainingTests.cs`, handler behavior; fix-absent
  should be RED).
...
- Verify forbidden workflow/catalog/enrollment/completion/certification/
  scoring/decision/sensitive fields are absent.
- Verify manager/employee learning fields are reserved/absent and no money
  field exists.
...
- Verify runtime literal scan:
  `rg -n "CAND-CAP-0030|MOD-0309" services/Diten.HumanCapitalService frontend gateway tests`
...
- [x] Metadata-only backend/API + golden-compact readiness CRUD + gateway scope
  is accepted.
- [x] Learning/training workflow prohibition is accepted.
- [x] Course catalog/enrollment/completion/certification prohibition is
  accepted.
- [x] Assessment scoring/model-output prohibition is accepted.
- [x] Automated decision behavior prohibition is accepted.
- [x] Reserved manager/employee learning fields and no-money-field constraint
  are accepted.
- [x] Sensitive payload prohibition is accepted.
- [x] Learning content / LMS / notification / document dependency deferral is
  accepted.
...
- Metadata-only backend/API slice plus golden-compact readiness CRUD frontend
  ...
  and the owned `frontend/Diten.Web/**` LearningTraining objects.
- Runtime owner/key remains limited to `hcm.learning-training`.
- Permission namespace remains limited to `hcm.learning-training.read`,
  `hcm.learning-training.manage`, `hcm.learning-training.evaluate`, and
  `hcm.learning-training.audit.read`.
- Section 4 metadata fields are aligned with the intended runtime
  public/persisted contract.
- `SkillTaxonomyDependencyState`, `LearningContentDependencyState`,
  `DocumentDependencyState`, and `NotificationDependencyState` remain
  metadata-only dependency states.
- Manager/employee learning UX fields remain reserved/out of scope and no money
  field is present.
- TEP, PSS, and Platform scopes remain closed.
- Learning/training workflow, course catalog, enrollment, completion tracking,
  certification, assessment scoring, model output, automated decision behavior,
  learning content / LMS integration, notification/document integration,
  free-text learning notes, training transcript, attachment payload, raw
  provider/LMS payload, credential/token/secret/password, and PII-heavy
...
- Candidate gate script is not executable in this checkout because
  ...
  `python3 .antigravity/scripts/verify_module_id.py . --candidate CAND-CAP-0030 --name "Learning / Training Records"`
...
- Learning data minimization fail-closed policy approved.
...
- Real learning/training workflow.
- Course catalog, enrollment, and completion tracking.
- Certification and assessment scoring governance.
- Automated decision approval.
- Manager/employee learning UX beyond readiness CRUD.
- Learning content / LMS integration.

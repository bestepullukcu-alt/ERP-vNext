...
### Draft MOD-0230 Interface Contract Packet

This packet records draft contract expectations only. It does not approve the dependent contracts, does not close
W-3A0, does not move MOD-0230 to `ready-for-dev`, and does not authorize runtime implementation.

| Owner | Draft expectation for MOD-0230 | Required closure before `ready-for-dev` |
|---|---|---|
| MOD-0018 RBAC / permissions | Actor context must include authenticated actor ID, actor type/role, tenant context, optional data-scope, and correlation context. Action checks must cover `read`, `create`, `update`, `triage`, `route`, `archive`, and `export`; `delete` and `bulk-delete` remain excluded. Permission seed/grant ownership remains with MOD-0018 / AuthService; MOD-0230 may only define and consume keys after the owner approves them. | Approved permission key registry, actor-role matrix, seed/grant owner, tenant authorization behavior, and denial/403/404 policy. |
| MOD-0019 masking / row-field security | Every user-entered field must bind to the recorded sensitivity class. Runtime must support mask, omit, or deny decisions for list/detail/create/update/export/audit surfaces. Missing or unavailable field policy must fail closed by denying the field or operation; raw PHI/PII/free text must not leak into logs, traces, metrics, validation errors, or audit payloads. | Approved sensitivity vocabulary, per-field policy matrix, row/field evaluation contract, unavailable-policy behavior, and fail-closed test expectations. |
| MOD-0021 AuditEvent v1 | Audited operations must use a redacted payload allow-list. Audit metadata must include actor, tenant, action, object reference, route/triage intent, UTC timestamp, correlation ID, and safe reason code where applicable. Audit append/outbox failure must block or queue regulated mutations according to the approved audit contract; no unaudited regulated mutation may silently succeed. | Approved event names, redacted payload shape, correlation propagation, audit failure policy, and audit payload tests. |
| MOD-0023 Workflow/Inbox v1 | Draft triage states are intake-created, intake-updated, triage-pending, triage-in-progress, triage-routed, returned-for-intake-update, void-or-archived, and handoff-ready, subject to workflow owner approval. Queue expectations include intake queue, triage queue, safety-manager queue, compliance-review queue, and partner/source-system queue if OD-7 chooses partner integration. Transition gates must fail closed when workflow is unavailable or the actor lacks authority. | Approved state set, queue registry, transition reason codes, route target contract, assignment/inbox semantics, and unavailable-workflow behavior. |
| MOD-0031 Evidence-Link | MOD-0230 object references must use a tenant-scoped Safety Case Intake reference and must not embed evidence content. Link types should distinguish source document, reporter communication, product evidence, medical/event evidence, triage rationale support, and partner/source-system reference. Provenance must capture source system/channel, received timestamp, actor/integration source, correlation ID, and immutable evidence reference. Evidence completeness must be evaluated through MOD-0031; no fake evidence pack fallback is allowed. | Approved object reference shape, link type taxonomy, provenance metadata, link/query API, completeness rules, and unavailable-evidence behavior. |
| Blueprint MOD-0040 / TRACE-BUNDLE | Canonical IDs must be server-generated and tenant-scoped. External IDs must be recorded as source references with source-system/channel metadata, not treated as authoritative primary keys. `X-Correlation-Id` must be accepted or generated at the boundary and propagated through audit, workflow, evidence, and error responses. Regulated errors must use reason codes and safe metadata without PHI/PII leakage. | Approved canonical ID policy, external ID policy, trace stitching contract, correlation propagation, regulated error model, and MOD-0288 boundary if organization/person/position references are later used. |

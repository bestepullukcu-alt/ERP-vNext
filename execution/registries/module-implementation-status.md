...
| 🔴 MOD-0285 System Navigation Management | **Kısmi** | 35 | ModulePageDescriptor + ModuleDomain entities; self-reg `register-manifest` (catalog descriptors) | **No menu/nav controller, no runtime menu loader, no admin tree-builder, no tests.** Tenant menu still hardcoded. (governed-publish blocked on MOD-0023; hierarchy can use catalog-domain) |

...
| MOD-0263 External Messaging Provider | Backend | 85 | SMTP/MailKit Batch 1 implementation evidence recorded: `SmtpMessagingProvider`, `MailKitSmtpClientFactory`, `SmtpProviderOptions`/validator, `SecretReferenceResolver`, `MessagingProviderErrorMapper`, DI registration, MailKit package, and mocked-transport SMTP tests are present. Validation recorded 2026-08-07: SMTP provider focused tests PASS 22/22; MOD-0027 Batch 1 / dispatch adapter regression slice PASS 32/32; Platform.API build PASS with 0 warnings / 0 errors; `git diff --check` PASS. No real SMTP server, external network, secrets, appsettings edits, fixture data, seeds, migrations, frontend, Gateway, or AuthService changes were used. | `review / pending-live-smoke`: live SMTP smoke remains pending approved local SMTP catcher, tenant settings, secret-reference, dispatch evidence, and cleanup/retention plan. SendGrid/failover/bounce webhooks remain deferred outside Batch 1. |

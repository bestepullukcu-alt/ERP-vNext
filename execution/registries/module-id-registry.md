...
| MOD-0040 | Tenant Organization Foundation | tenant-organization-foundation | Foundation | ready-for-dev | NEW-MOD-0040 | MOD-0040 | platform-shared-services | Required before Tenant Users / Tenant Roles implementation. Minimal backend-only schema reconciliation promoted to ready-for-dev; Tenant User validation and Position-role binding are deferred behind explicit guards. |
...
| MOD-0047 | Tenant User Foundation | tenant-user-foundation | Foundation | done |  |  | platform-shared-services | AuthService-owned read-only Tenant User lookup-validation contract completed: minimal `UserId` + `Referenceable` response, tenant-isolated active-user check, endpoint-specific mismatch guard, and permission seed. MOD-0040 Platform consumer integration remains deferred. |

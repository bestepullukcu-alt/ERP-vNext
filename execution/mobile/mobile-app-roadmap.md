...
**Durum:** **M0 + M1 PİLOT + FEATURE-PLUGIN + M1 ROLLOUT DALGA 1 TAMAM** ✅ — 7 çekirdek + `:core:feature` (plugin) + **5 HR feature** (applicant-intake pilot + Wave1: candidate-pipeline, employee-onboarding, offer-management, performance-reviews). Commit'ler: pilot fbbd816a · plugin refactor cfe08454 · Wave1 4dbd105f · remember-me 27dc717b · dev-connectivity 6dd49f77. **~190 mobil unit test yeşil, R8 release ~2.1MB.** **CANLI:** emülatörde SuperAdmin login (remember-me oturum kalıcı) → Home menüsünde 5 HR modülü (permission-gated, plugin-discovered) → feature navigasyonu. Sonraki: **M1 rollout Dalga 2–5** (kalan 15 standart HR) + 3 özel modül.

> **Feature-plugin mimarisi (cfe08454):** `:core:feature` `FeatureNavGraph` (Compose nav) + `:core:common` `FeatureEntry` (menü). Her feature `@IntoSet FeatureEntry`+`@IntoSet FeatureNavGraph` katkılar; `:app` `Set<...>` inject edip menüyü permission-filtreyle **dinamik** kurar, NavHost'u iterate eder. **Rollout kuralı:** yeni feature = `feature/<slug>/` + settings include (CT, dizinleri ÖNCE oluştur) + **`:app`'e tek dep satırı** (CT aggregation — @IntoSet keşfi için şart) + agent app'e dokunmaz.
> **⚠️ K2 kritik bulgu (Wave 1):** backend `ReadinessState` int kodlaması modüller arası TUTARSIZ — applicant-intake `Deferred=1/Ready=2`, diğer 4'ü `Ready=1/Deferred=2`. Her modülün Kotlin enum'u kendi C# enum'una karşı ÇAPRAZ doğrulandı (varsayılsaydı sessiz veri bozulması). **Her rollout modülünde enum'u ölç, varsayma.**
> **Dev connectivity (6dd49f77):** bu ortamda emülatör 10.0.2.2:5080 NAT'ı stall (ping OK, TCP değil); debug `BASE_URL=localhost:5080` + `adb reverse tcp:5080 tcp:5080` (adb köprüsü). Emülatör/adb restart sonrası adb reverse tekrar. Seed login: tenant `00000000-…-001`, `admin@diten.com` / `Admin123!` (SuperAdmin).
>
> **M1 rollout Dalga 2 (bed19f81) — TAMAM:** compensation-benefits, competency-skills, learning-training, time-attendance-leave (hepsi Ready=1/Deferred=2 enum, 27 test). Menüde **9 HR modülü** canlı (permission-gated, plugin-discovered). Kalan: Dalga 3–5 (11 standart) + 3 özel.
>
> **Offline-first sync bug'ları (canlı debug, 5 defekt) — ÇÖZÜLDÜ:** (1) `encodeDefaults=true` — push tam gövdeyi gönderiyor (eskiden default alanlar atlanıp 400'e düşüyordu) [6a76582a]; (2) WorkManager `NetworkType.NOT_REQUIRED` — emülatör validated-network false-negative'i aşıldı; (3) unique-work `REPLACE`/`UPDATE` — eski bloklu iş yok sayılmıyor; (4) M0.6 demo `ReadinessSyncHandler` binding kaldırıldı — Set'i RETRY'a zehirlemiyordu; (5) template sweep [bed19f81] — SyncHandler PENDING+FAILED retry + Unauthorized→Transient, delete sync-olmamış/404 yerel siler. **Backend (475ff5b9):** `RefreshTokenCommandHandler` refresh'te `legal_entities` claim'ini düşürüyordu → scoped write 403 → düzeltildi (login gibi legalEntities geçiriliyor). **Canlı kanıt:** takılı 4 mobil kayıt taze login sonrası 201 → SYNCED → backend'de (web'de) görünüyor; mobil↔web çift yönlü sync tam çalışıyor.

> **M1 pilot (`:feature:applicant-intake`) — ŞABLON:** Clean Architecture (domain/data/presentation), offline-first. Domain `ReadinessState` (int 0-5) + model + repo arayüzü. Data: DTO **int-state** + `sourceContractVersion="v1"` (ölçülen sözleşme; HCM string-enum converter yok) · `ApplicantIntakeApi` (ölçülen route'lar) · **kendi Room DB'si** (`:core:database` ScopeKeys/converters/SyncStatus reuse) · offline-first repo (create→PENDING→`enqueueOneTimeSync`; observe Room SSOT tenant+legal-entity scoped; refresh→SYNCED) · **`ApplicantIntakeSyncHandler @IntoSet`** (M0.6'nın ertelenen gerçek readiness sync'i) · marker-safe Code auto-default. Presentation MVI+`:core:design`: List(StatusChip+pull-refresh+FAB)/Create(auto-code+facet dropdown)/Detail(evaluate/delete), `FeatureEntry` `hcm.applicant-intake` gate'li. `:app` menü→gerçek ekranlar. CT: 22 test (backend'siz create + scope izolasyon + sync + int-enum), canlı emülatörde çökmesiz açılış.
>
> **Feature şablon reçetesi (rollout için):** her HR modülü = ölç (routes/DTO/enum/permission) → `:feature:<slug>` (domain enum+model+repo · data DTO(int-state)+Api+kendi RoomDB(ScopeKeys reuse)+offline-first repo+`@IntoSet SyncHandler`+marker-safe code · presentation MVI list/create/detail) → `:app` wiring (dep+menü→route+navgraph) → CT doğrula+commit.

> **M0.7 (`:app` shell):** WorkManager app-init (`Configuration.Provider`+`HiltWorkerFactory`, manifest initializer kaldırıldı) · Navigation Compose tek-Activity (login/mfa/home/module rotaları, `authState`→start-destination, logout geri-yığın temizler) · Login MVI (`LoginViewModel`+`AuthRepository`, tenant/email/parola→X-Tenant-Id) + minimal MFA · legal-entity switcher (top bar→`selectLegalEntity`→X-Legal-Entity-Id) · permission-gated HR modül menüsü (`PermissionGate.hasResourceAccess`, JWT permission claim'leri) + M1 placeholder · M0.1 teması emekli→`:core:design DitenTheme`. **CANLI emülatör doğrulaması:** APK kurulur+açılır (çökme yok=WorkManager/Hilt init OK), login Di10 tasarımında render, MVI form+loading çalışır, OkHttp gerçek isteği yapılandırılmış gateway'e (10.0.2.2:5080) ateşler, connectivity hatası uçtan uca "Cannot reach the server" + indigo (#696CFF) butonla haritalanır. (Emülatör↔host :5080 erişimi ortam koşulu; host gateway 401 dönüyor.)
>
> **Bilinen ortam notu (kullanıcı için):** Emülatörde tam yeşil login için emülatörün host `:5080`'ine ulaşması gerekir — gateway'i `0.0.0.0`'a bind et **veya** app `BASE_URL`'ini host LAN IP'sine çevir. Mobil kod tele kadar kanıtlı.

> **M0.6 (`:core:sync` + `:core:common`):** `:core:common` genişletme — MVI base (`UiState/UiEvent/UiEffect` + `MviViewModel`: `StateFlow` state + **tek-seferlik** effect channel `receiveAsFlow` replay-yok, `viewModelScope` leak-safe) · `PermissionGate` (saf, wildcard-yok: exact case-insensitive + `hasResourceAccess` prefix menü görünürlüğü, backend `<servis>.<kaynak>.<aksiyon>`) · `NavRoute`/`FeatureEntry` nav contract. `:core:sync` — `SyncHandler`+`SyncEngine` (Hilt `@IntoSet`, hata-izolasyonlu) + `SyncResult` + `@HiltWorker SyncWorker` + `SyncScheduler` (periyodik CONNECTED + expedited) + `ReadinessSyncHandler` referans (gerçek in-memory Room'a karşı pull PENDING→push→reconcile→SYNCED kanıtı; `ReadinessRemoteSink` `@BindsOptionalOf`, gerçek impl feature'da). **App WorkManager init (`Configuration.Provider`+`HiltWorkerFactory`) M0.7'ye ertelendi.**

> **M0.4 (`:core:auth`):** güvenli token store (`TokenCipher` soyutlaması → prod `KeystoreTokenCipher` AES/GCM AndroidKeyStore + test `PlaintextTokenCipher`; DataStore, logout'ta wipe) · JWT decoder (imza doğrulamasız; `tenant_id`, `permission` string-or-array, **`legal_entities` CSV**→List split/trim) · `SessionManager` (`StateFlow<AuthState>`, restore, `selectLegalEntity`) · `AuthRepository` login(X-Tenant-Id)/verifyMfa/refresh/logout · `LoginResult` sealed (Authenticated|MfaRequired) · **gerçek seam** `SessionTokenProviderImpl`+`TokenRefresherImpl` (`dagger.Lazy` ile OkHttp↔refresher döngüsü kırıldı); M0.2 no-op'ları kaldırıldı, `:app`→`:core:auth`. Kanıt: app R8 release Hilt grafı uçtan uca çözülüyor (D).

...
| Service | `Diten.ProcurementService` · port **5065** (was 5062; OD-5 resolved 2026-09-17) |
...
- **AD-1 Ayrı servis/domain:** `Diten.ProcurementService` (port 5065), ayrı procurement domaini — envanterden bağımsız bounded context; iletişim yalnız contract.
...
## 14. Review questions
- Ayrı `Diten.ProcurementService` (5065) doğru mu, yoksa mevcut bir servise mi katılmalı? (öneri: ayrı — bounded context.)
...
## 18. Open decisions
- **OD-1 (RESOLVED, product owner 2026-09-16):** Ayrı procurement domaini + `Diten.ProcurementService` port 5065 (OD-5, was 5062) + contract-first tam MVP-2, MVP-1'i beklemeden (frozen INVENTORY mock'una karşı). Branch `feature/procurement-mvp-2`. **DCP-010 approved (Ali, 2026-09-16); FAZ 1 = MOD-0140 scaffold.**
...
- **OD-5 (RESOLVED, 2026-09-17):** Port çakışması çözüldü. Ölçüm: 5062'yi `Diten.DataKnowledgeService` (çalışıyor) + `Diten.PpmService` (config) tutuyor; 5063 `HumanCapitalService`, 5064 `TalentEcosystemService` (hepsi AGENTS.md §3'te kayıtsız — drift). Karar: en yeni ve henüz fleet'te çalışmayan servis olan **Procurement, en düşük boş porta = 5065'e taşındı** (çalışan/başka-domain servisleri bozmadan, boundary-içi). Uygulandı: appsettings + launchSettings + ocelot 7 route ailesi (14 downstream port) → 5065; Ppm'in `/api/v1/ppm` 5062'de bırakıldı (dokunulmadı); AGENTS.md §3 + tüm procurement docs güncellendi. Runtime smoke 5065'te doğrulandı. **Kalan drift (procurement dışı, EA):** Ppm/DataKnowledge/HumanCapital/Talent'ın §3'e kanonik kaydı domain-owner/EA kararı.

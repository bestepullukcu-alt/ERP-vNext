...
runtime_code_scope: "MOD-0140 ONLY (2026-09-16). DCP-010 approved + MOD-0140 module pack ready-for-dev → FAZ 1 authorizes scaffolding Diten.ProcurementService via @orchestrator /add-module for MOD-0140 (JWT + Mongo V3 GUID subtype-4 + per-query TenantId isolation + port 5062 + gateway route). 0141/0142/0143/0144/0145 remain draft and NOT code-authorized until each reaches its own ready-for-dev gate (CAP-001 §7)."
inputs:
...
> **`status: approved`** (product owner Ali via CT, 2026-09-16). Kararlar (scope/service/port/ayrı domain) onaylandı. **`runtime_code_allowed: true` — yalnız MOD-0140** (ready-for-dev). Diğer üyeler (0141-0145) hâlâ `draft` ve kod-yetkisiz; her biri kendi `ready-for-dev` kapısından geçmeli (CAP-001 §7).
...
| runtime_code_allowed | **true — MOD-0140 only** (ready-for-dev; FAZ 1 scaffold) |

...
| 1b | **WC-1b** frontend wiring + **tenant module manifest / catalog self-registration** | Wire `/WorkCenterNext` from mock → real WC-1 API; add a `WorkAggregation` **manifest provider** (parallels the 6 existing `*ManifestProvider`s) declaring the tenant page + nav (tenant shell) + the `platform.work-aggregation.inbox.view` permission (auto-seeded to AuthService via catalog→auth sync) + 7-lang l10n. **Newly recorded seam ([BL-022](../../../docs/product-backlog.md))** — without it WorkCenter is invisible in tenant nav and returns 403. Additive (self-reg reconcile never revokes); safe **after** WC-1 because the stable identifiers (ModuleCode/permission/shell) are already locked. | WC-1 shipped |

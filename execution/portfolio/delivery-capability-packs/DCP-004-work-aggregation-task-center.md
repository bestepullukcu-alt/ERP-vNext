...
## Decision amendment 2026-09-15 — Work Center for every tenant user (owner; supersedes the access-by-entitlement line)

**Decision (owner, 2026-09-15, BL-410 A + team-view (ii)).**
1. The personal Work Center inbox (`GET api/v1/work-items/mine` with scope self) and the single task read behind the detail page are available to every signed-in tenant user. They no longer require `platform.work-aggregation.inbox.view`; the single read keeps `platform.tasks.read` (record-page parity) and the task read policy. These login-only endpoints are listed by a named attribute and a reflection test.
2. WORK-AGGREGATION becomes a baseline module; its page needs no key, so the sidebar and Ctrl+K show Görev Merkezi to every tenant user.
3. Team view ("Ekibim", `scope=team`, team availability) follows the organization structure: a user whose active position has subordinate positions sees those subordinates' work; no separate key. The task read policy gains the same subordinate leg (BL-417 option a), resolved by the same team resolver, so the team list and the detail page give one answer.
4. Opening the inbox grants no write: every action stays authorized by the source module's own key and relationship rule. A dispatcher that returns a blank key is refused (fail closed).
5. UI follows UAS-001 §6: "+ Yeni ▸ Görev" is hidden without `platform.tasks.create`.
6. Creating and configuring tasks and meetings stays with the entitled modules (TASKS, MEETINGS, WORK-REPORT).

**Evidence.** Blueprint MOD-0023/MOD-0024: workflow and task inbox in the Platform Backbone. This pack :80 and :270-272: one personal work surface modelled on SAP Task Center and Oracle Worklist. SAP Task Center: the `TaskCenterEveryone` role collection gives every user the inbox; Oracle Fusion: the Employee abstract role and the global-header notifications. Manager visibility from the organization structure: Oracle HCM auto-provisions the Line Manager role to users with reports; SAP SuccessFactors builds manager groups from reporting relationships (general vendor practice; not source-checked in this session). Design measurement: WP-WCN-INBOX-FOR-EVERY-TENANT-USER-DESIGN-01 (option O1 + O3b, team view relationship-based instead of a new key).

**Consequences to handle in the implementation WP.** Existing WORK-AGGREGATION entitlement rows (baseline modules refuse disable/remove) — measure and retire through a reviewed data step; `inbox.view` stays in the catalog unenforced until delete-sync exists; managers in other legal entities now see their subordinates' task rows and details (owner accepted the recommendation without a legal-entity restriction).

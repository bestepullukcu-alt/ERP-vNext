...
## 3. Problem statement
Repository module IDs had drifted from the Blueprint: some repo IDs occupied Blueprint numbers reserved for different (SRE/resilience/correlation) modules, several names diverged from Blueprint canonical names, and a parallel legacy namespace (`PSS-*`, `NEW-*`) and repo-only IDs existed without explicit Blueprint reservation. See audit `docs/audits/2026-06/blueprint-module-id-reconciliation-2026-06-03.md`.

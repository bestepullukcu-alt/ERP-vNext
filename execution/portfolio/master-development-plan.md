...
| **MOD-0288** | Organization, Person & Position Directory | — | High | done | 100% | Track G-prime org master-data keystone (platform-shared-services). Runtime merged (entities/controllers/repositories under services/Diten.Platform); governance reconciled to done. Depends on MDM Legal Entity read-only LegalEntityId contract. Tenant User validation and Position-role binding are deferred behind explicit guards. |
| **MOD-0288-FU01** | Position Assignment User Reference Validation | — | High | done | 100% | Platform follow-up for PositionAssignment.UserId AuthService lookup-validation consumer integration. Runtime merged via PR #24 / d816db6 (510 tests pass); governance reconciled to done. MOD-0047 contract dependency complete. |

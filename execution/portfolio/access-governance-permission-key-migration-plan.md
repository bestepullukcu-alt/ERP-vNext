...
| **5A** ✅ **READY** | **D-4 Platform tenant verb aliases** *(needs 1B; map-covered)* — split from old Slice 5 per live-repo audit | `platform.tenants.quotas.view → …read`; `platform.tenants.commercial.subscription.view → …read`; `platform.tenants.commercial.subscription.history.view → …read` (3 distinct keys) | **3 files / 10 enforcement points:** `QuotasController.cs` (2 attr), `TenantCommercialSubscriptionsController.cs` (5 attr), `TenantModuleEntitlementsController.cs` (`const ViewPermission`, applied ×3). Alias map §1.3 already covers all 3 (canonical `.read` ← legacy `.view`) → **dual-read safe via existing Platform seam**. Intentional seam fixtures (`PermissionAliasResolverTests`, `HasPermissionAttributeDualReadTests`) **kept**; no surface-test coupling | each canonical `.read` enforces; legacy `.view` grant passes via 1B dual-read; bypass unchanged | revert the 3 files | **single atomic runtime commit** (tenant view→read) |
| **5B** 🛑 **BLOCKED — separate scoping audit required** | **D-3 EnterpriseStrategy `_ → -`** *(out of Platform seam scope)* — split from old Slice 5 per live-repo audit | `strategy.planning_cycle.{view,create} → strategy.planning-cycle.{view,create}`; `strategy.strategy_period.{view,create} → strategy.strategy-period.{view,create}` (4 const keys) | **`Diten.EnterpriseStrategyService`** — 4 consts in `EnterpriseStrategyPlatform.cs`, 27 applications via **`[EnterpriseStrategyPermission]`** (separate enforcement stack, `service.HasPermissionAsync`). **NOT in the Platform alias map; EnterpriseStrategy has no alias/dual-read seam; grant source unverified (no in-repo seeder ref).** D-3 only — strategy `.view/.edit` verb normalization (D-4) is out of scope | **none in this step.** Pre-req: trace `HasPermissionAsync` grant source → if grants derive from the same consts (single source → atomic rename, à la 1A) it's clean; else design an EnterpriseStrategy-side compatibility shim first | n/a (no change) | **no runtime change; does not auto-proceed** — a blind cross-service rename is forbidden |

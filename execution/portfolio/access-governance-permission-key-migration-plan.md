...
| **6** ✅ **DONE** | **Standards reconciliation** — completed in integration branch, **docs commit `0dd362d`**; no runtime/test change; **not pushed / no PR / not merged to `main`** | docs only (§8) | **7 `.antigravity/**` standards/agent docs** (erp-architecture, module-pack-standard, security-jwt, response-envelope, agents: backend-architect, security-agent, product-manager) — stale PascalCase `Platform.*`/`Modules.*` examples → PKS-001 lowercase-dotted (kebab-case multiword, `View→read`). **PKS-001 & BME-001 intentional historical/census records preserved (untouched)**; `module-pack-author.md` already clean (excluded) | grep: no PascalCase mandate remains in the 7 docs; no runtime diff | revert the 7 docs | docs-only commit `0dd362d` |
| **7** 🛑 **BLOCKED — Option C: external evidence required** *(readiness audit done; alias map NOT removed/narrowed; no runtime/test change)* | **Alias retirement** | remove aliases | **Readiness audit (PKS-001 §9):** production `[HasPermission]` legacy `Platform.*`/`Modules.*`/tenant `.view` = **0** (criterion 4 ✅). But the 55 aliases bridge the **JWT `permission` claim**, whose spelling is **external**: AuthService `TokenService` is **pass-through** (`new Claim("permission", permission)`), AuthService carries **no in-repo Platform permission catalog**, and the real strings live in the external `Permission`/`RolePermission` **DB** (criteria 1/2/3 ❌ unverifiable). **No deprecation window** has elapsed (migration unmerged; `main` = `d3ab4a4`; merge-freeze) → criteria 6/7 ❌. **0 aliases safely removable from repo evidence; all 55 require external evidence.** Intentional consumers (resolver tests `count==55`, dual-read tests, PKS-001 census) are **not** dead code | full suite green; grep zero legacy keys anywhere **AND** external grant-catalog confirmed canonical + deprecation window elapsed | re-add alias rows | retirement commit per namespace — **deferred; not executed** |

...
| Plan Step | Capability / Module | Canonical ID | Current State | Pack Needed? | Dependencies | Parallel? | Owner Agent | Repo Scope | Build/Test Gate | Done Criteria | Unlocks |
|---|---|---|---|---|---|---|---|---|---|---|---|
| AG-STEP-010 | Cache-invalidation convention | MOD-0018-FU13 (**Permission Convention + Cache Invalidation Events**) | **pack `ready-for-dev` in integration branch (commit `ba13177`); runtime implementation NOT started** — pack `execution/domains/platform-shared-services/module-packs/MOD-0018-FU13-permission-convention-cache-invalidation.md`; identity gate PASS. **Locked decisions:** OD-FU13-01 access-token staleness **≤15 min** (refresh re-reads grants; reuse existing `RevokeAllByUserAsync`; deny-list + new cross-service role-change events **deferred**); OD-FU13-02 **per-instance fan-out** receive endpoint mandatory **before horizontal scaling** (`IMemoryCache` retained; Redis **deferred**); OD-FU13-03 **no cross-request data-scope cache** (resolver request-fresh; future fail-closed eviction guard locked). **No open decision remains. Not pushed / no PR / not merged to `main`.** | yes | AG-STEP-009 | yes (pack ‖ 009) | module-pack-author → backend-architect | pack + Platform cache wiring | tests | scope/context caches evicted on change events | live scope changes |

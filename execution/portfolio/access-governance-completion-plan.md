...
>
> **Post-merge refresh (current).** `main` is now `473a7f4`. The Access Governance integration branch `feature/governance/access-governance-execution` has been **merged to `main` via PR #28** (preceded by the plan-refresh PR #27 → `169acf8` and PR #26 → `d3ab4a4`). The merge-freeze is **over**; normal per-branch PR flow is restored. All formerly "pushed to origin / no PR / not merged to `main`" integration-branch milestones below — AG-STEP-002, 006A, 009, 010, 011, 012, 013, 021 and the completed AG-STEP-004B slices, plus the MOD-0288/FU01 drift closure — are **now live on `main`**. The authoritative live-state record is the **Current Execution State** section below; where any later per-step row or paragraph still says "not merged to `main`", "pending batch merge", "merge freeze", or quotes baseline `d3ab4a4`, that wording is **superseded by this refresh** and retained only as historical step-completion evidence.
...
1. **Blueprint** — `docs/reference/blueprint/System Capability & Implementation Blueprint - master 8.1.xlsx`, sheet `Blueprint_Data`.
...
**Spine status (now merged to `main` via PR #28 — `main` @ `473a7f4`):** **Now DONE and live on `main`:** real `IDataScopeResolver` (MOD-0018-FU15) — production DI binds `IDataScopeResolver → OrgDataScopeResolver` (commit `26d4fe7`); `NoOpDataScopeResolver` is **no longer the production default**; FU12 hydrates real scopes; **Explain Access (FU14)** self-explain v1 complete; **cache-invalidation convention (FU13)** runtime complete (RabbitMQ two-instance verification **OPEN as a rollout gate only**). **Still open / not real:** **temporary access (FU11)** NoOp; **Tenant Group / Group→Role** absent in code with no Blueprint ID; **permission-key naming inconsistent** (4 styles); `[HasPermission]` **not universal** (Platform-only; AuthService/MDM ungated); **governance drift (MOD-0288/FU01) is now closed** — packs/registry/master-plan/DCP-001 reconciled to `done` and merged via PR #25; **DCP-002 promotion (`draft → approved`) completed via AG-STEP-002 / commit `3b9d342` and merged to `main` via PR #28**.
...
- AG-STEP-002: **completed and merged to `main` via PR #28** (commit `3b9d342`) — DCP-002 `status: draft → approved`; §16/§21/§14/§19 wording reconciled with §16a; `verify_module_id.py --check-all` → **EXIT 0**
- AG-STEP-006A: **completed and merged to `main` via PR #28** (commit `91a2604`) — BME-001 mandatory `[HasPermission]` enforcement rule for new business-domain modules (`.antigravity/rules/business-module-enforcement-standard.md`)
- AG-STEP-009: **completed and merged to `main` via PR #28** (commit `26d4fe7`) — real `OrgDataScopeResolver`; production DI `IDataScopeResolver → OrgDataScopeResolver`, `NoOpDataScopeResolver` no longer the production default; **G4 integration audit PASSED**; tests **528 passed, 0 failed**; blocker **none**
- AG-STEP-010: **completed and merged to `main` via PR #28** (commits `3a8f9dd`/`a9ad416`/`34e38cc`/`c035bf5`) — MOD-0018-FU13 cache-invalidation convention + runtime; **rollout gate still OPEN** (RabbitMQ two-instance fan-out verification — mandatory before horizontal scaling, not yet verified; not a merge blocker)
- AG-STEP-011: **completed and merged to `main` via PR #28** (commits `a2445b8`/`9ddb36f`/`bba038a`/`35308a0`) — MOD-0018-FU14 self-explain v1; cross-user explain deferred (`OD-FU14-08B`/`OD-FU14-09`)
- AG-STEP-012: **completed and merged to `main` via PR #28** — OD-D audit policy LOCKED as deny-only baseline; allow-logging runtime deferred
- AG-STEP-013: **completed and merged to `main` via PR #28** (commit `45561e7`) — BME-001 Part II business-module permission + data-scope enforcement contract (C1–C8) **and** repo-grounded reference pattern documented
- AG-STEP-021: **Business-Domain Start Gate — PASS, now live on `main`** (audit @ `02362a3`; merged via PR #28); all Backend Start Gate criteria (000/001/004/009/013/006A + tenant isolation) verified. `main` @ `473a7f4` now carries the real `OrgDataScopeResolver` default + BME-001 → the gate is **unlocked on `main`**. Business-domain backend may begin; production rollout / module selection (AG-STEP-022) remains a later, explicit, Blueprint-driven step (no automatic rollout)
- AG-STEP-004B: **merged to `main` via PR #28; partially BLOCKED on external evidence (Slices 5B & 7 — deferred follow-up, not a backend-start blocker)** — migration design + compatibility map authored (commits `ba57460`, `257a68c`); **Slice 1A (D-5) done** (runtime commit `64417c2`; security audit PASS; 25 tests) — MDM Legal Entity controller switched to canonical `mdm.legal-entities.*`, no alias seam needed; **Slice 2 (D-6 validator) done** (runtime commit `20d9306`; audit PASS; 532 tests) — both ModulePage `IsCanonicalPermission` validators relaxed `== 3 → >= 3`, uppercase normalized-accepted; **Slice 1B (Platform-only alias seam) done** (Commit A `129c62e`, Commit B `a800407`; security audit PASS; 554 tests) — 55-entry immutable Platform alias map + dual-read in `HasPermissionAttribute`; canonical OR legacy alias claim → allow; bypass unchanged; AuthService/DevEnablement/MDM untouched; **Slice 3 (Platform.* rename) done** (audit PASS; 554 tests; legacy `Platform.*` = 0) — 80 uses / 32 keys / 8 controllers → canonical, 100% map coverage; 6 commits (`ac12223`…`2768b2c`); alias map/wiring unchanged; **Slice 4 (Modules.* org rename) done** (audit PASS; 554 tests; Platform legacy `Modules.*` = 0) — 45 uses / 20 keys / 6 controllers → canonical, 100% map coverage; 4 commits `749fc3d` module-catalog · `d3d3fda` organization-units · `172c837` positions+manager-chain · `4395125` position-assignments; alias map/wiring unchanged, dual-read fixture preserved; **Slice 5A (tenant verb view→read) done** (runtime commit `cb3fda5`; 554 tests) — 3 keys / 10 enforcement points / 3 files → canonical `.read`, dual-read still supports legacy `.view` claims; **Slice 5B (EnterpriseStrategy `_→-`) BLOCKED — Option C: external evidence required** (grant-source audit done; no runtime change; EnterpriseStrategy untouched) — `[EnterpriseStrategyPermission]` → `DefaultEnterpriseStrategyAuthorizationService.HasPermissionAsync`: dev-bootstrap path uses the const as single source (rename self-consistent) but the enforced path exact-matches an **external JWT `permission` claim with no alias/normalization**; no ES-side seam, no in-repo issuer/seeder. **External fact needed:** does the production issuer/AuthService grant these 4 keys with `_` or `-`, and is ESBP enforced-mode live? → Option A (1-file atomic rename) if no live `_` grant; Option B (ES compatibility shim first) if it can; **Slice 6 (standards reconciliation) done** (docs commit `0dd362d`; no runtime/test change) — 7 `.antigravity/**` standards/agent docs: stale PascalCase `Platform.*`/`Modules.*` examples → PKS-001 lowercase-dotted; PKS-001 & BME-001 intentional records preserved; **Slice 7 (alias retirement) BLOCKED — Option C: external evidence required** (readiness audit done; alias map NOT removed/narrowed; no runtime/test change) — production legacy `[HasPermission]` = 0 (criterion 4 met), but the 55 aliases bridge the JWT `permission` claim whose spelling is external (AuthService `TokenService` pass-through; real strings in the `Permission`/`RolePermission` DB; no in-repo Platform permission catalog), and no post-merge/deploy dual-read deprecation window has yet elapsed → 0 aliases safely removable from repo evidence. **Both 5B and 7 remain BLOCKED pending external grant-catalog evidence (deferred follow-ups, out of AG-INFRA-COMPLETION scope).** Latest blocker-doc commit `95a1b97` is **live on `main` (PR #28)**.
...
| Plan Step | Capability / Module | Canonical ID | Current State | Pack Needed? | Dependencies | Parallel? | Owner Agent | Repo Scope | Build/Test Gate | Done Criteria | Unlocks |
|---|---|---|---|---|---|---|---|---|---|---|---|
| AG-STEP-021 | Business-Domain Start Gate verification | — | **integration-branch PASS** (audit HEAD `02362a3`); **main unlock PENDING BATCH MERGE** | no (audit) | AG-STEP-000, 001, 004, 009, 013, 006A | — | read-only-auditor | read-only | none | **DONE (integration branch).** All Backend Start Gate criteria verified against live repo @ `02362a3`: AG-STEP-000 ✓, 001 (MOD-0288 + FU01 `done`) ✓, 004 (PKS-001 OD-C locked) ✓, 009 (`IDataScopeResolver → OrgDataScopeResolver`; NoOp not prod default) ✓, 013 (BME-001 Part II contract) ✓, 006A (BME-001 Part I rule) ✓, tenant isolation (`ExecutionFilter` tenant+soft-delete; server-side `tenant_id` claim; BME-001 C4 "no TenantId from request") ✓. **`main` still `d3ab4a4`; the integration branch is tracked on origin and checkpointed there; routine docs-only roadmap-maintenance commits may briefly lead the pushed tip until the next checkpoint push. Real unlock gated on batch merge — not unlocked on `main`.** | business-domain backend start |
...
> **Gate result (AG-STEP-021, audit @ `02362a3`):** **integration-branch PASS** — every Backend Start Gate
...
> integration branch is **tracked on origin and checkpointed there**; routine docs-only roadmap-maintenance commits may briefly lead the pushed tip until the next checkpoint push. On `main` the production `IDataScopeResolver`
...
| Real DataScopeResolver | MOD-0018-FU15 | reserved | `ready-for-dev` | No | **runtime DONE in integration branch** (commit `26d4fe7`); real `OrgDataScopeResolver` is prod default (NoOp not default); emits OrgUnit/Position/ManagerChain/LegalEntity (Company in enum, not emitted; Country not emitted); pushed to origin; **not merged to `main`** |
| Cache-invalidation convention | MOD-0018-FU13 | reserved | `ready-for-dev` | No | **runtime DONE in integration branch** (commits `3a8f9dd`/`a9ad416`/`34e38cc`/`c035bf5`); **rollout gate OPEN**: RabbitMQ two-instance verification; **not merged to `main`** |
| Explain Access | MOD-0018-FU14 | reserved | `ready-for-dev` | OD-D locked | **self-explain v1 runtime DONE in integration branch** (commits `a2445b8`/`9ddb36f`/`bba038a`/`35308a0`); Allow Audit deny-only baseline LOCKED, allow logging runtime deferred; cross-user deferred (`OD-FU14-08B`/`OD-FU14-09`); **not merged to `main`** |

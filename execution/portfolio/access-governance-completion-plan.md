...
| AG-STEP-011 | Explain Access (decision trace) | MOD-0018-FU14 | **pack `ready-for-dev` in integration branch — commit `7f1d005`; runtime implementation NOT started** (pack MOD-0018-FU14 *Effective Access Explain + Allow Audit*, `execution/domains/platform-shared-services/module-packs/MOD-0018-FU14-effective-access-explain-allow-audit.md`; identity gate PASS). **Scope split:** AG-STEP-011 = Explain Access decision trace; **AG-STEP-012 = Allow Audit policy (separate EA `OD-D` — NOT complete)**; explain-request audit ≠ allow-decision audit. **Locked (v1):** self-explain own-subject-only; cross-user only `platform_admin`/`partner_admin` via marker `platform.access.explain` (PKS-001, actor-bypass-covered → no seed/migration/alias-row in v1); explicit tenant isolation; not a grant-inventory endpoint; backend-only v1 (frontend/gateway deferred); bounded reason-code model (no raw exception text); ALL explain requests audited (self/cross-user/denied/diagnostic-failure); alias visibility = `matchedViaLegacyAlias` boolean only; token-freshness = expiry + bounded refresh-required note; scope-detail default kinds+counts (raw scope ids / raw JWT / claims / secrets out). **Deferred — OD-FU14-08B:** non-bypass tenant-admin/auditor extension (external AuthService Permission/RolePermission DB catalog migration + role-grant + external evidence) — **not a v1 runtime blocker, out of v1 scope, NOT complete**. **Runtime Groups A–E NOT started** (A reason model+observer · B self-explain · C cross-user · D tests+audit · E governance reconciliation). FU13 RabbitMQ horizontal-scaling rollout gate unchanged; Slice 5B/7 blockers unchanged; AG-STEP-008 stale row not fixed here. **Pushed to integration branch; no PR / not merged to `main` (origin/main `d3ab4a4`).** | yes | AG-STEP-009 | yes | module-pack-author → backend-architect | pack + Platform | tests | allow/deny provenance surfaced (basic flow) | debuggable access |

...
| AG-STEP-011 | Explain Access (decision trace) | MOD-0018-FU14 | **pack `ready-for-dev` in integration branch; runtime NOT started — FU14 v1 narrowed to SELF-EXPLAIN ONLY** (pack `7f1d005`; roadmap milestone `8c1d8fb`; self-explain-only scope `bc9435e`; pack MOD-0018-FU14 *Effective Access Explain + Allow Audit*, `execution/domains/platform-shared-services/module-packs/MOD-0018-FU14-effective-access-explain-allow-audit.md`; identity gate PASS). **Scope split:** AG-STEP-011 = Explain Access decision trace; **AG-STEP-012 = Allow Audit policy (separate EA `OD-D` — NOT complete)**; explain-request audit ≠ allow-decision audit. **v1 = self-explain only:** authenticated caller explains only its OWN effective-access; **no caller-controlled `subjectUserId`** (subject from JWT `sub`/NameIdentifier + tenant context); proposed route `GET api/platform/access/explain/me` (proposal only); self route `[Authorize]`; tenant isolation mandatory; not a grant-inventory endpoint; runtime not started. **Side-effect boundary:** the explain observer does NOT call the real `HasPermissionAttribute` filter (its platform-actor branch mutates admin lifecycle — invitation-acceptance + last-login + repo update) — Group A extracts a shared pure `PermissionClaimEvaluator`, `HasPermissionAttribute` delegates claim-matching to it, the observer uses only the pure evaluator, enforcement preserved by regression. **Data-scope:** read-only `OrgDataScopeResolver.ResolveAsync(tenantId, userId, …)` reuse → kinds+counts only (raw scope ids/JWT/claims/secrets out); empty-scope⇒deny + LegalEntity live fail-closed preserved. **Token-freshness:** expiry + bounded refresh-required note only (no token-version/revocation-timestamp/cache-debug). **Reserved marker:** `platform.access.explain` (PKS-001) is RESERVED for the future cross-user route, NOT active on the self-v1 route (no seed/migration/alias-row; not written to runtime). **Audit (honestly bounded):** every self-explain execution reaching the handler is audited (success/app-level-deny/diagnostic-failure) via `IAuditService.AppendAsync`; audit-write failure does not change enforcement; a pre-handler `[Authorize]` 401 is NOT guaranteed to emit a FU14 AuditEvent (no new middleware/filter seam invented); explain audit ≠ AG-STEP-012 allow-audit. **Cross-user explain DEFERRED:** Platform has no seam to read another user's effective grants (they live in AuthService RolePermission DB / the target JWT); cross-user data-scope-only alternative REJECTED (permission-less trace misleading); no AuthService cross-service contract invented; cross-user runtime not started. **Deferred — OD-FU14-08B** (non-bypass tenant-admin/auditor extension; external AuthService catalog migration + role-grant + external evidence) and **OD-FU14-09** (cross-user target effective-grants contract; bounded DTO + tenant-isolation + grant-inventory-leak-prevention + audit/redaction + Platform→AuthService client extension) — both NOT v1 blockers, NOT complete, distinct from AG-STEP-012. **Runtime Groups A–D (self-explain-only) NOT started** (A pure evaluator+reason model+`HasPermissionAttribute` delegation · B self-explain backend flow · C self-explain tests+integration audit · D governance reconciliation; deferred cross-user follow-up = reserved marker + OD-FU14-08B + OD-FU14-09); next runtime step needs separate authorization. FU13 RabbitMQ horizontal-scaling rollout gate unchanged; Slice 5B/7 blockers unchanged; AG-STEP-008 stale row not fixed here. **Pushed to integration branch (`7f1d005`/`8c1d8fb`/`bc9435e`); no PR / not merged to `main` (origin/main `d3ab4a4`).** | yes | AG-STEP-009 | yes | module-pack-author → backend-architect | pack + Platform | tests | allow/deny provenance surfaced (basic flow) | debuggable access |

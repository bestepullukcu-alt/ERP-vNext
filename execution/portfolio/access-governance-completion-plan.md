...
- AG-STEP-004B: **IN PROGRESS** — migration design + compatibility map authored (commits `ba57460`, `257a68c`); **Slice 1A (D-5) done** (runtime commit `64417c2`; security audit PASS; 25 tests pass) — MDM Legal Entity controller switched to canonical `mdm.legal-entities.*`, no alias seam needed. **Remaining slices (1B/2/3/4/5/6/7) not started.** **Slice 1A not yet pushed / no PR / not merged to `main`**
...
| **AG-STEP-004B** | **Existing permission-key migration plan and compatibility strategy** | MOD-0018 (governance → impl) | **IN PROGRESS (integration branch).** Migration design + compatibility map authored (`access-governance-permission-key-migration-plan.md`); slice ordering reconciled. **Only Slice 1A (D-5) done** (runtime commit `64417c2`; security audit PASS; 25 tests pass) — MDM `LegalEntitiesController` 6 legacy attrs → canonical `mdm.legal-entities.*`, **no alias seam (not needed for D-5)**. **Remaining slices not started:** 1B global alias seam, 2 (D-6 validator), 3–5 (Platform.*/Modules.*/strategy renames), 6 standards reconciliation, 7 alias retirement. **Not pushed / no PR / not merged to `main`.** | yes (migration milestone) | AG-STEP-004 | no | backend-architect | migration design doc; impl per slice | tests per slice | **no blind mass-rename**; identifies every impacted surface (seed, role-permission, JWT claims, HasPermission attrs, frontend nav, tests, audit); uses deprecated alias / compatibility map where needed; runtime migration is a separate controlled implementation milestone | clean migration |

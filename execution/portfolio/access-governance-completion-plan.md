...
**Spine status (integration branch — not merged to `main`; merge freeze ongoing):** **Now DONE in the integration branch:** real `IDataScopeResolver` (MOD-0018-FU15) — production DI binds `IDataScopeResolver → OrgDataScopeResolver` (commit `26d4fe7`); `NoOpDataScopeResolver` is **no longer the production default**; FU12 hydrates real scopes; **Explain Access (FU14)** self-explain v1 complete; **cache-invalidation convention (FU13)** runtime complete (RabbitMQ two-instance verification **OPEN as a rollout gate only**). **Still open / not real:** **temporary access (FU11)** NoOp; **Tenant Group / Group→Role** absent in code with no Blueprint ID; **permission-key naming inconsistent** (4 styles); `[HasPermission]` **not universal** (Platform-only; AuthService/MDM ungated); **governance drift (MOD-0288/FU01) is now closed** — packs/registry/master-plan/DCP-001 reconciled to `done` and merged via PR #25; **DCP-002 promotion (`draft → approved`) completed in the integration branch via AG-STEP-002 / commit `3b9d342`; the work commit is pushed to origin and is not merged to `main`**.

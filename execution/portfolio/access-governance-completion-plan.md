...
- AG-STEP-021: **Business-Domain Start Gate — integration-branch PASS** (audit @ `02362a3`); all Backend Start Gate criteria (000/001/004/009/013/006A + tenant isolation) verified against the live repo. **Main unlock `PENDING BATCH MERGE`** — all work commits through upstream `02362a3` are pushed to origin; local docs-only reconciliation commit(s) remain ahead of upstream until the next checkpoint push; `main` still `d3ab4a4`; **no production business-domain rollout until merged**; no automatic rollout
...
> integration branch's work commits through upstream `02362a3` are **pushed to origin**, while local docs-only reconciliation commit(s) remain **ahead of upstream until the next checkpoint push**. On `main` the production `IDataScopeResolver`

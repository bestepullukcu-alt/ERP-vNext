...
data class CompensationBenefitsListState(
    val items: UiResult<List<CompensationBenefitsListItem>> = UiResult.Loading,
    val isRefreshing: Boolean = false,
) : UiState

sealed interface CompensationBenefitsListEvent : UiEvent {
    /** Pull-to-refresh: pull the server list into the cache. */
    data object Refresh : CompensationBenefitsListEvent
}

sealed interface CompensationBenefitsListEffect : UiEffect {
    /** A transient message (e.g. a failed refresh) to surface to the user. */
    data class ShowMessage(val message: String) : CompensationBenefitsListEffect
}

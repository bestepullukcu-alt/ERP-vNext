...
import eu.grandmedical.diten.mobile.feature.compensationbenefits.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [CompensationBenefitsApi];
 ...
 */
class CompensationBenefitsSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in unsynced) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing compensation-benefits row ${row.id}")
    ...
    private suspend fun push(row: CompensationBenefitsEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: CompensationBenefitsEntity, serverId: String) {

...
@Composable
fun CompensationBenefitsDetailRoute(
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: CompensationBenefitsDetailViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is CompensationBenefitsDetailEffect.ShowMessage -> snackbarHostState.showSnackbar(effect.message)
                CompensationBenefitsDetailEffect.NavigateBack -> onBack()
            }
        }
    }

    DitenScaffold(
        title = "Ücret & Yan Hak Kaydı",
        modifier = modifier,
        navigationIcon = {
            ...
            UiStateContainer(state = state.record) { record ->
                if (record == null) {
                    ...
                    DetailContent(
                        record = record,
                        isBusy = state.isBusy,
                        onEvaluate = { viewModel.onEvent(CompensationBenefitsDetailEvent.Evaluate) },
                        onDelete = { viewModel.onEvent(CompensationBenefitsDetailEvent.Delete) },
                    )
                }
            }
        }
...
private fun DetailContent(
    record: CompensationBenefitsReadiness,
    isBusy: Boolean,
    onEvaluate: () -> Unit,
    onDelete: () -> Unit,
) {
    Column(
        ...
        Text(
            text = "${record.code} · ${record.sourceContractVersion} · v${record.compensationBenefitsReadinessVersion}",
        ...
        FacetRow("Ücret & yan haklar durumu", record.compensationBenefitsReadinessState)
        FacetRow("Ücret planı sınırı", record.compensationPlanBoundaryState)
        FacetRow("Yan hak programı sınırı", record.benefitProgramBoundaryState)
        FacetRow("Ücret kademesi eşleme sınırı", record.payGradeMappingBoundaryState)
        FacetRow("Yan hak kaydı sınırı", record.benefitEnrollmentBoundaryState)
        FacetRow("Ücret değerlendirme sınırı", record.compensationReviewBoundaryState)
        FacetRow("Otomatik karar sınırı", record.automatedDecisionBoundaryState)
        FacetRow("Ücret kaynağı bağımlılığı", record.compensationSourceDependencyState)
        FacetRow("Yan hak sağlayıcı bağımlılığı", record.benefitProviderSourceDependencyState)

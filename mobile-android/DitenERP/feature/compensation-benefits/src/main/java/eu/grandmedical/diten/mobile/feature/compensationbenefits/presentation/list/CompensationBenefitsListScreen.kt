...
@Composable
fun CompensationBenefitsListRoute(
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: CompensationBenefitsListViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is CompensationBenefitsListEffect.ShowMessage ->
                    snackbarHostState.showSnackbar(effect.message)
            }
        }
    }

    CompensationBenefitsListScreen(
        state = state,
        snackbarHostState = snackbarHostState,
        onRefresh = { viewModel.onEvent(CompensationBenefitsListEvent.Refresh) },
...
@Composable
private fun CompensationBenefitsListScreen(
    state: CompensationBenefitsListState,
    snackbarHostState: SnackbarHostState,
    onRefresh: () -> Unit,
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            UiStateContainer(
                ...
                CompensationBenefitsList(items = items, onOpenDetail = onOpenDetail)
            }
        }
    }
}
...
@Composable
private fun CompensationBenefitsList(
    items: List<CompensationBenefitsListItem>,
    onOpenDetail: (String) -> Unit,
) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(items, key = { it.id }) { item ->
            DitenListRow(
                ...
                trailing = { StatusChip(state = item.compensationBenefitsReadinessState.name) },
            )

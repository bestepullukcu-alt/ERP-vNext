...
class CompensationBenefitsCreateViewModelTest {
    ...
    fun defaultCode_isPrefilledAtInit() {
        val vm = CompensationBenefitsCreateViewModel(repository)
        assertTrue(vm.state.value.code.isNotBlank())
        assertTrue(vm.state.value.code.startsWith("CB-"))
    ...
    fun submit_success_emitsNavigateBackOnce() = runTest(mainDispatcher) {
        ...
        val vm = CompensationBenefitsCreateViewModel(repository)
        val effects = mutableListOf<CompensationBenefitsCreateEffect>()
        val job = backgroundScope.launch { vm.effect.toList(effects) }

        vm.onEvent(CompensationBenefitsCreateEvent.DisplayNameChanged("Local only"))
        vm.onEvent(CompensationBenefitsCreateEvent.Submit)

        assertEquals(listOf(CompensationBenefitsCreateEffect.NavigateBack), effects)

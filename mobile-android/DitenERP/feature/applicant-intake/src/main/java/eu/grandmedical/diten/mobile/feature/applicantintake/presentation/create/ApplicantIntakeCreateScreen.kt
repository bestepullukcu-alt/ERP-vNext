...
@Composable
fun ApplicantIntakeCreateRoute(
    onDone: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: ApplicantIntakeCreateViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                ApplicantIntakeCreateEffect.NavigateBack -> onDone()
            }
        }
    }

    ApplicantIntakeCreateScreen(
        state = state,
...
@Composable
private fun ApplicantIntakeCreateScreen(
    state: ApplicantIntakeCreateState,
    onEvent: (ApplicantIntakeCreateEvent) -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
) {
    DitenScaffold(
        title = "Yeni Başvuru Kaydı",
        modifier = modifier,
        navigationIcon = {
        ...
        Column(
            ...
            DitenTextField(
                value = state.code,
                onValueChange = { onEvent(ApplicantIntakeCreateEvent.CodeChanged(it)) },
                label = "Kod",
                supportingText = "Otomatik üretildi; düzenlenebilir.",
            )
            DitenTextField(
                value = state.displayName,
                onValueChange = { onEvent(ApplicantIntakeCreateEvent.DisplayNameChanged(it)) },
                label = "Görünen ad",
            )
            DitenTextField(
                value = state.sourceContractVersion,
                onValueChange = { onEvent(ApplicantIntakeCreateEvent.SourceContractVersionChanged(it)) },
                label = "Kaynak sözleşme sürümü",
            )

            IntakeStateField.entries.forEach { field ->
                ...
                DitenDropdownField(
                    ...
                    onSelected = { name ->
                        onEvent(ApplicantIntakeCreateEvent.StateChanged(field, ReadinessState.valueOf(name)))
                    },
                )
            }
            ...
            DitenTextField(
                ...
                onValueChange = { onEvent(ApplicantIntakeCreateEvent.DeferredReasonChanged(it)) },
            ...
            DitenButton(
                ...
                onClick = { onEvent(ApplicantIntakeCreateEvent.Submit) },

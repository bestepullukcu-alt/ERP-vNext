...
import eu.grandmedical.diten.mobile.feature.applicantintake.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [ApplicantIntakeApi];
 ...
 */
class ApplicantIntakeSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in pending) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing applicant-intake row ${row.id}")
    ...
    private suspend fun push(row: ApplicantIntakeEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: ApplicantIntakeEntity, serverId: String) {

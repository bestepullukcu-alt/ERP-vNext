...
@Composable
fun ApplicantIntakeDetailRoute(
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: ApplicantIntakeDetailViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is ApplicantIntakeDetailEffect.ShowMessage -> snackbarHostState.showSnackbar(effect.message)
                ApplicantIntakeDetailEffect.NavigateBack -> onBack()
            }
        }
    }
            ...
            UiStateContainer(state = state.record) { record ->
                if (record == null) {
                    ...
                    DetailContent(
                        record = record,
                        isBusy = state.isBusy,
                        onEvaluate = { viewModel.onEvent(ApplicantIntakeDetailEvent.Evaluate) },
                        onDelete = { viewModel.onEvent(ApplicantIntakeDetailEvent.Delete) },
                    )
                }
            }
        }
...
private fun DetailContent(
    record: ApplicantIntakeReadiness,
    isBusy: Boolean,
    onEvaluate: () -> Unit,
    onDelete: () -> Unit,
) {
    Column(
        ...
        Text(
            text = "${record.code} · ${record.sourceContractVersion} · v${record.applicantIntakeVersion}",
        ...
        FacetRow("Alım durumu", record.intakeState)
        FacetRow("Kaynak kanalı", record.sourceChannelState)
        FacetRow("Rıza ön koşulu", record.consentPreconditionState)
        FacetRow("Veri minimizasyonu", record.dataMinimizationState)
        FacetRow("Yinelenen kayıt", record.duplicateHandlingState)
        FacetRow("Saklama politikası", record.retentionPolicyState)
        FacetRow("Kanıt politikası", record.evidencePolicyState)
        FacetRow("Aday kimlik sınırı", record.applicantIdentityBoundaryState)
        FacetRow("Genel UX sınırı", record.publicUxBoundaryState)

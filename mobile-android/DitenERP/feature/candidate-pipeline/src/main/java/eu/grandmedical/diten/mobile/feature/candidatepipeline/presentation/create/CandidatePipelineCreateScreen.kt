...
@Composable
fun CandidatePipelineCreateRoute(
    onDone: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: CandidatePipelineCreateViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                CandidatePipelineCreateEffect.NavigateBack -> onDone()
            }
        }
    }

    CandidatePipelineCreateScreen(
        state = state,
...
@Composable
private fun CandidatePipelineCreateScreen(
    state: CandidatePipelineCreateState,
    onEvent: (CandidatePipelineCreateEvent) -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
) {
    DitenScaffold(
        title = "Yeni Aday Havuzu Kaydı",
        modifier = modifier,
        navigationIcon = {
        ...
        Column(
            ...
            DitenTextField(
                value = state.code,
                onValueChange = { onEvent(CandidatePipelineCreateEvent.CodeChanged(it)) },
                label = "Kod",
                supportingText = "Otomatik üretildi; düzenlenebilir.",
            )
            DitenTextField(
                value = state.displayName,
                onValueChange = { onEvent(CandidatePipelineCreateEvent.DisplayNameChanged(it)) },
                label = "Görünen ad",
            )
            DitenTextField(
                value = state.sourceContractVersion,
                onValueChange = { onEvent(CandidatePipelineCreateEvent.SourceContractVersionChanged(it)) },
                label = "Kaynak sözleşme sürümü",
            )

            PipelineStateField.entries.forEach { field ->
                ...
                DitenDropdownField(
                    ...
                    onSelected = { name ->
                        onEvent(CandidatePipelineCreateEvent.StateChanged(field, ReadinessState.valueOf(name)))
                    },
                )
            }
            ...
            DitenTextField(
                ...
                onValueChange = { onEvent(CandidatePipelineCreateEvent.DeferredReasonChanged(it)) },
            ...
            DitenButton(
                ...
                onClick = { onEvent(CandidatePipelineCreateEvent.Submit) },

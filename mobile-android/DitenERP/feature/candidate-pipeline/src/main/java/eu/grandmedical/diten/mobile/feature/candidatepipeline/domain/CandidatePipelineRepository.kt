 ...
 */
interface CandidatePipelineRepository {

    /** Observes the scoped list from the local cache, offline-first. */
    fun observeList(): Flow<List<CandidatePipelineListItem>>

    /** Observes a single record by [id] from the local cache (null if absent). */
    fun observe(id: String): Flow<CandidatePipelineReadiness?>
     ...
     */
    suspend fun create(new: NewCandidatePipeline): UiResult<String>

    /** Asks the backend to (re)evaluate [id], then reconciles the result locally. */
    suspend fun evaluate(id: String): UiResult<CandidatePipelineReadiness>

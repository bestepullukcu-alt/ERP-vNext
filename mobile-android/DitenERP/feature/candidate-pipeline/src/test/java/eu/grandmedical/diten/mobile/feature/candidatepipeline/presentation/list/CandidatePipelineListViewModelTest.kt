...
class CandidatePipelineListViewModelTest {
    ...
    private fun item(id: String) = CandidatePipelineListItem(
        id = id,
        code = id,
        displayName = "Name $id",
        pipelineReadinessState = ReadinessState.Ready,
        sourceContractVersion = "v1",
    )

    @Test
    fun startsLoading_thenRendersContentFromTheCache() = runTest(mainDispatcher) {
        val vm = CandidatePipelineListViewModel(repository)
    ...
    fun refresh_triggersRepositoryRefresh() = runTest(mainDispatcher) {
        ...
        vm.onEvent(CandidatePipelineListEvent.Refresh)

...
class CandidatePipelineSyncHandlerTest {
    ...
    private fun handler() = CandidatePipelineSyncHandler(dao = dao, api = api, safeApiCaller = safeApiCaller)
    ...
    fun pending_isPushed_thenMarkedSynced_withServerId() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeCandidatePipelineApi.ok("server-1") }
    ...
    fun transientServerError_retries_andLeavesRowPending() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeCandidatePipelineApi.error(503) }
    ...
    fun unauthorizedError_retries_andLeavesRowUnchanged() = runTest {
        ...
        seedPending("local-1")
        api.onCreate = { FakeCandidatePipelineApi.error(401) }
    ...
    fun permanentValidationError_fails_andMarksRowFailed_neverSynced() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeCandidatePipelineApi.error(400) }

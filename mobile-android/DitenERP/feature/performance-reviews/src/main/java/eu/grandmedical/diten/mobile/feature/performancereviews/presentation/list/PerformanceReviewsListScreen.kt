...
@Composable
fun PerformanceReviewsListRoute(
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: PerformanceReviewsListViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is PerformanceReviewsListEffect.ShowMessage ->
                    snackbarHostState.showSnackbar(effect.message)
            }
        }
    }

    PerformanceReviewsListScreen(
        state = state,
        snackbarHostState = snackbarHostState,
        onRefresh = { viewModel.onEvent(PerformanceReviewsListEvent.Refresh) },
...
@Composable
private fun PerformanceReviewsListScreen(
    state: PerformanceReviewsListState,
    snackbarHostState: SnackbarHostState,
    onRefresh: () -> Unit,
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            UiStateContainer(
                ...
                PerformanceReviewsList(items = items, onOpenDetail = onOpenDetail)
            }
        }
    }
}
...
@Composable
private fun PerformanceReviewsList(
    items: List<PerformanceReviewsListItem>,
    onOpenDetail: (String) -> Unit,
) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(items, key = { it.id }) { item ->
            DitenListRow(
                ...
                trailing = { StatusChip(state = item.performanceReviewReadinessState.name) },
            )

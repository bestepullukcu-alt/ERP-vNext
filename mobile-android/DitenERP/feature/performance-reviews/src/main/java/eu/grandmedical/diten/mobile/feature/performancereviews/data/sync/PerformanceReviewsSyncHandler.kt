...
import eu.grandmedical.diten.mobile.feature.performancereviews.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [PerformanceReviewsApi];
 ...
 */
class PerformanceReviewsSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in pending) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing performance-reviews row ${row.id}")
    ...
    private suspend fun push(row: PerformanceReviewsEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: PerformanceReviewsEntity, serverId: String) {

package eu.grandmedical.diten.mobile.feature.performancereviews.data
...
import eu.grandmedical.diten.mobile.feature.performancereviews.FakePerformanceReviewsApi
import eu.grandmedical.diten.mobile.feature.performancereviews.FakeScopeProvider
...
import eu.grandmedical.diten.mobile.feature.performancereviews.data.mapper.toEntity
import eu.grandmedical.diten.mobile.feature.performancereviews.domain.NewPerformanceReviews
import eu.grandmedical.diten.mobile.feature.performancereviews.listItemDto
import eu.grandmedical.diten.mobile.feature.performancereviews.readinessDto
 ...
 * [PerformanceReviewsDatabase] plus a fake [FakePerformanceReviewsApi]:
...
class PerformanceReviewsRepositoryImplTest {

    private lateinit var db: PerformanceReviewsDatabase
    private lateinit var dao: PerformanceReviewsDao
    private lateinit var api: FakePerformanceReviewsApi
    ...
    fun setUp() {
        ...
        db = Room.inMemoryDatabaseBuilder(context, PerformanceReviewsDatabase::class.java)
            .allowMainThreadQueries()
            .build()
        dao = db.performanceReviewsDao()
        api = FakePerformanceReviewsApi()
    ...
    private fun repository(activeScope: ScopeKeys? = scope) = PerformanceReviewsRepositoryImpl(
        dao = dao,
        api = api,
    ...
    fun create_writesPendingRow_observableWithoutAnyServerCall() = runTest {
        ...
        val result = repo.create(NewPerformanceReviews(code = "PR-1", displayName = "Local only"))
    ...
    fun refresh_upsertsServerRowsAsSynced_inScope() = runTest {
        api.onList = { FakePerformanceReviewsApi.ok(listOf(listItemDto("s1"), listItemDto("s2"))) }
        val repo = repository()
    ...
    fun observeList_excludesOtherTenantAndLegalEntity() = runTest {
        ...
        repo.create(NewPerformanceReviews(code = "PR-mine", displayName = "Mine"))
    ...
    fun evaluate_updatesTheRow() = runTest {
        dao.upsert(readinessDto("id-1", performanceReviewReadinessState = 0).toEntity(scope)) // Draft
        api.onEvaluate = {
            FakePerformanceReviewsApi.ok(readinessDto("id-1", performanceReviewReadinessState = 1)) // Ready
        }
        val repo = repository()
        ...
        assertTrue(result is UiResult.Success)
        assertEquals(1, dao.getById("id-1")?.performanceReviewReadinessState)
    }
    ...
    fun delete_syncedRow_serverReturns404_stillRemovesLocalRow() = runTest {
        dao.upsert(readinessDto("id-1").toEntity(scope)) // SYNCED
        api.onDelete = { FakePerformanceReviewsApi.error(404) }
        val repo = repository()

...
class EmployeeOnboardingCreateViewModelTest {
    ...
    fun defaultCode_isPrefilledAtInit() {
        val vm = EmployeeOnboardingCreateViewModel(repository)
        assertTrue(vm.state.value.code.isNotBlank())
        assertTrue(vm.state.value.code.startsWith("EO-"))
    ...
    fun submit_success_emitsNavigateBackOnce() = runTest(mainDispatcher) {
        ...
        val vm = EmployeeOnboardingCreateViewModel(repository)
        val effects = mutableListOf<EmployeeOnboardingCreateEffect>()
        val job = backgroundScope.launch { vm.effect.toList(effects) }

        vm.onEvent(EmployeeOnboardingCreateEvent.DisplayNameChanged("Local only"))
        vm.onEvent(EmployeeOnboardingCreateEvent.Submit)

        assertEquals(listOf(EmployeeOnboardingCreateEffect.NavigateBack), effects)

...
@Composable
fun EmployeeOnboardingCreateRoute(
    onDone: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: EmployeeOnboardingCreateViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                EmployeeOnboardingCreateEffect.NavigateBack -> onDone()
            }
        }
    }

    EmployeeOnboardingCreateScreen(
        state = state,
...
@Composable
private fun EmployeeOnboardingCreateScreen(
    state: EmployeeOnboardingCreateState,
    onEvent: (EmployeeOnboardingCreateEvent) -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
) {
    DitenScaffold(
        title = "Yeni Oryantasyon Kaydı",
        modifier = modifier,
        navigationIcon = {
        ...
        Column(
            ...
            DitenTextField(
                value = state.code,
                onValueChange = { onEvent(EmployeeOnboardingCreateEvent.CodeChanged(it)) },
                label = "Kod",
                supportingText = "Otomatik üretildi; düzenlenebilir.",
            )
            DitenTextField(
                value = state.displayName,
                onValueChange = { onEvent(EmployeeOnboardingCreateEvent.DisplayNameChanged(it)) },
                label = "Görünen ad",
            )
            DitenTextField(
                value = state.sourceContractVersion,
                onValueChange = { onEvent(EmployeeOnboardingCreateEvent.SourceContractVersionChanged(it)) },
                label = "Kaynak sözleşme sürümü",
            )

            OnboardingStateField.entries.forEach { field ->
                ...
                DitenDropdownField(
                    ...
                    onSelected = { name ->
                        onEvent(EmployeeOnboardingCreateEvent.StateChanged(field, ReadinessState.valueOf(name)))
                    },
                )
            }
            ...
            DitenTextField(
                ...
                onValueChange = { onEvent(EmployeeOnboardingCreateEvent.DeferredReasonChanged(it)) },
            ...
            DitenButton(
                ...
                onClick = { onEvent(EmployeeOnboardingCreateEvent.Submit) },

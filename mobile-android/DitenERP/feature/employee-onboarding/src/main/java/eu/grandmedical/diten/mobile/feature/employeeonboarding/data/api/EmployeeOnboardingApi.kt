package eu.grandmedical.diten.mobile.feature.employeeonboarding.data.api

import eu.grandmedical.diten.mobile.core.network.NetworkEnvelope
import eu.grandmedical.diten.mobile.core.network.NetworkHeaders
import eu.grandmedical.diten.mobile.feature.employeeonboarding.data.dto.EmployeeOnboardingCreateRequestDto
import eu.grandmedical.diten.mobile.feature.employeeonboarding.data.dto.EmployeeOnboardingReadinessDto
import eu.grandmedical.diten.mobile.feature.employeeonboarding.data.dto.EmployeeOnboardingReadinessListItemDto
import retrofit2.Response
import retrofit2.http.Body
import retrofit2.http.DELETE
import retrofit2.http.GET
import retrofit2.http.Headers
import retrofit2.http.POST
import retrofit2.http.Path

/**
 * Retrofit surface for the MEASURED employee-onboarding gateway endpoints. Every
 * call is authenticated: the `@Headers(AUTHENTICATED)` marker opts the request
 * into the network layer's Bearer + X-Tenant-Id + X-Legal-Entity-Id stamping
 * ([HeaderInterceptor][eu.grandmedical.diten.mobile.core.network.interceptor.HeaderInterceptor]),
 * and the marker is stripped before the request leaves the process.
 *
 * Built from the injected `Retrofit` (see the feature DI module).
 */
interface EmployeeOnboardingApi {

    @Headers(NetworkHeaders.AUTHENTICATED)
    @GET("api/employee-onboarding")
    suspend fun list(): Response<NetworkEnvelope<List<EmployeeOnboardingReadinessListItemDto>>>

    @Headers(NetworkHeaders.AUTHENTICATED)
    @GET("api/employee-onboarding/{id}")
    suspend fun get(@Path("id") id: String): Response<NetworkEnvelope<EmployeeOnboardingReadinessDto>>

    /** Returns the new record's id (Guid string) in the envelope's `data`. */
    @Headers(NetworkHeaders.AUTHENTICATED)
    @POST("api/employee-onboarding")
    suspend fun create(
        @Body request: EmployeeOnboardingCreateRequestDto,
    ): Response<NetworkEnvelope<String>>

    @Headers(NetworkHeaders.AUTHENTICATED)
    @POST("api/employee-onboarding/{id}/evaluate")
    suspend fun evaluate(@Path("id") id: String): Response<NetworkEnvelope<EmployeeOnboardingReadinessDto>>

    @Headers(NetworkHeaders.AUTHENTICATED)
    @DELETE("api/employee-onboarding/{id}")
    suspend fun delete(@Path("id") id: String): Response<NetworkEnvelope<String>>
}

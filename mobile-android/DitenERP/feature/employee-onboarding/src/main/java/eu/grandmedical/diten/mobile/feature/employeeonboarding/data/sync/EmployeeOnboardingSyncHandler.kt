...
import eu.grandmedical.diten.mobile.feature.employeeonboarding.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [EmployeeOnboardingApi];
 ...
 */
class EmployeeOnboardingSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in unsynced) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing employee-onboarding row ${row.id}")
    ...
    private suspend fun push(row: EmployeeOnboardingEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: EmployeeOnboardingEntity, serverId: String) {

...
@Composable
fun EmployeeOnboardingListRoute(
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: EmployeeOnboardingListViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is EmployeeOnboardingListEffect.ShowMessage ->
                    snackbarHostState.showSnackbar(effect.message)
            }
        }
    }

    EmployeeOnboardingListScreen(
        state = state,
        snackbarHostState = snackbarHostState,
        onRefresh = { viewModel.onEvent(EmployeeOnboardingListEvent.Refresh) },
...
@Composable
private fun EmployeeOnboardingListScreen(
    state: EmployeeOnboardingListState,
    snackbarHostState: SnackbarHostState,
    onRefresh: () -> Unit,
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            UiStateContainer(
                ...
                EmployeeOnboardingList(items = items, onOpenDetail = onOpenDetail)
            }
        }
    }
}
...
@Composable
private fun EmployeeOnboardingList(
    items: List<EmployeeOnboardingListItem>,
    onOpenDetail: (String) -> Unit,
) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(items, key = { it.id }) { item ->
            DitenListRow(
                ...
                trailing = { StatusChip(state = item.onboardingReadinessState.name) },
            )

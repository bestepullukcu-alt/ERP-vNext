 ...
 */
interface EmployeeOnboardingRepository {

    /** Observes the scoped list from the local cache, offline-first. */
    fun observeList(): Flow<List<EmployeeOnboardingListItem>>

    /** Observes a single record by [id] from the local cache (null if absent). */
    fun observe(id: String): Flow<EmployeeOnboardingReadiness?>
     ...
     */
    suspend fun create(new: NewEmployeeOnboarding): UiResult<String>

    /** Asks the backend to (re)evaluate [id], then reconciles the result locally. */
    suspend fun evaluate(id: String): UiResult<EmployeeOnboardingReadiness>

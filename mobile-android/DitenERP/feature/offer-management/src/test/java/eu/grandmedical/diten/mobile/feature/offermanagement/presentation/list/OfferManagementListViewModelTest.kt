...
class OfferManagementListViewModelTest {
    ...
    private fun item(id: String) = OfferManagementListItem(
        id = id,
        code = id,
        displayName = "Name $id",
        offerReadinessState = ReadinessState.Ready,
        sourceContractVersion = "v1",
    )

    @Test
    fun startsLoading_thenRendersContentFromTheCache() = runTest(mainDispatcher) {
        val vm = OfferManagementListViewModel(repository)
    ...
    fun refresh_triggersRepositoryRefresh() = runTest(mainDispatcher) {
        ...
        vm.onEvent(OfferManagementListEvent.Refresh)

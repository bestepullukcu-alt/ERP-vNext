...
import eu.grandmedical.diten.mobile.feature.offermanagement.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [OfferManagementApi];
 ...
 */
class OfferManagementSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in pending) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing offer-management row ${row.id}")
    ...
    private suspend fun push(row: OfferManagementEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: OfferManagementEntity, serverId: String) {

...
/**
 * Client-side defaults for a new offer-management record.
 *
 * The auto-generated [defaultCode] must be MARKER-SAFE: the backend rejects
 * certain forbidden marker substrings in text fields (e.g. salary, payroll,
 * bank, tax, secret), so the code is built only from a fixed safe prefix, the
 * date, and Crockford base32 characters (digits + uppercase letters, excluding
 * I/L/O/U to avoid ambiguity). It stays short and user-editable.
 */
object CodeDefaults {

    private const val PREFIX = "OM-"
     ...
     * `OM-260911-K7QM`. Two calls differ (random suffix).
     */
    fun defaultCode(

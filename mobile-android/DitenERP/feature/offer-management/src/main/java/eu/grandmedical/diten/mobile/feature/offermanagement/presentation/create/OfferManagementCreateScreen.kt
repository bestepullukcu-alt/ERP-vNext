...
@Composable
fun OfferManagementCreateRoute(
    onDone: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: OfferManagementCreateViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                OfferManagementCreateEffect.NavigateBack -> onDone()
            }
        }
    }

    OfferManagementCreateScreen(
        state = state,
...
@Composable
private fun OfferManagementCreateScreen(
    state: OfferManagementCreateState,
    onEvent: (OfferManagementCreateEvent) -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
) {
    DitenScaffold(
        title = "Yeni Teklif Kaydı",
        modifier = modifier,
        navigationIcon = {
        ...
        Column(
            ...
            DitenTextField(
                value = state.code,
                onValueChange = { onEvent(OfferManagementCreateEvent.CodeChanged(it)) },
                label = "Kod",
                supportingText = "Otomatik üretildi; düzenlenebilir.",
            )
            DitenTextField(
                value = state.displayName,
                onValueChange = { onEvent(OfferManagementCreateEvent.DisplayNameChanged(it)) },
                label = "Görünen ad",
            )
            DitenTextField(
                value = state.sourceContractVersion,
                onValueChange = { onEvent(OfferManagementCreateEvent.SourceContractVersionChanged(it)) },
                label = "Kaynak sözleşme sürümü",
            )

            OfferStateField.entries.forEach { field ->
                ...
                DitenDropdownField(
                    ...
                    onSelected = { name ->
                        onEvent(OfferManagementCreateEvent.StateChanged(field, ReadinessState.valueOf(name)))
                    },
                )
            }
            ...
            DitenTextField(
                ...
                onValueChange = { onEvent(OfferManagementCreateEvent.DeferredReasonChanged(it)) },
            ...
            DitenButton(
                ...
                onClick = { onEvent(OfferManagementCreateEvent.Submit) },

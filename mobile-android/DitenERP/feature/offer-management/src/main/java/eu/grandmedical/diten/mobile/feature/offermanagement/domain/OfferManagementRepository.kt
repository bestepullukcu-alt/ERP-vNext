 ...
 */
interface OfferManagementRepository {

    /** Observes the scoped list from the local cache, offline-first. */
    fun observeList(): Flow<List<OfferManagementListItem>>

    /** Observes a single record by [id] from the local cache (null if absent). */
    fun observe(id: String): Flow<OfferManagementReadiness?>
     ...
     */
    suspend fun create(new: NewOfferManagement): UiResult<String>

    /** Asks the backend to (re)evaluate [id], then reconciles the result locally. */
    suspend fun evaluate(id: String): UiResult<OfferManagementReadiness>

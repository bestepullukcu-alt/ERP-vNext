package eu.grandmedical.diten.mobile.feature.timeattendanceleave.data
...
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.FakeScopeProvider
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.FakeTimeAttendanceLeaveApi
...
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.data.mapper.toEntity
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.domain.NewTimeAttendanceLeave
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.listItemDto
import eu.grandmedical.diten.mobile.feature.timeattendanceleave.readinessDto
 ...
 * [TimeAttendanceLeaveDatabase] plus a fake [FakeTimeAttendanceLeaveApi]:
...
class TimeAttendanceLeaveRepositoryImplTest {

    private lateinit var db: TimeAttendanceLeaveDatabase
    private lateinit var dao: TimeAttendanceLeaveDao
    private lateinit var api: FakeTimeAttendanceLeaveApi
    ...
    fun setUp() {
        ...
        db = Room.inMemoryDatabaseBuilder(context, TimeAttendanceLeaveDatabase::class.java)
            .allowMainThreadQueries()
            .build()
        dao = db.timeAttendanceLeaveDao()
        api = FakeTimeAttendanceLeaveApi()
    ...
    private fun repository(activeScope: ScopeKeys? = scope) = TimeAttendanceLeaveRepositoryImpl(
        dao = dao,
        api = api,
    ...
    fun create_writesPendingRow_observableWithoutAnyServerCall() = runTest {
        ...
        val result = repo.create(NewTimeAttendanceLeave(code = "TAL-1", displayName = "Local only"))
    ...
    fun refresh_upsertsServerRowsAsSynced_inScope() = runTest {
        api.onList = { FakeTimeAttendanceLeaveApi.ok(listOf(listItemDto("s1"), listItemDto("s2"))) }
        val repo = repository()
    ...
    fun observeList_excludesOtherTenantAndLegalEntity() = runTest {
        ...
        repo.create(NewTimeAttendanceLeave(code = "TAL-mine", displayName = "Mine"))
    ...
    fun evaluate_updatesTheRow() = runTest {
        dao.upsert(readinessDto("id-1", readinessState = 0).toEntity(scope)) // Draft
        api.onEvaluate = { FakeTimeAttendanceLeaveApi.ok(readinessDto("id-1", readinessState = 1)) } // Ready
        val repo = repository()

        val result = repo.evaluate("id-1")

        assertTrue(result is UiResult.Success)
        assertEquals(1, dao.getById("id-1")?.readinessState)
    }
    ...
    fun delete_syncedRow_serverReturns404_stillRemovesLocalRow() = runTest {
        dao.upsert(readinessDto("id-1").toEntity(scope)) // SYNCED
        api.onDelete = { FakeTimeAttendanceLeaveApi.error(404) }
        val repo = repository()

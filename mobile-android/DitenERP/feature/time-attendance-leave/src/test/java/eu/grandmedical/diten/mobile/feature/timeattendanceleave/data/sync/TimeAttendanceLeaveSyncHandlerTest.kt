...
class TimeAttendanceLeaveSyncHandlerTest {
    ...
    private fun handler() = TimeAttendanceLeaveSyncHandler(dao = dao, api = api, safeApiCaller = safeApiCaller)
    ...
    fun pending_isPushed_thenMarkedSynced_withServerId() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeTimeAttendanceLeaveApi.ok("server-1") }
    ...
    fun transientServerError_retries_andLeavesRowPending() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeTimeAttendanceLeaveApi.error(503) }
    ...
    fun unauthorizedError_retries_andLeavesRowUnchanged() = runTest {
        ...
        seedPending("local-1")
        api.onCreate = { FakeTimeAttendanceLeaveApi.error(401) }
    ...
    fun permanentValidationError_fails_andMarksRowFailed_neverSynced() = runTest {
        seedPending("local-1")
        api.onCreate = { FakeTimeAttendanceLeaveApi.error(400) }

 ...
 */
interface TimeAttendanceLeaveRepository {

    /** Observes the scoped list from the local cache, offline-first. */
    fun observeList(): Flow<List<TimeAttendanceLeaveListItem>>

    /** Observes a single record by [id] from the local cache (null if absent). */
    fun observe(id: String): Flow<TimeAttendanceLeaveReadiness?>
     ...
     */
    suspend fun create(new: NewTimeAttendanceLeave): UiResult<String>

    /** Asks the backend to (re)evaluate [id], then reconciles the result locally. */
    suspend fun evaluate(id: String): UiResult<TimeAttendanceLeaveReadiness>

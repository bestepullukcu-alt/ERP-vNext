...
@Composable
fun TimeAttendanceLeaveListRoute(
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: TimeAttendanceLeaveListViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is TimeAttendanceLeaveListEffect.ShowMessage ->
                    snackbarHostState.showSnackbar(effect.message)
            }
        }
    }

    TimeAttendanceLeaveListScreen(
        state = state,
        snackbarHostState = snackbarHostState,
        onRefresh = { viewModel.onEvent(TimeAttendanceLeaveListEvent.Refresh) },
...
@Composable
private fun TimeAttendanceLeaveListScreen(
    state: TimeAttendanceLeaveListState,
    snackbarHostState: SnackbarHostState,
    onRefresh: () -> Unit,
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            UiStateContainer(
                ...
                TimeAttendanceLeaveList(items = items, onOpenDetail = onOpenDetail)
            }
        }
    }
}
...
@Composable
private fun TimeAttendanceLeaveList(
    items: List<TimeAttendanceLeaveListItem>,
    onOpenDetail: (String) -> Unit,
) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(items, key = { it.id }) { item ->
            DitenListRow(
                ...
                trailing = { StatusChip(state = item.readinessState.name) },
            )

...
@Composable
fun TimeAttendanceLeaveCreateRoute(
    onDone: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: TimeAttendanceLeaveCreateViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                TimeAttendanceLeaveCreateEffect.NavigateBack -> onDone()
            }
        }
    }

    TimeAttendanceLeaveCreateScreen(
        state = state,
...
@Composable
private fun TimeAttendanceLeaveCreateScreen(
    state: TimeAttendanceLeaveCreateState,
    onEvent: (TimeAttendanceLeaveCreateEvent) -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
) {
    DitenScaffold(
        title = "Yeni Zaman & İzin Kaydı",
        modifier = modifier,
        navigationIcon = {
        ...
        Column(
            ...
            DitenTextField(
                value = state.code,
                onValueChange = { onEvent(TimeAttendanceLeaveCreateEvent.CodeChanged(it)) },
                label = "Kod",
                supportingText = "Otomatik üretildi; düzenlenebilir.",
            )
            DitenTextField(
                value = state.displayName,
                onValueChange = { onEvent(TimeAttendanceLeaveCreateEvent.DisplayNameChanged(it)) },
                label = "Görünen ad",
            )
            DitenTextField(
                value = state.sourceContractVersion,
                onValueChange = { onEvent(TimeAttendanceLeaveCreateEvent.SourceContractVersionChanged(it)) },
                label = "Kaynak sözleşme sürümü",
            )

            TimeAttendanceLeaveStateField.entries.forEach { field ->
                ...
                DitenDropdownField(
                    ...
                    onSelected = { name ->
                        onEvent(TimeAttendanceLeaveCreateEvent.StateChanged(field, ReadinessState.valueOf(name)))
                    },
                )
            }
            ...
            DitenTextField(
                ...
                onValueChange = { onEvent(TimeAttendanceLeaveCreateEvent.DeferredReasonChanged(it)) },
            ...
            DitenButton(
                ...
                onClick = { onEvent(TimeAttendanceLeaveCreateEvent.Submit) },

...
@Composable
fun TimeAttendanceLeaveDetailRoute(
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: TimeAttendanceLeaveDetailViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is TimeAttendanceLeaveDetailEffect.ShowMessage -> snackbarHostState.showSnackbar(effect.message)
                TimeAttendanceLeaveDetailEffect.NavigateBack -> onBack()
            }
        }
    }

    DitenScaffold(
        title = "Zaman & İzin Kaydı",
        modifier = modifier,
        navigationIcon = {
            ...
            UiStateContainer(state = state.record) { record ->
                if (record == null) {
                    ...
                    DetailContent(
                        record = record,
                        isBusy = state.isBusy,
                        onEvaluate = { viewModel.onEvent(TimeAttendanceLeaveDetailEvent.Evaluate) },
                        onDelete = { viewModel.onEvent(TimeAttendanceLeaveDetailEvent.Delete) },
                    )
                }
            }
        }
...
private fun DetailContent(
    record: TimeAttendanceLeaveReadiness,
    isBusy: Boolean,
    onEvaluate: () -> Unit,
    onDelete: () -> Unit,
) {
    Column(
        ...
        Text(
            text = "${record.code} · ${record.sourceContractVersion} · v${record.readinessVersion}",
        ...
        FacetRow("Genel hazırlık durumu", record.readinessState)
        FacetRow("Zaman çizelgesi alım sınırı", record.timesheetIntakeBoundaryState)
        FacetRow("Devam senkron sınırı", record.attendanceSyncBoundaryState)
        FacetRow("İzin talep sınırı", record.leaveRequestBoundaryState)
        FacetRow("İzin bakiye sınırı", record.leaveBalanceBoundaryState)
        FacetRow("Çizelge tüketim sınırı", record.scheduleConsumptionBoundaryState)
        FacetRow("Otomatik karar sınırı", record.automatedDecisionBoundaryState)
        FacetRow("Zaman/devam kaynak bağımlılığı", record.timeAttendanceSourceDependencyState)
        FacetRow("İzin kaynak bağımlılığı", record.leaveSourceDependencyState)

...
/**
 * The HCM readiness state, mirroring the backend `TimeAttendanceLeaveReadinessState`
 * enum.
 *
 ...
 * time-attendance-leave enum ORDERS `Ready` BEFORE `Deferred` — the OPPOSITE of
 * the applicant-intake pilot, and the SAME as candidate-pipeline — so the wire
 * codes are:
 * `Draft=0, Ready=1, Deferred=2, Blocked=3, NotRequired=4, Archived=5`.
 * (Measured directly from `Domain.Enums.TimeAttendanceLeaveReadinessState`.)
 */

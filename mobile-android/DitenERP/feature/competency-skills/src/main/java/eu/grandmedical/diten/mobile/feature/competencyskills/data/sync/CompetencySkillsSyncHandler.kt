package eu.grandmedical.diten.mobile.feature.competencyskills.data.sync
...
import eu.grandmedical.diten.mobile.feature.competencyskills.data.mapper.toCreateRequest
 ...
 *  2. **push** — POST each as a create request via [CompetencySkillsApi];
 ...
 */
class CompetencySkillsSyncHandler @Inject constructor(
    ...
    override suspend fun sync(): SyncResult {
        ...
        for (row in unsynced) {
            when (val outcome = push(row)) {
                Outcome.Transient ->
                    return SyncResult.Retry("connectivity/server pushing competency-skills row ${row.id}")
    ...
    private suspend fun push(row: CompetencySkillsEntity): Outcome =
    ...
    private suspend fun reconcileAccepted(row: CompetencySkillsEntity, serverId: String) {

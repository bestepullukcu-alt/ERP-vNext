 ...
 */
interface LearningTrainingRepository {

    /** Observes the scoped list from the local cache, offline-first. */
    fun observeList(): Flow<List<LearningTrainingListItem>>

    /** Observes a single record by [id] from the local cache (null if absent). */
    fun observe(id: String): Flow<LearningTrainingReadiness?>
     ...
     */
    suspend fun create(new: NewLearningTraining): UiResult<String>

    /** Asks the backend to (re)evaluate [id], then reconciles the result locally. */
    suspend fun evaluate(id: String): UiResult<LearningTrainingReadiness>

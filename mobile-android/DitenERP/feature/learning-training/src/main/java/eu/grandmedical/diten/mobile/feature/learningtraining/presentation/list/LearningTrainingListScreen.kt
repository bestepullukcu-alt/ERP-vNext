...
@Composable
fun LearningTrainingListRoute(
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: LearningTrainingListViewModel = hiltViewModel(),
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is LearningTrainingListEffect.ShowMessage ->
                    snackbarHostState.showSnackbar(effect.message)
            }
        }
    }

    LearningTrainingListScreen(
        state = state,
        snackbarHostState = snackbarHostState,
        onRefresh = { viewModel.onEvent(LearningTrainingListEvent.Refresh) },
...
@Composable
private fun LearningTrainingListScreen(
    state: LearningTrainingListState,
    snackbarHostState: SnackbarHostState,
    onRefresh: () -> Unit,
    onOpenDetail: (String) -> Unit,
    onOpenCreate: () -> Unit,
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            UiStateContainer(
                ...
                LearningTrainingList(items = items, onOpenDetail = onOpenDetail)
            }
        }
    }
}
...
@Composable
private fun LearningTrainingList(
    items: List<LearningTrainingListItem>,
    onOpenDetail: (String) -> Unit,
) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(items, key = { it.id }) { item ->
            DitenListRow(
                ...
                trailing = { StatusChip(state = item.learningTrainingReadinessState.name) },
            )

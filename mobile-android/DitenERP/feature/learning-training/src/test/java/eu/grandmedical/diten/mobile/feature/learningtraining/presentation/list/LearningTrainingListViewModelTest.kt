...
class LearningTrainingListViewModelTest {
    ...
    private fun item(id: String) = LearningTrainingListItem(
        id = id,
        code = id,
        displayName = "Name $id",
        learningTrainingReadinessState = ReadinessState.Ready,
        sourceContractVersion = "v1",
    )

    @Test
    fun startsLoading_thenRendersContentFromTheCache() = runTest(mainDispatcher) {
        val vm = LearningTrainingListViewModel(repository)
    ...
    fun refresh_triggersRepositoryRefresh() = runTest(mainDispatcher) {
        ...
        vm.onEvent(LearningTrainingListEvent.Refresh)

...
public sealed class SensitiveDataLogEventEnricher : ILogEventEnricher
{
    public void Enrich(LogEvent logEvent, ILogEventPropertyFactory propertyFactory)
    {
        ...
        {
            var redactedValue = SensitiveDataRedactor.IsSensitivePropertyName(property.Key)
    ...
    private static StructureValue RedactStructure(StructureValue structure)
    {
        var properties = structure.Properties
            .Select(property => new LogEventProperty(
                property.Name,
                SensitiveDataRedactor.IsSensitivePropertyName(property.Name)
                    ? new ScalarValue("[REDACTED]")
                    : Redact(property.Value)));
    ...
    private static DictionaryValue RedactDictionary(DictionaryValue dictionary)
    {
        var elements = dictionary.Elements.ToDictionary(
            pair => pair.Key,
            pair => pair.Key.Value is string key && SensitiveDataRedactor.IsSensitivePropertyName(key)
                ? new ScalarValue("[REDACTED]")
                : Redact(pair.Value));

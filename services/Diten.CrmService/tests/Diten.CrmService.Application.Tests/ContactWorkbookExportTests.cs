...
public sealed class ContactWorkbookExportTests
{
    ...
    private sealed class FakeContactRepo : IContactRepository
    {
        ...
        public Task<(IReadOnlyList<DomainContact> Items, long Total)> ListAsync(Guid t, string? s, int p, int ps, CancellationToken ct) => Task.FromResult(((IReadOnlyList<DomainContact>)new List<DomainContact>(), 0L));
    ...
    private sealed class FakeAccountRepo : IAccountRepository
    {
        ...
        public Task<(IReadOnlyList<DomainAccount> Items, long Total)> ListAsync(Guid t, string? s, int p, int ps, CancellationToken ct)
            => Task.FromResult(((IReadOnlyList<DomainAccount>)Items.Where(a => a.TenantId == t && !a.IsDeleted).Take(ps).ToList(), (long)Items.Count));

...
namespace Diten.CrmService.Application.Tests.Tenancy;

/*
 * THE GUARD — TENANT CONTRADICTION IS REFUSED IN CrmService (BL-324, rule from BL-323 / DCP-004 §7.4).
 *
 * WHAT WAS WRONG. This service's tenant middleware never read the JWT. The `X-Tenant-Id` header alone named the
 * tenant, so an authenticated user of tenant A could send `X-Tenant-Id: B` and the service acted for B. CrmService

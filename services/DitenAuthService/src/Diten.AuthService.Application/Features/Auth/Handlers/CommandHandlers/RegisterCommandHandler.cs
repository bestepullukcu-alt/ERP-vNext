...
public sealed class RegisterCommandHandler : IRequestHandler<RegisterCommand, AuthResponse>
{
    ...
    public async Task<AuthResponse> Handle(RegisterCommand request, CancellationToken ct)
    {
        ...
        var roles = role != null ? new[] { role.Name } : Array.Empty<string>();
        var accessToken = _tokenService.GenerateAccessToken(created, roles, Array.Empty<string>());
        var refreshTokenStr = _tokenService.GenerateRefreshToken();

        var refreshToken = new RefreshToken(created.Id, refreshTokenStr, DateTime.UtcNow.AddDays(7), "0.0.0.0", _tenantContext.TenantId);
        ...
        return new AuthResponse(
            ...
            new UserDto(created.Id, created.Email, created.FirstName, created.LastName, created.IsActive, roles)
        );
    }
}

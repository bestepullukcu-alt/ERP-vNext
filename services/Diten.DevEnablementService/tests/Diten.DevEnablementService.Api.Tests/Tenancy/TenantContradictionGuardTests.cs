...
public sealed class TenantContradictionGuardTests
{
    ...
    public async Task Contradiction_is_refused_with_the_tenant_mismatch_body()
    {
        ...
        // CrmService, HcmService, MdmService). Only the media type is affected; the status and the body shape are
        // correct. Pinned AS IT ACTUALLY IS so a later fix fails here and gets noticed, instead of the source and
        // the wire disagreeing with nobody measuring which one is true.

...
public sealed class HiringRiskIndicatorsTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetHiringRiskIndicatorsReadinessListHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
            .Handle(new GetHiringRiskIndicatorsReadinessListQuery(), CancellationToken.None);
        var get = await new GetHiringRiskIndicatorsReadinessByIdHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteHiringRiskIndicatorsReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryHiringRiskIndicatorsReadinessMetadataRepository(metadata);
        var handler = new EvaluateHiringRiskIndicatorsReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryHiringRiskIndicatorsReadinessMetadataRepository(metadata);
        var response = await new GetHiringRiskIndicatorsAuditMetadataHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    private static async Task<IReadOnlyList<HiringRiskIndicatorsReadinessListItemDto>> ListWith(
        IHiringRiskIndicatorsReadinessMetadataRepository repository,
        Guid tenantId,
        IReadOnlyCollection<Guid> effective)
    {
        var handler = new GetHiringRiskIndicatorsReadinessListHandler(
            repository,
            new FixedTenantContext(tenantId),
            new FixedLegalEntityContext(effective.First(), effective));
        var response = await handler.Handle(new GetHiringRiskIndicatorsReadinessListQuery(), CancellationToken.None);
    ...
    private sealed class InMemoryHiringRiskIndicatorsReadinessMetadataRepository : IHiringRiskIndicatorsReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<HiringRiskIndicatorsReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct) =>
        ...
        public Task<HiringRiskIndicatorsReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct) =>
    ...
    public async Task LegalEntity_create_stamps_the_selected_legal_entity()
    {
        ...
        var created = await handler.Handle(new CreateHiringRiskIndicatorsReadinessCommand(ValidRequest()), CancellationToken.None);
    ...
    public async Task LegalEntity_list_rolls_up_holding_and_isolates_siblings()
    {
        ...
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Medikal, new[] { Medikal }))
            .Handle(new CreateHiringRiskIndicatorsReadinessCommand(ValidRequest(code: "MED-01")), CancellationToken.None);
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Teknoloji, new[] { Teknoloji }))
            .Handle(new CreateHiringRiskIndicatorsReadinessCommand(ValidRequest(code: "TEK-01")), CancellationToken.None);
    ...
    public async Task LegalEntity_same_code_is_unique_per_legal_entity_not_per_tenant()
    {
        ...
        var first = await CreateHandler(repository, tenantId, medikal)
            .Handle(new CreateHiringRiskIndicatorsReadinessCommand(ValidRequest(code: "SHARED-01")), CancellationToken.None);

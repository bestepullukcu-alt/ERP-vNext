...
public sealed class VerifiedCertificationRegistryTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetVerifiedCertificationRegistryReadinessListHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
            .Handle(new GetVerifiedCertificationRegistryReadinessListQuery(), CancellationToken.None);
        var get = await new GetVerifiedCertificationRegistryReadinessByIdHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteVerifiedCertificationRegistryReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryVerifiedCertificationRegistryReadinessMetadataRepository(metadata);
        var handler = new EvaluateVerifiedCertificationRegistryReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryVerifiedCertificationRegistryReadinessMetadataRepository(metadata);
        var response = await new GetVerifiedCertificationRegistryAuditMetadataHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    private static async Task<IReadOnlyList<VerifiedCertificationRegistryReadinessListItemDto>> ListWith(
        IVerifiedCertificationRegistryReadinessMetadataRepository repository,
        Guid tenantId,
        IReadOnlyCollection<Guid> effective)
    {
        var handler = new GetVerifiedCertificationRegistryReadinessListHandler(
            repository,
            new FixedTenantContext(tenantId),
            new FixedLegalEntityContext(effective.First(), effective));
        var response = await handler.Handle(new GetVerifiedCertificationRegistryReadinessListQuery(), CancellationToken.None);
    ...
    private sealed class InMemoryVerifiedCertificationRegistryReadinessMetadataRepository : IVerifiedCertificationRegistryReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<VerifiedCertificationRegistryReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct) =>
        ...
        public Task<VerifiedCertificationRegistryReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct) =>
    ...
    public async Task LegalEntity_create_stamps_the_selected_legal_entity()
    {
        ...
        var created = await handler.Handle(new CreateVerifiedCertificationRegistryReadinessCommand(ValidRequest()), CancellationToken.None);
    ...
    public async Task LegalEntity_list_rolls_up_holding_and_isolates_siblings()
    {
        ...
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Medikal, new[] { Medikal }))
            .Handle(new CreateVerifiedCertificationRegistryReadinessCommand(ValidRequest(code: "MED-01")), CancellationToken.None);
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Teknoloji, new[] { Teknoloji }))
            .Handle(new CreateVerifiedCertificationRegistryReadinessCommand(ValidRequest(code: "TEK-01")), CancellationToken.None);
    ...
    public async Task LegalEntity_same_code_is_unique_per_legal_entity_not_per_tenant()
    {
        ...
        var first = await CreateHandler(repository, tenantId, medikal)
            .Handle(new CreateVerifiedCertificationRegistryReadinessCommand(ValidRequest(code: "SHARED-01")), CancellationToken.None);

...
public sealed class MentorshipRecommendationNetworkTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetMentorshipRecommendationNetworkReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetMentorshipRecommendationNetworkReadinessListQuery(), CancellationToken.None);
        var get = await new GetMentorshipRecommendationNetworkReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteMentorshipRecommendationNetworkReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryMentorshipRecommendationNetworkReadinessMetadataRepository(metadata);
        var handler = new EvaluateMentorshipRecommendationNetworkReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryMentorshipRecommendationNetworkReadinessMetadataRepository(metadata);
        var response = await new GetMentorshipRecommendationNetworkAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryMentorshipRecommendationNetworkReadinessMetadataRepository : IMentorshipRecommendationNetworkReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<MentorshipRecommendationNetworkReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<MentorshipRecommendationNetworkReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

...
public sealed class SectorTalentTrendsTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetSectorTalentTrendsReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetSectorTalentTrendsReadinessListQuery(), CancellationToken.None);
        var get = await new GetSectorTalentTrendsReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteSectorTalentTrendsReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemorySectorTalentTrendsReadinessMetadataRepository(metadata);
        var handler = new EvaluateSectorTalentTrendsReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemorySectorTalentTrendsReadinessMetadataRepository(metadata);
        var response = await new GetSectorTalentTrendsAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemorySectorTalentTrendsReadinessMetadataRepository : ISectorTalentTrendsReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<SectorTalentTrendsReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<SectorTalentTrendsReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

...
public sealed class RehireRecommendationTests
{
    ...
    public async Task Create_list_and_get_preserve_metadata_contract()
    {
        ...
        var list = await new GetRehireRecommendationReadinessListHandler(repository, new FixedTenantContext(TenantA))
            .Handle(new(), CancellationToken.None);
        var detail = await new GetRehireRecommendationReadinessByIdHandler(repository, new FixedTenantContext(TenantA))
    ...
    public async Task Get_by_id_is_tenant_scoped_and_returns_404_for_cross_tenant()
    {
        ...
        var query = new GetRehireRecommendationReadinessByIdHandler(repository, new FixedTenantContext(TenantB));
    ...
    public async Task Archive_sets_soft_delete_deleted_at_and_hides_record()
    {
        ...
        var archive = new ArchiveRehireRecommendationReadinessHandler(repository, new FixedTenantContext(TenantA));
    ...
    public async Task Readiness_evaluation_succeeds_with_approved_metadata_preconditions()
    {
        ...
        var evaluate = new EvaluateRehireRecommendationReadinessHandler(repository, new FixedTenantContext(TenantA));
    ...
    private sealed class MemoryRehireRecommendationRepository : ITepRehireRecommendationReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<TepRehireRecommendationReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<TepRehireRecommendationReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

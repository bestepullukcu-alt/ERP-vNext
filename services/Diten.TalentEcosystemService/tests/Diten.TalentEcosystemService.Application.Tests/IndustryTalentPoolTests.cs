...
public sealed class IndustryTalentPoolTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetIndustryTalentPoolReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetIndustryTalentPoolReadinessListQuery(), CancellationToken.None);
        var get = await new GetIndustryTalentPoolReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteIndustryTalentPoolReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryIndustryTalentPoolReadinessMetadataRepository(metadata);
        var handler = new EvaluateIndustryTalentPoolReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryIndustryTalentPoolReadinessMetadataRepository(metadata);
        var response = await new GetIndustryTalentPoolAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryIndustryTalentPoolReadinessMetadataRepository : IIndustryTalentPoolReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<IndustryTalentPoolReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<IndustryTalentPoolReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

...
public sealed class TalentDevelopmentNetworkTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetTalentDevelopmentNetworkReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetTalentDevelopmentNetworkReadinessListQuery(), CancellationToken.None);
        var get = await new GetTalentDevelopmentNetworkReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteTalentDevelopmentNetworkReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryTalentDevelopmentNetworkReadinessMetadataRepository(metadata);
        var handler = new EvaluateTalentDevelopmentNetworkReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryTalentDevelopmentNetworkReadinessMetadataRepository(metadata);
        var response = await new GetTalentDevelopmentNetworkAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryTalentDevelopmentNetworkReadinessMetadataRepository : ITalentDevelopmentNetworkReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<TalentDevelopmentNetworkReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<TalentDevelopmentNetworkReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

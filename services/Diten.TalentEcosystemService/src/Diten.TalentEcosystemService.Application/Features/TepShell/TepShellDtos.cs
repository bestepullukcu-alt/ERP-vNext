using Diten.TalentEcosystemService.Domain.Enums;

namespace Diten.TalentEcosystemService.Application.Features.TepShell;

public sealed record TepShellMetadataDto(
    Guid Id,
    string Code,
    string DisplayName,
    TepShellState ShellState,
    TepShellDependencyStatus HcmFoundationState,
    TepShellDependencyStatus PrivacyLegalState,
    TepShellDependencyStatus ConsentBoundaryState,
    TepShellDependencyStatus VisibilityBoundaryState,
    string SourceContractVersion,
    IReadOnlyList<TepShellDependencyStateDto> DependencyStates,
    DateTimeOffset? LastEvaluatedAt,
    int ShellVersion);

public sealed record TepShellMetadataListItemDto(
    Guid Id,
    string Code,
    string DisplayName,
    TepShellState ShellState,
    TepShellDependencyStatus HcmFoundationState,
    TepShellDependencyStatus PrivacyLegalState,
    TepShellDependencyStatus ConsentBoundaryState,
    TepShellDependencyStatus VisibilityBoundaryState,
    DateTimeOffset? LastEvaluatedAt,
    int ShellVersion);

public sealed record TepShellDependencyStateDto(
    string DependencyKey,
    TepShellDependencyStatus State,
    string? Reason);

public sealed record TepShellHealthDto(string RuntimeOwnerKey, string Status);

...
public sealed class MongoIndustryTalentPoolReadinessMetadataRepository : IIndustryTalentPoolReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<IndustryTalentPoolReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<IndustryTalentPoolReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

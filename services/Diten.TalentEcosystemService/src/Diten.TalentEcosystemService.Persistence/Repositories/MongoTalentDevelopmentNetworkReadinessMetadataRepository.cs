...
public sealed class MongoTalentDevelopmentNetworkReadinessMetadataRepository : ITalentDevelopmentNetworkReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TalentDevelopmentNetworkReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TalentDevelopmentNetworkReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TalentDevelopmentNetworkReadinessMetadata>.Filter.And(
            ...
            Builders<TalentDevelopmentNetworkReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TalentDevelopmentNetworkReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TalentDevelopmentNetworkReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TalentDevelopmentNetworkReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

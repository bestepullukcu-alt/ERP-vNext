...
public sealed class MongoTepCandidateProfileMetadataRepository : ITepCandidateProfileMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TepCandidateProfileMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TepCandidateProfileMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TepCandidateProfileMetadata>.Filter.And(
            ...
            Builders<TepCandidateProfileMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TepCandidateProfileMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TepCandidateProfileMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TepCandidateProfileMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

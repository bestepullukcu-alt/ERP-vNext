...
public sealed class MongoWorkforceAnalyticsReadinessMetadataRepository : IWorkforceAnalyticsReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<WorkforceAnalyticsReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<WorkforceAnalyticsReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<WorkforceAnalyticsReadinessMetadata>.Filter.And(
            ...
            Builders<WorkforceAnalyticsReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<WorkforceAnalyticsReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<WorkforceAnalyticsReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<WorkforceAnalyticsReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

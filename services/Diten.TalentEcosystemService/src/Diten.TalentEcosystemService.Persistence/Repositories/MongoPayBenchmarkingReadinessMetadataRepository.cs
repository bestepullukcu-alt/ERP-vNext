...
public sealed class MongoPayBenchmarkingReadinessMetadataRepository : IPayBenchmarkingReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<PayBenchmarkingReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<PayBenchmarkingReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<PayBenchmarkingReadinessMetadata>.Filter.And(
            ...
            Builders<PayBenchmarkingReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<PayBenchmarkingReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<PayBenchmarkingReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<PayBenchmarkingReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

...
public sealed class MongoIndustrySuccessionPoolReadinessMetadataRepository : IIndustrySuccessionPoolReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<IndustrySuccessionPoolReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<IndustrySuccessionPoolReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<IndustrySuccessionPoolReadinessMetadata>.Filter.And(
            ...
            Builders<IndustrySuccessionPoolReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<IndustrySuccessionPoolReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<IndustrySuccessionPoolReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<IndustrySuccessionPoolReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

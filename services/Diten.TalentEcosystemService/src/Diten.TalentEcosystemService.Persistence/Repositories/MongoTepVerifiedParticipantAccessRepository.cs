...
public sealed class MongoTepVerifiedParticipantAccessRepository : ITepVerifiedParticipantAccessRepository
{
    ...
    public async Task<IReadOnlyList<TepVerifiedParticipantAccess>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TepVerifiedParticipantAccess?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TepVerifiedParticipantAccess>.Filter.And(
            ...
            Builders<TepVerifiedParticipantAccess>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TepVerifiedParticipantAccess> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TepVerifiedParticipantAccess>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TepVerifiedParticipantAccess>.Filter.In(x => x.LegalEntityId, legalEntityIds));

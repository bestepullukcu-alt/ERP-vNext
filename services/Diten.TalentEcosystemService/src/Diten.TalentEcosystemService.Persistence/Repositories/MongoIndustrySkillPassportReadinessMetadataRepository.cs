...
public sealed class MongoIndustrySkillPassportReadinessMetadataRepository : IIndustrySkillPassportReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<IndustrySkillPassportReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<IndustrySkillPassportReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

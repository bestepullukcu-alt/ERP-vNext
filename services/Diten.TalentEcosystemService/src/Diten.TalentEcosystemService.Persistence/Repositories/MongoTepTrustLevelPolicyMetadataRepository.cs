...
public sealed class MongoTepTrustLevelPolicyMetadataRepository : ITepTrustLevelPolicyMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TepTrustLevelPolicyMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<TepTrustLevelPolicyMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

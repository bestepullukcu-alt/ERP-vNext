...
public sealed class MongoTepConsentVisibilityPolicyRepository : ITepConsentVisibilityPolicyRepository
{
    ...
    public async Task<IReadOnlyList<TepConsentVisibilityPolicy>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TepConsentVisibilityPolicy?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TepConsentVisibilityPolicy>.Filter.And(
            ...
            Builders<TepConsentVisibilityPolicy>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TepConsentVisibilityPolicy> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TepConsentVisibilityPolicy>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TepConsentVisibilityPolicy>.Filter.In(x => x.LegalEntityId, legalEntityIds));

...
public sealed class MongoCandidateCareerPassportReadinessMetadataRepository : ICandidateCareerPassportReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<CandidateCareerPassportReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<CandidateCareerPassportReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<CandidateCareerPassportReadinessMetadata>.Filter.And(
            ...
            Builders<CandidateCareerPassportReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<CandidateCareerPassportReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<CandidateCareerPassportReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<CandidateCareerPassportReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

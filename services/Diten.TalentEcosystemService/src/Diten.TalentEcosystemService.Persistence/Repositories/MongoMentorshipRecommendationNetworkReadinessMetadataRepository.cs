...
public sealed class MongoMentorshipRecommendationNetworkReadinessMetadataRepository : IMentorshipRecommendationNetworkReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<MentorshipRecommendationNetworkReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<MentorshipRecommendationNetworkReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<MentorshipRecommendationNetworkReadinessMetadata>.Filter.And(
            ...
            Builders<MentorshipRecommendationNetworkReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<MentorshipRecommendationNetworkReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<MentorshipRecommendationNetworkReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<MentorshipRecommendationNetworkReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

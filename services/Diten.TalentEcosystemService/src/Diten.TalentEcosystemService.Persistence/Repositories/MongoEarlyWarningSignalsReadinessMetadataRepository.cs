...
public sealed class MongoEarlyWarningSignalsReadinessMetadataRepository : IEarlyWarningSignalsReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<EarlyWarningSignalsReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<EarlyWarningSignalsReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<EarlyWarningSignalsReadinessMetadata>.Filter.And(
            ...
            Builders<EarlyWarningSignalsReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<EarlyWarningSignalsReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<EarlyWarningSignalsReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<EarlyWarningSignalsReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

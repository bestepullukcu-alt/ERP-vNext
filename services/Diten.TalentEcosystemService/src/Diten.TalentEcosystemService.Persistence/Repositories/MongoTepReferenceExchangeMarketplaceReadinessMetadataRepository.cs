    ...
    : ITepReferenceExchangeMarketplaceReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TepReferenceExchangeMarketplaceReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TepReferenceExchangeMarketplaceReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TepReferenceExchangeMarketplaceReadinessMetadata>.Filter.And(
            ...
            Builders<TepReferenceExchangeMarketplaceReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TepReferenceExchangeMarketplaceReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TepReferenceExchangeMarketplaceReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TepReferenceExchangeMarketplaceReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

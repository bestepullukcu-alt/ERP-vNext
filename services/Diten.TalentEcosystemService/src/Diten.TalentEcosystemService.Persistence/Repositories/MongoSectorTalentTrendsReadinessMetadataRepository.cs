...
public sealed class MongoSectorTalentTrendsReadinessMetadataRepository : ISectorTalentTrendsReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<SectorTalentTrendsReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<SectorTalentTrendsReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

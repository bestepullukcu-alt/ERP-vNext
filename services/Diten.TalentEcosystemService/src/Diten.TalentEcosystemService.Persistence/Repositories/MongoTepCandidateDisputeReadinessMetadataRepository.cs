    ...
    : ITepCandidateDisputeReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TepCandidateDisputeReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<TepCandidateDisputeReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<TepCandidateDisputeReadinessMetadata>.Filter.And(
            ...
            Builders<TepCandidateDisputeReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<TepCandidateDisputeReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<TepCandidateDisputeReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<TepCandidateDisputeReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

...
/// <summary>MOD-0357 S11 (KS4) — the recurring sweep must process EVERY active tenant in its own scope, and one
/// tenant's failure must not abort the rest. Mirrors WorkflowEscalationSweepJobTests' own shape exactly (the
/// same precedent this WP's reading list points at).</summary>
public sealed class MeetingSeriesSweepJobTests
{
    ...
    public async Task Sweeps_every_active_tenant_in_its_own_scope()
    {
        ...
        var job = new MeetingSeriesSweepJob(registry, ctx, mediator, NullLogger<MeetingSeriesSweepJob>.Instance);

        await job.HandleAsync(new MeetingSeriesSweepJobArgs(100), Context(), CancellationToken.None);
    ...
    private sealed class RecordingMediator(ITenantContext ctx) : IMediator
    {
        ...
        {
            if (request is GenerateDueMeetingSeriesCommand)
            {
                TenantsAtSend.Add(ctx.TenantId);
                if (FailTenant == ctx.TenantId)
                {
                    throw new InvalidOperationException("tenant series sweep failed");
                }

                object resp = Response<GenerateDueMeetingSeriesResponse>.Success(
                    new GenerateDueMeetingSeriesResponse(0, 0, 0, 0), 200, "corr");

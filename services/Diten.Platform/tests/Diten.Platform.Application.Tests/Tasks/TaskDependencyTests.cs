...
public sealed class TaskDependencyTests
{
    ...
    private static TaskWorkItemProvider Provider(FakeTaskItemRepository tasks, FakeTaskDependencyRepository edges)
        => new(
            ...
            edges, new FakeTaskCommentRepository(), new FakeTaskTransitionRepository(), new FakeTaskPersonalOverlayRepository(), new FakeTaskWatcherRepository(), TaskActors.PermitAll(), new FakePositionRepository(), new FakeOrganizationUnitRepository(), SlaForTests.Real(), new FakeTaskFieldDefinitionRepository(), new FakeTaskTypeRepository());

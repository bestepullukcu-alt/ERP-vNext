...
/// <summary>
/// Pushes this service's module manifest(s) to the Platform module-catalog at startup (self-registration). BEST-EFFORT
/// with retry: runs after the app is up, retries a few times with increasing backoff to swallow the "Platform not ready
/// yet" startup race, and NEVER crashes startup. Safe to repeat every restart (Platform reconcile is idempotent). Sends
/// X-Internal-Api-Key (S2S, direct to Platform — /api/internal is not gateway-exposed). Mirror of DevEnablement.
/// </summary>
        ...
        foreach (var provider in _manifestProviders)
        {
            ...
            try
            {
                var stopped = await ModuleRegistrationRetry.RunAsync(
                    ...
                    backoff: attemptNumber => TimeSpan.FromSeconds(Math.Pow(2, attemptNumber)),
                    delay: Task.Delay,
                    stoppingToken);
                ...
                if (!stopped)
                {
                    _logger.LogWarning(
                        ...
                        MaxAttempts, manifest.ModuleCode);
                }
            }
    ...
    private async Task<bool> TryRegisterAsync(ModuleManifestDocument manifest, int attemptNumber, CancellationToken ct)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                _logger.LogInformation(
                    ...
                    manifest.ModuleCode, attemptNumber);
                return true;
            }
            ...
            if ((int)response.StatusCode >= 500)
            {
                _logger.LogWarning(
                    ...
                    attemptNumber, (int)response.StatusCode, manifest.ModuleCode);
                return false;
            }

            _logger.LogWarning(
                "Module self-registration got non-retryable {StatusCode}. ModuleCode={ModuleCode}",
                (int)response.StatusCode, manifest.ModuleCode);
            return true;
        }
        ...
        catch (HttpRequestException ex)
        {
            _logger.LogWarning(
                ...
                attemptNumber, manifest.ModuleCode);
            return false;
        }
    }
}

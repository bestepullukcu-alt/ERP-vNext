...
public sealed class HttpLegalEntityContext : ILegalEntityContext
{
    ...
    private IReadOnlyCollection<Guid> ReadAssignedLegalEntityIds()
    {
        ...
        // The `legal_entities` JWT claim foundation is NOT ported to main (deferred; see
        // the gap record). Until Ali wires that claim, the actable legal-entity set is
        // intentionally EMPTY -> every scoped read is fail-closed (returns nothing). Tenant
        // isolation is unaffected (repositories always filter by TenantId). Restore the
        // original claim-read body when the token integration lands.

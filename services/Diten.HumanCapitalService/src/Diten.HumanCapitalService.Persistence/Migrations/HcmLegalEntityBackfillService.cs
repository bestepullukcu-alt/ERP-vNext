...
/// <summary>
/// One-shot startup migration for the legal-entity scoping rollout across every scoped HCM collection.
...
/// the repository constants, so new scoped repositories are covered automatically.
/// </summary>
public sealed class HcmLegalEntityBackfillService : IHostedService
{
    ...
    private readonly ILogger<HcmLegalEntityBackfillService> _logger;

    public HcmLegalEntityBackfillService(
        IMongoClient mongoClient,
        MongoSettings mongoSettings,
        ILogger<HcmLegalEntityBackfillService> logger)
    {
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (totalBackfilled > 0)
            {
                _logger.LogInformation(
                    "HCM legal-entity backfill stamped {Count} record(s) to the tenant root holding {LegalEntityId}.",
                    totalBackfilled,
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            _logger.LogError(ex, "HCM legal-entity backfill migration failed; continuing startup.");
        }
    }
    ...
    {
        var assembly = typeof(HcmLegalEntityBackfillService).Assembly;

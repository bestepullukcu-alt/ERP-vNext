...
public sealed class MongoHrDocumentationReadinessMetadataRepository : IHrDocumentationReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<HrDocumentationReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<HrDocumentationReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<HrDocumentationReadinessMetadata>.Filter.And(
            ...
            Builders<HrDocumentationReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<HrDocumentationReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<HrDocumentationReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<HrDocumentationReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

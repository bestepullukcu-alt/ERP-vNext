...
public sealed class MongoHeadcountBudgetReadinessMetadataRepository : IHeadcountBudgetReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<HeadcountBudgetReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<HeadcountBudgetReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<HeadcountBudgetReadinessMetadata>.Filter.And(
            ...
            Builders<HeadcountBudgetReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<HeadcountBudgetReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<HeadcountBudgetReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<HeadcountBudgetReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

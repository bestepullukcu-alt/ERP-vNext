...
public sealed class MongoEmployeeProjectionRepository : IEmployeeProjectionRepository
{
    ...
    public async Task<IReadOnlyList<EmployeeProfileProjection>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<EmployeeProfileProjection?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

...
public sealed class MongoHrCaseManagementReadinessMetadataRepository : IHrCaseManagementReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<HrCaseManagementReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<HrCaseManagementReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<HrCaseManagementReadinessMetadata>.Filter.And(
            ...
            Builders<HrCaseManagementReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<HrCaseManagementReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<HrCaseManagementReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<HrCaseManagementReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

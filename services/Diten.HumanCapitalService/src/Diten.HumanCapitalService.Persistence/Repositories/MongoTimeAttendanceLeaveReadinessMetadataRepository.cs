...
public sealed class MongoTimeAttendanceLeaveReadinessMetadataRepository : ITimeAttendanceLeaveReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<TimeAttendanceLeaveReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<TimeAttendanceLeaveReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

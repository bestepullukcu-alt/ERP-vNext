...
public sealed class MongoHrKpiAnalyticsReadinessMetadataRepository : IHrKpiAnalyticsReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<HrKpiAnalyticsReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<HrKpiAnalyticsReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<HrKpiAnalyticsReadinessMetadata>.Filter.And(
            ...
            Builders<HrKpiAnalyticsReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<HrKpiAnalyticsReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<HrKpiAnalyticsReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<HrKpiAnalyticsReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

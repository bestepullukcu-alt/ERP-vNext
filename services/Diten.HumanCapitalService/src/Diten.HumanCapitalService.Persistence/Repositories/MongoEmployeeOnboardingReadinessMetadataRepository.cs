...
public sealed class MongoEmployeeOnboardingReadinessMetadataRepository : IEmployeeOnboardingReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<EmployeeOnboardingReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<EmployeeOnboardingReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

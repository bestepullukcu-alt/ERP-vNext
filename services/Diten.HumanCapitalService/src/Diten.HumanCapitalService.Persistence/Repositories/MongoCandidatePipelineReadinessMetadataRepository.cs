...
public sealed class MongoCandidatePipelineReadinessMetadataRepository : ICandidatePipelineReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<CandidatePipelineReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<CandidatePipelineReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

...
public sealed class MongoDevelopmentPlanReadinessMetadataRepository : IDevelopmentPlanReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<DevelopmentPlanReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<DevelopmentPlanReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

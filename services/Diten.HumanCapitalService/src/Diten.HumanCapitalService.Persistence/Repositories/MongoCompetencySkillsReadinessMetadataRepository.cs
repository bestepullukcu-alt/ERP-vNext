...
public sealed class MongoCompetencySkillsReadinessMetadataRepository : ICompetencySkillsReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<CompetencySkillsReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct)
    {
    ...
    public async Task<CompetencySkillsReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);
    ...
    public async Task<bool> ExistsActiveCodeAsync(Guid tenantId, Guid legalEntityId, string code, Guid? excludingId, CancellationToken ct)
    {
        ...
        var filter = Builders<CompetencySkillsReadinessMetadata>.Filter.And(
            ...
            Builders<CompetencySkillsReadinessMetadata>.Filter.Eq(x => x.LegalEntityId, legalEntityId),
    ...
    private static FilterDefinition<CompetencySkillsReadinessMetadata> ActiveScopeFilter(
        Guid tenantId,
        IReadOnlyCollection<Guid> legalEntityIds) =>
        Builders<CompetencySkillsReadinessMetadata>.Filter.And(
            ActiveTenantFilter(tenantId),
            Builders<CompetencySkillsReadinessMetadata>.Filter.In(x => x.LegalEntityId, legalEntityIds));

...
public sealed class MongoSelfServiceReadinessMetadataRepository : ISelfServiceReadinessMetadataRepository
{
    ...
    public async Task<IReadOnlyList<SelfServiceReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct)
    {
    ...
    public async Task<SelfServiceReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct)
    {
        await EnsureIndexesAsync(ct);

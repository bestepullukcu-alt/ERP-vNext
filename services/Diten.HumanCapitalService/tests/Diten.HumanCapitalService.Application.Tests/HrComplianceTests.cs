...
public sealed class HrComplianceTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetHrComplianceReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetHrComplianceReadinessListQuery(), CancellationToken.None);
        var get = await new GetHrComplianceReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteHrComplianceReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryHrComplianceReadinessMetadataRepository(metadata);
        var handler = new EvaluateHrComplianceReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryHrComplianceReadinessMetadataRepository(metadata);
        var response = await new GetHrComplianceAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryHrComplianceReadinessMetadataRepository : IHrComplianceReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<HrComplianceReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<HrComplianceReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

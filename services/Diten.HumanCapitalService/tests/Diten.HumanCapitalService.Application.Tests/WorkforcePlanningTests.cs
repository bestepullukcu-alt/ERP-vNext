...
public sealed class WorkforcePlanningTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetWorkforcePlanningReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetWorkforcePlanningReadinessListQuery(), CancellationToken.None);
        var get = await new GetWorkforcePlanningReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteWorkforcePlanningReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryWorkforcePlanningReadinessMetadataRepository(metadata);
        var handler = new EvaluateWorkforcePlanningReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryWorkforcePlanningReadinessMetadataRepository(metadata);
        var response = await new GetWorkforcePlanningAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryWorkforcePlanningReadinessMetadataRepository : IWorkforcePlanningReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<WorkforcePlanningReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<WorkforcePlanningReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

...
public sealed class SelfServiceTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetSelfServiceReadinessListHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
            .Handle(new GetSelfServiceReadinessListQuery(), CancellationToken.None);
        var get = await new GetSelfServiceReadinessByIdHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteSelfServiceReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemorySelfServiceReadinessMetadataRepository(metadata);
        var handler = new EvaluateSelfServiceReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemorySelfServiceReadinessMetadataRepository(metadata);
        var response = await new GetSelfServiceAuditMetadataHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    private static async Task<IReadOnlyList<SelfServiceReadinessListItemDto>> ListWith(
        ISelfServiceReadinessMetadataRepository repository,
        Guid tenantId,
        IReadOnlyCollection<Guid> effective)
    {
        var handler = new GetSelfServiceReadinessListHandler(
            repository,
            new FixedTenantContext(tenantId),
            new FixedLegalEntityContext(effective.First(), effective));
        var response = await handler.Handle(new GetSelfServiceReadinessListQuery(), CancellationToken.None);
    ...
    private sealed class InMemorySelfServiceReadinessMetadataRepository : ISelfServiceReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<SelfServiceReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct) =>
        ...
        public Task<SelfServiceReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct) =>
    ...
    public async Task LegalEntity_create_stamps_the_selected_legal_entity()
    {
        ...
        var created = await handler.Handle(new CreateSelfServiceReadinessCommand(ValidRequest()), CancellationToken.None);
    ...
    public async Task LegalEntity_list_rolls_up_holding_and_isolates_siblings()
    {
        ...
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Medikal, new[] { Medikal }))
            .Handle(new CreateSelfServiceReadinessCommand(ValidRequest(code: "MED-01")), CancellationToken.None);
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Teknoloji, new[] { Teknoloji }))
            .Handle(new CreateSelfServiceReadinessCommand(ValidRequest(code: "TEK-01")), CancellationToken.None);
    ...
    public async Task LegalEntity_same_code_is_unique_per_legal_entity_not_per_tenant()
    {
        ...
        var first = await CreateHandler(repository, tenantId, medikal)
            .Handle(new CreateSelfServiceReadinessCommand(ValidRequest(code: "SHARED-01")), CancellationToken.None);

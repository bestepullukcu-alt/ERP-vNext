...
public sealed class CompensationBenefitsTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetCompensationBenefitsReadinessListHandler(repository, new FixedTenantContext(tenantId))
            .Handle(new GetCompensationBenefitsReadinessListQuery(), CancellationToken.None);
        var get = await new GetCompensationBenefitsReadinessByIdHandler(repository, new FixedTenantContext(tenantId))
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteCompensationBenefitsReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryCompensationBenefitsReadinessMetadataRepository(metadata);
        var handler = new EvaluateCompensationBenefitsReadinessHandler(repository, new FixedTenantContext(tenantId));
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryCompensationBenefitsReadinessMetadataRepository(metadata);
        var response = await new GetCompensationBenefitsAuditMetadataHandler(repository, new FixedTenantContext(tenantId))
    ...
    private sealed class InMemoryCompensationBenefitsReadinessMetadataRepository : ICompensationBenefitsReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<CompensationBenefitsReadinessMetadata>> ListAsync(Guid tenantId, CancellationToken ct) =>
        ...
        public Task<CompensationBenefitsReadinessMetadata?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct) =>

...
public sealed class ApplicantIntakeTests
{
    ...
    public async Task Create_list_and_get_metadata_contract()
    {
        ...
        var list = await new GetApplicantIntakeReadinessListHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
            .Handle(new GetApplicantIntakeReadinessListQuery(), CancellationToken.None);
        var get = await new GetApplicantIntakeReadinessByIdHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    public async Task Create_stamps_the_selected_legal_entity()
    {
        ...
        var created = await handler.Handle(new CreateApplicantIntakeReadinessCommand(ValidRequest()), CancellationToken.None);
    ...
    public async Task Create_without_a_permitted_legal_entity_is_forbidden()
    {
        ...
        var response = await handler.Handle(new CreateApplicantIntakeReadinessCommand(ValidRequest()), CancellationToken.None);
    ...
    public async Task List_rolls_up_holding_and_isolates_sibling_legal_entities()
    {
        ...
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Medikal, new[] { Medikal }))
            .Handle(new CreateApplicantIntakeReadinessCommand(ValidRequest(code: "MED-01")), CancellationToken.None);
        await CreateHandler(repository, tenantId, new FixedLegalEntityContext(Teknoloji, new[] { Teknoloji }))
            .Handle(new CreateApplicantIntakeReadinessCommand(ValidRequest(code: "TEK-01")), CancellationToken.None);
    ...
    public async Task Same_code_is_unique_per_legal_entity_not_per_tenant()
    {
        ...
        var first = await CreateHandler(repository, tenantId, medikal)
            .Handle(new CreateApplicantIntakeReadinessCommand(ValidRequest(code: "SHARED-01")), CancellationToken.None);
    ...
    public async Task Soft_delete_hides_record_and_sets_deleted_at()
    {
        ...
        var handler = new DeleteApplicantIntakeReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Evaluation_promotes_ready_only_when_metadata_preconditions_are_satisfied()
    {
        ...
        var repository = new InMemoryApplicantIntakeReadinessMetadataRepository(metadata);
        var handler = new EvaluateApplicantIntakeReadinessHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext());
    ...
    public async Task Audit_metadata_uses_audit_permission_surface()
    {
        ...
        var metadata = Metadata(tenantId);
        var repository = new InMemoryApplicantIntakeReadinessMetadataRepository(metadata);
        var response = await new GetApplicantIntakeAuditMetadataHandler(repository, new FixedTenantContext(tenantId), PilotLegalEntityContext())
    ...
    private static async Task<IReadOnlyList<ApplicantIntakeReadinessListItemDto>> ListWith(
        IApplicantIntakeReadinessMetadataRepository repository,
        Guid tenantId,
        IReadOnlyCollection<Guid> effective)
    {
        var handler = new GetApplicantIntakeReadinessListHandler(
            repository,
            new FixedTenantContext(tenantId),
            new FixedLegalEntityContext(effective.First(), effective));
        var response = await handler.Handle(new GetApplicantIntakeReadinessListQuery(), CancellationToken.None);
    ...
    private sealed class InMemoryApplicantIntakeReadinessMetadataRepository : IApplicantIntakeReadinessMetadataRepository
    {
        ...
        public Task<IReadOnlyList<ApplicantIntakeReadinessMetadata>> ListAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, CancellationToken ct) =>
        ...
        public Task<ApplicantIntakeReadinessMetadata?> GetByIdAsync(Guid tenantId, IReadOnlyCollection<Guid> legalEntityIds, Guid id, CancellationToken ct) =>

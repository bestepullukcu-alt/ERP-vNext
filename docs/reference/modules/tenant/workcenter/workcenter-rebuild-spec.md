...
🟡 **Faz 3 (genişletilmiş):** Kanban — WP-WCN-KANBAN-01 (2026-09-17/18) ile gerçek DURUMA güncellendi: sütun = durum (normalizedStatus) · sürükleme panoyu değil, motorun mevcut geçiş kuralını yazar — targetStatus işlem başına sunucudan gelir, kart hiçbir zaman durum alanını doğrudan yazmaz · sütun anahtarı sunucunun kablo değerini (normalizedStatus, "InProgress") değil, projeksiyonun EKRAN biçimini (item.status, "In Progress") kullanır — kanbanColumnKey ikisini köprüler · Tamamlandı ve İptal, İşlerim ve Başlattıklarım'da kartsız, ince bırakma alanı olarak durur (Havuz ve Geçmiş bugünkü gibi salt-okunur kalır; Takım kapsamı da sürüklemeye kapalıdır — bir meslektaşın işi bu okuyucunun sürükleyeceği bir şey değildir) · bir sütuna iki ya da daha fazla enabled işlemle gidiliyorsa küçük bir seçim menüsü açılır · kulvar, WIP sınırı ve elle sıralama YOK. Takvim (salt-okunur deadline kümelenmesi) · Gelen Kutusu'nda **onaylar üst bandı** · gelişmiş filtreler (öncelik/tarih/atama-modu) · **N-yönlü vekâlet** (+ birleşik "Tümü") · **havuz grup seçici** · **stale/sistem durumları** ("kayıt değişti"/"kaynak erişilemez"/"yetkin bitti") · **"+ Yeni"** (① self-task inline · ② kaynakta-oluştur launcher — WorkCenter modül formu KURMAZ).

...
/// <summary>
/// The "Active" switch must post what the user left on screen.
///
/// <para><b>The measured defect this pins.</b> Two forms carried a hand-written
...
///
/// <para>Restoring the hand-written hidden turns <see cref="A_ticked_Active_switch_binds_TRUE"/> red.</para>
/// </summary>
public sealed class ActiveSwitchBindingTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Theory]
    [InlineData(RecurrenceRuleForm)]
    [InlineData(FieldDefinitionForm)]
    public async Task A_ticked_Active_switch_binds_TRUE(string viewPath)
    {
        var posted = await PostedValuesAsync(viewPath, nameof(TaskRecurrenceRuleEditViewModel.IsActive), ticked: true);

        _output.WriteLine($"{viewPath} — ticked switch posts IsActive={string.Join(",", posted)}");

        var bound = await BindBooleanAsync(viewPath, nameof(TaskRecurrenceRuleEditViewModel.IsActive), posted);

        Assert.True(
            bound,
            $"The user left the Active switch ON, the browser posted IsActive={string.Join(",", posted)}, "
            + "and model binding produced FALSE. The record would save passive with nothing on screen saying so.");
    }
    ...
    [InlineData(RecurrenceRuleForm)]
    [InlineData(FieldDefinitionForm)]
    public async Task An_untouched_Active_switch_binds_FALSE(string viewPath)
    {
        var posted = await PostedValuesAsync(viewPath, nameof(TaskRecurrenceRuleEditViewModel.IsActive), ticked: false);

        _output.WriteLine($"{viewPath} — untouched switch posts IsActive={string.Join(",", posted)}");
        ...
        // the model was seeded with, and turning the switch off would silently do nothing.
        Assert.NotEmpty(posted);

        var bound = await BindBooleanAsync(viewPath, nameof(TaskRecurrenceRuleEditViewModel.IsActive), posted);

        Assert.False(bound, "The user turned the Active switch OFF and model binding produced TRUE.");
    }
    ...
    [InlineData(RecurrenceRuleForm)]
    [InlineData(FieldDefinitionForm)]
    public async Task The_FIRST_control_named_IsActive_is_the_switch_itself(string viewPath)
    {
        /*
        ...
        var controls = ControlsNamed(html, nameof(TaskRecurrenceRuleEditViewModel.IsActive));

        Assert.NotEmpty(controls);

        var order = string.Join(" then ", controls.Select(c => $"{c.Type}={c.Value}"));
        _output.WriteLine($"{viewPath} — controls named IsActive: {order}");

        Assert.True(
            string.Equals(controls[0].Type, "checkbox", StringComparison.OrdinalIgnoreCase),
            $"A '{controls[0].Type}' input named IsActive comes BEFORE the switch ({order}). Model binding takes "
            + "the first value for a repeated name, so that input — not the user — decides what gets saved. The "
            + "companion hidden asp-for emits at the closing form tag is the only one that belongs here; delete "
    ...
    private async Task<bool> BindBooleanAsync(string viewPath, string name, string[] postedValues)
    {
        ...
        var model = NewModel(viewPath, seed: !postedValues.Contains("true"));
    ...
    private static object NewModel(string viewPath, bool seed) => viewPath switch
    {
        RecurrenceRuleForm => new TaskRecurrenceRuleEditViewModel { IsActive = seed },
        FieldDefinitionForm => new TaskFieldDefinitionEditViewModel { IsActive = seed },
    ...
    private async Task<string> RenderAsync(string viewPath)
    {
        ...
        var model = NewModel(viewPath, seed: true);

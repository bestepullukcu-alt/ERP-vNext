...
using Diten.Web.Models.HumanCapital.DevelopmentPlans;
using Diten.Web.Views.HumanCapital.DevelopmentPlans;
...
[Authorize]
[Route("HumanCapital/DevelopmentPlans")]
public sealed class DevelopmentPlansController : Controller
{
    private const string BaseView = "~/Views/HumanCapital/DevelopmentPlans/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<DevelopmentPlansIndex> _localizer;
    ...
    public DevelopmentPlansController(
        ...
        IStringLocalizer<DevelopmentPlansIndex> localizer,
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new DevelopmentPlansEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] DevelopmentPlansEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/development-plans",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "DevelopmentPlans create failed.");
    ...
    private async Task<DevelopmentPlansDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/development-plans/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<DevelopmentPlansDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    // ManagerActionUxBoundaryState is intentionally omitted from the payload (pack-reserved);
    // the service applies its own default.
    private static DevelopmentPlansSavePayload ToPayload(DevelopmentPlansEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        DevelopmentPlanReadinessState = model.DevelopmentPlanReadinessState,
        DevelopmentPlanWorkflowBoundaryState = model.DevelopmentPlanWorkflowBoundaryState,
        GoalAssignmentBoundaryState = model.GoalAssignmentBoundaryState,
        LearningAssignmentBoundaryState = model.LearningAssignmentBoundaryState,
        SkillGapScoringBoundaryState = model.SkillGapScoringBoundaryState,
        RatingBoundaryState = model.RatingBoundaryState,
        RecommendationBoundaryState = model.RecommendationBoundaryState,
        RankingBoundaryState = model.RankingBoundaryState,
        AutomatedDecisionBoundaryState = model.AutomatedDecisionBoundaryState,
        CoachingActionBoundaryState = model.CoachingActionBoundaryState,
        ...
        DevelopmentPlanReadinessVersion = model.DevelopmentPlanReadinessVersion,

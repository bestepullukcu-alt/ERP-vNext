...
using Diten.Web.Models.TalentEcosystem.WorkforceAnalytics;
using Diten.Web.Views.TalentEcosystem.WorkforceAnalytics;
...
[Authorize]
[Route("TalentEcosystem/WorkforceAnalytics")]
public sealed class WorkforceAnalyticsController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/WorkforceAnalytics/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<WorkforceAnalyticsIndex> _localizer;
    private readonly ILogger<WorkforceAnalyticsController> _logger;
    ...
    public WorkforceAnalyticsController(
        ...
        IStringLocalizer<WorkforceAnalyticsIndex> localizer,
        ILogger<WorkforceAnalyticsController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new WorkforceAnalyticsEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] WorkforceAnalyticsEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/workforce-analytics",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "WorkforceAnalytics create failed.");
    ...
    private async Task<WorkforceAnalyticsDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/workforce-analytics/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<WorkforceAnalyticsDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static WorkforceAnalyticsSavePayload ToPayload(WorkforceAnalyticsEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        WorkforceAnalyticsReadinessState = model.WorkforceAnalyticsReadinessState,
        AnalyticsCatalogBoundaryState = model.AnalyticsCatalogBoundaryState,
        MetricBindingIntakeBoundaryState = model.MetricBindingIntakeBoundaryState,
        ...
        AnalyticsReviewBoundaryState = model.AnalyticsReviewBoundaryState,
        ...
        WorkforceAnalyticsReadinessVersion = model.WorkforceAnalyticsReadinessVersion,

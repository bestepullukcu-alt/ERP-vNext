...
using Diten.Web.Models.TalentEcosystem.AssociationOperations;
using Diten.Web.Views.TalentEcosystem.AssociationOperations;
...
[Authorize]
[Route("TalentEcosystem/AssociationOperations")]
public sealed class AssociationOperationsController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/AssociationOperations/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<AssociationOperationsIndex> _localizer;
    private readonly ILogger<AssociationOperationsController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public AssociationOperationsController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<AssociationOperationsIndex> localizer,
        ILogger<AssociationOperationsController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new AssociationOperationsEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] AssociationOperationsEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/association-operations",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "AssociationOperations create failed.");
    ...
    private async Task<AssociationOperationsDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/association-operations/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<AssociationOperationsDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static AssociationOperationsSavePayload ToPayload(AssociationOperationsEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        AssociationOperationsReadinessState = model.AssociationOperationsReadinessState,
        MembershipCatalogBoundaryState = model.MembershipCatalogBoundaryState,
        ServiceBindingIntakeBoundaryState = model.ServiceBindingIntakeBoundaryState,
        ProgramScopeBoundaryState = model.ProgramScopeBoundaryState,
        VisibilityControlBoundaryState = model.VisibilityControlBoundaryState,
        OperationsReviewBoundaryState = model.OperationsReviewBoundaryState,
        AutomatedDecisionBoundaryState = model.AutomatedDecisionBoundaryState,
        MemberRegistrySourceDependencyState = model.MemberRegistrySourceDependencyState,
        ...
        AssociationOperationsReadinessVersion = model.AssociationOperationsReadinessVersion,

...
using Diten.Web.Models.TalentEcosystem.SkillsGapHeatmap;
using Diten.Web.Views.TalentEcosystem.SkillsGapHeatmap;
...
[Authorize]
[Route("TalentEcosystem/SkillsGapHeatmap")]
public sealed class SkillsGapHeatmapController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/SkillsGapHeatmap/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<SkillsGapHeatmapIndex> _localizer;
    private readonly ILogger<SkillsGapHeatmapController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public SkillsGapHeatmapController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<SkillsGapHeatmapIndex> localizer,
        ILogger<SkillsGapHeatmapController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new SkillsGapHeatmapEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] SkillsGapHeatmapEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/skills-gap-heatmap",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "SkillsGapHeatmap create failed.");
    ...
    private async Task<SkillsGapHeatmapDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/skills-gap-heatmap/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<SkillsGapHeatmapDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static SkillsGapHeatmapSavePayload ToPayload(SkillsGapHeatmapEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        SkillsGapHeatmapReadinessState = model.SkillsGapHeatmapReadinessState,
        GapCatalogBoundaryState = model.GapCatalogBoundaryState,
        HeatmapBindingIntakeBoundaryState = model.HeatmapBindingIntakeBoundaryState,
        SeverityScopeBoundaryState = model.SeverityScopeBoundaryState,
        VisibilityControlBoundaryState = model.VisibilityControlBoundaryState,
        GapReviewBoundaryState = model.GapReviewBoundaryState,
        ...
        TalentDemandForecastSourceDependencyState = model.TalentDemandForecastSourceDependencyState,
        ...
        SkillsGapHeatmapReadinessVersion = model.SkillsGapHeatmapReadinessVersion,

...
using Diten.Web.Models.HumanCapital.SelfService;
using Diten.Web.Views.HumanCapital.SelfService;
...
[Authorize]
[Route("HumanCapital/SelfService")]
public sealed class SelfServiceController : Controller
{
    private const string BaseView = "~/Views/HumanCapital/SelfService/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<SelfServiceIndex> _localizer;
    ...
    public SelfServiceController(
        ...
        IStringLocalizer<SelfServiceIndex> localizer,
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new SelfServiceEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] SelfServiceEditViewModel model)
    {
        ...
        {
            _logger.LogError(ex, "SelfService create failed.");
    ...
    private async Task<SelfServiceDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/self-service/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<SelfServiceDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static SelfServiceSavePayload ToPayload(SelfServiceEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        SelfServiceReadinessState = model.SelfServiceReadinessState,
        RequestIntakeBoundaryState = model.RequestIntakeBoundaryState,
        ApprovalRoutingBoundaryState = model.ApprovalRoutingBoundaryState,
        InboxDeliveryBoundaryState = model.InboxDeliveryBoundaryState,
        ProfileSelfUpdateBoundaryState = model.ProfileSelfUpdateBoundaryState,
        DelegationScopeBoundaryState = model.DelegationScopeBoundaryState,
        AutomatedDecisionBoundaryState = model.AutomatedDecisionBoundaryState,
        IdentityDirectoryDependencyState = model.IdentityDirectoryDependencyState,
        HcmCapabilityDependencyState = model.HcmCapabilityDependencyState,
        ...
        SelfServiceReadinessVersion = model.SelfServiceReadinessVersion,

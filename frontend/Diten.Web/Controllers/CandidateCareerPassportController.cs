...
using Diten.Web.Models.TalentEcosystem.CandidateCareerPassport;
using Diten.Web.Views.TalentEcosystem.CandidateCareerPassport;
...
[Authorize]
[Route("TalentEcosystem/CandidateCareerPassport")]
public sealed class CandidateCareerPassportController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/CandidateCareerPassport/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<CandidateCareerPassportIndex> _localizer;
    private readonly ILogger<CandidateCareerPassportController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public CandidateCareerPassportController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<CandidateCareerPassportIndex> localizer,
        ILogger<CandidateCareerPassportController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new CandidateCareerPassportEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] CandidateCareerPassportEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/candidate-career-passport",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "CandidateCareerPassport create failed.");
    ...
    private async Task<CandidateCareerPassportDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/candidate-career-passport/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<CandidateCareerPassportDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static CandidateCareerPassportSavePayload ToPayload(CandidateCareerPassportEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        CandidateCareerPassportReadinessState = model.CandidateCareerPassportReadinessState,
        CareerMilestoneCatalogBoundaryState = model.CareerMilestoneCatalogBoundaryState,
        ExperienceIntakeBoundaryState = model.ExperienceIntakeBoundaryState,
        OwnershipScopeBoundaryState = model.OwnershipScopeBoundaryState,
        ...
        PortabilityPolicyState = model.PortabilityPolicyState,
        SourceContractVersion = model.SourceContractVersion,
        CandidateCareerPassportReadinessVersion = model.CandidateCareerPassportReadinessVersion,

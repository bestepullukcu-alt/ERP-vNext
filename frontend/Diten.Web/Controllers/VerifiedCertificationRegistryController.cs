...
using Diten.Web.Models.TalentEcosystem.VerifiedCertificationRegistry;
using Diten.Web.Views.TalentEcosystem.VerifiedCertificationRegistry;
...
[Authorize]
[Route("TalentEcosystem/VerifiedCertificationRegistry")]
public sealed class VerifiedCertificationRegistryController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/VerifiedCertificationRegistry/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<VerifiedCertificationRegistryIndex> _localizer;
    private readonly ILogger<VerifiedCertificationRegistryController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public VerifiedCertificationRegistryController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<VerifiedCertificationRegistryIndex> localizer,
        ILogger<VerifiedCertificationRegistryController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new VerifiedCertificationRegistryEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] VerifiedCertificationRegistryEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/verified-certification-registry",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "VerifiedCertificationRegistry create failed.");
    ...
    private async Task<VerifiedCertificationRegistryDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/verified-certification-registry/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<VerifiedCertificationRegistryDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static VerifiedCertificationRegistrySavePayload ToPayload(VerifiedCertificationRegistryEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        VerifiedCertificationRegistryReadinessState = model.VerifiedCertificationRegistryReadinessState,
        CertificationCatalogBoundaryState = model.CertificationCatalogBoundaryState,
        VerificationIntakeBoundaryState = model.VerificationIntakeBoundaryState,
        IssuerBindingScopeBoundaryState = model.IssuerBindingScopeBoundaryState,
        ...
        VerifiedCertificationRegistryReadinessVersion = model.VerifiedCertificationRegistryReadinessVersion,

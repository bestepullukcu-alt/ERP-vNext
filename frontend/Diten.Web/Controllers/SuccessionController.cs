...
using Diten.Web.Models.HumanCapital.Succession;
using Diten.Web.Views.HumanCapital.Succession;
...
[Authorize]
[Route("HumanCapital/Succession")]
public sealed class SuccessionController : Controller
{
    private const string BaseView = "~/Views/HumanCapital/Succession/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<SuccessionIndex> _localizer;
    private readonly ILogger<SuccessionController> _logger;
    ...
    public SuccessionController(
        ...
        IStringLocalizer<SuccessionIndex> localizer,
        ILogger<SuccessionController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new SuccessionEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] SuccessionEditViewModel model)
    {
        ...
        {
            _logger.LogError(ex, "Succession create failed.");
    ...
    private async Task<SuccessionDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/succession/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<SuccessionDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static SuccessionSavePayload ToPayload(SuccessionEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        SuccessionReadinessState = model.SuccessionReadinessState,
        SuccessionPoolBoundaryState = model.SuccessionPoolBoundaryState,
        HighPotentialIdentificationBoundaryState = model.HighPotentialIdentificationBoundaryState,
        NominationWorkflowBoundaryState = model.NominationWorkflowBoundaryState,
        ReadinessAssessmentBoundaryState = model.ReadinessAssessmentBoundaryState,
        TalentReviewBoundaryState = model.TalentReviewBoundaryState,
        AutomatedDecisionBoundaryState = model.AutomatedDecisionBoundaryState,
        TalentProfileDependencyState = model.TalentProfileDependencyState,
        ...
        SuccessionReadinessVersion = model.SuccessionReadinessVersion,

...
using Diten.Web.Models.DataKnowledge.KpiCatalog;
using Diten.Web.Views.DataKnowledge.KpiCatalog;
...
[Authorize]
[Route("DataKnowledge/KpiCatalog")]
public sealed class KpiCatalogController : Controller
{
    private const string BaseView = "~/Views/DataKnowledge/KpiCatalog/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<KpiCatalogIndex> _localizer;
    private readonly ILogger<KpiCatalogController> _logger;
    ...
    public KpiCatalogController(
        ...
        IStringLocalizer<KpiCatalogIndex> localizer,
        ILogger<KpiCatalogController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new KpiCatalogEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] KpiCatalogEditViewModel model)
    {
        ...
        {
            _logger.LogError(ex, "KpiCatalog create failed.");
    ...
    private async Task<KpiCatalogDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/kpi-catalog/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<KpiCatalogDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static KpiCatalogSavePayload ToPayload(KpiCatalogEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        KpiCatalogReadinessState = model.KpiCatalogReadinessState,
        KpiIdentityCatalogBoundaryState = model.KpiIdentityCatalogBoundaryState,
        DefinitionBindingIntakeBoundaryState = model.DefinitionBindingIntakeBoundaryState,
        ...
        CatalogReviewBoundaryState = model.CatalogReviewBoundaryState,
        ...
        DataDictionaryDependencyState = model.DataDictionaryDependencyState,
        ...
        KpiCatalogReadinessVersion = model.KpiCatalogReadinessVersion,

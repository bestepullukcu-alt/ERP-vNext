...
using Diten.Web.Models.HumanCapital.ApplicantIntake;
...
{
    private const string BaseView = "~/Views/HumanCapital/ApplicantIntake/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<ApplicantIntakeIndex> _localizer;
    private readonly ILogger<ApplicantIntakeController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public ApplicantIntakeController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<ApplicantIntakeIndex> localizer,
        ILogger<ApplicantIntakeController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new ApplicantIntakeEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] ApplicantIntakeEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/applicant-intake",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "ApplicantIntake create failed.");
    ...
    private async Task<ApplicantIntakeDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/applicant-intake/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<ApplicantIntakeDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static ApplicantIntakeSavePayload ToPayload(ApplicantIntakeEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        IntakeState = model.IntakeState,
        SourceChannelState = model.SourceChannelState,
        ConsentPreconditionState = model.ConsentPreconditionState,
        DataMinimizationState = model.DataMinimizationState,
        DuplicateHandlingState = model.DuplicateHandlingState,
        RetentionPolicyState = model.RetentionPolicyState,
        EvidencePolicyState = model.EvidencePolicyState,
        ApplicantIdentityBoundaryState = model.ApplicantIdentityBoundaryState,
        PublicUxBoundaryState = model.PublicUxBoundaryState,
        ...
        ApplicantIntakeVersion = model.ApplicantIntakeVersion,

...
using Diten.Web.Models.TalentEcosystem.IndustrySuccessionPool;
using Diten.Web.Views.TalentEcosystem.IndustrySuccessionPool;
...
[Authorize]
[Route("TalentEcosystem/IndustrySuccessionPool")]
public sealed class IndustrySuccessionPoolController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/IndustrySuccessionPool/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<IndustrySuccessionPoolIndex> _localizer;
    private readonly ILogger<IndustrySuccessionPoolController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public IndustrySuccessionPoolController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<IndustrySuccessionPoolIndex> localizer,
        ILogger<IndustrySuccessionPoolController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new IndustrySuccessionPoolEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] IndustrySuccessionPoolEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/industry-succession-pool",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "IndustrySuccessionPool create failed.");
    ...
    private async Task<IndustrySuccessionPoolDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/industry-succession-pool/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<IndustrySuccessionPoolDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static IndustrySuccessionPoolSavePayload ToPayload(IndustrySuccessionPoolEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        IndustrySuccessionPoolReadinessState = model.IndustrySuccessionPoolReadinessState,
        PoolCatalogBoundaryState = model.PoolCatalogBoundaryState,
        CandidateInclusionIntakeBoundaryState = model.CandidateInclusionIntakeBoundaryState,
        ReadinessTierScopeBoundaryState = model.ReadinessTierScopeBoundaryState,
        VisibilityControlBoundaryState = model.VisibilityControlBoundaryState,
        SuccessionReviewBoundaryState = model.SuccessionReviewBoundaryState,
        ...
        TalentPoolSourceDependencyState = model.TalentPoolSourceDependencyState,
        ...
        IndustrySuccessionPoolReadinessVersion = model.IndustrySuccessionPoolReadinessVersion,

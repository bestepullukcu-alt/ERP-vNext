...
[Route("Platform/DomainManagement")]
public sealed class DomainManagementController : Controller
{
    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;

    public DomainManagementController(HttpClient httpClient, IConfiguration configuration)
    {
    ...
    [HttpGet("")]
    public IActionResult Index() => View("~/Views/Platform/DomainManagement/Index.cshtml");

    [HttpGet("api")]
    public Task<IActionResult> ListProxy()
    {
        var targetUrl = $"{_gatewayUrl}/api/platform/module-domains{Request.QueryString}";
    ...
    {
        return ProxyGatewayAsync(HttpMethod.Get, $"{_gatewayUrl}/api/platform/module-domains/{id}");
    }
    ...
    public async Task<IActionResult> CreateProxy()
    {
        ...
        return await ProxyGatewayAsync(HttpMethod.Post, $"{_gatewayUrl}/api/platform/module-domains", body);
    }
    ...
    public async Task<IActionResult> UpdateProxy(Guid id)
    {
        using var reader = new StreamReader(Request.Body, Encoding.UTF8);
        var body = await reader.ReadToEndAsync();
        return await ProxyGatewayAsync(HttpMethod.Put, $"{_gatewayUrl}/api/platform/module-domains/{id}", body);
    }

    [HttpDelete("api/{id:guid}")]
    public Task<IActionResult> DeleteProxy(Guid id)
    {
        return ProxyGatewayAsync(HttpMethod.Delete, $"{_gatewayUrl}/api/platform/module-domains/{id}");
    }

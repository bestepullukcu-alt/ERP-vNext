...
using Diten.Web.Models.TalentEcosystem.PayBenchmarking;
using Diten.Web.Views.TalentEcosystem.PayBenchmarking;
...
[Authorize]
[Route("TalentEcosystem/PayBenchmarking")]
public sealed class PayBenchmarkingController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/PayBenchmarking/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<PayBenchmarkingIndex> _localizer;
    private readonly ILogger<PayBenchmarkingController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public PayBenchmarkingController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<PayBenchmarkingIndex> localizer,
        ILogger<PayBenchmarkingController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new PayBenchmarkingEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] PayBenchmarkingEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/salary-benchmarking",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "PayBenchmarking create failed.");
    ...
    private async Task<PayBenchmarkingDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/salary-benchmarking/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<PayBenchmarkingDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static PayBenchmarkingSavePayload ToPayload(PayBenchmarkingEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        PayBenchmarkingReadinessState = model.PayBenchmarkingReadinessState,
        ReferenceRangeCatalogBoundaryState = model.ReferenceRangeCatalogBoundaryState,
        ContributionIntakeBoundaryState = model.ContributionIntakeBoundaryState,
        ...
        BenchmarkingReviewBoundaryState = model.BenchmarkingReviewBoundaryState,
        ...
        PayBenchmarkingReadinessVersion = model.PayBenchmarkingReadinessVersion,

...
using Diten.Web.Models.TalentEcosystem.SectorTalentTrends;
using Diten.Web.Views.TalentEcosystem.SectorTalentTrends;
...
[Authorize]
[Route("TalentEcosystem/SectorTalentTrends")]
public sealed class SectorTalentTrendsController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/SectorTalentTrends/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<SectorTalentTrendsIndex> _localizer;
    private readonly ILogger<SectorTalentTrendsController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public SectorTalentTrendsController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<SectorTalentTrendsIndex> localizer,
        ILogger<SectorTalentTrendsController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new SectorTalentTrendsEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] SectorTalentTrendsEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/sector-talent-trends",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "SectorTalentTrends create failed.");
    ...
    private async Task<SectorTalentTrendsDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/sector-talent-trends/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<SectorTalentTrendsDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static SectorTalentTrendsSavePayload ToPayload(SectorTalentTrendsEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        SectorTalentTrendsReadinessState = model.SectorTalentTrendsReadinessState,
        TrendCatalogBoundaryState = model.TrendCatalogBoundaryState,
        SignalBindingIntakeBoundaryState = model.SignalBindingIntakeBoundaryState,
        AggregationScopeBoundaryState = model.AggregationScopeBoundaryState,
        VisibilityControlBoundaryState = model.VisibilityControlBoundaryState,
        TrendReviewBoundaryState = model.TrendReviewBoundaryState,
        ...
        SectorTalentTrendsReadinessVersion = model.SectorTalentTrendsReadinessVersion,

...
using Diten.Web.Models.TalentEcosystem.IndustryTalentPool;
using Diten.Web.Views.TalentEcosystem.IndustryTalentPool;
...
[Authorize]
[Route("TalentEcosystem/IndustryTalentPool")]
public sealed class IndustryTalentPoolController : Controller
{
    private const string BaseView = "~/Views/TalentEcosystem/IndustryTalentPool/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<IndustryTalentPoolIndex> _localizer;
    private readonly ILogger<IndustryTalentPoolController> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public IndustryTalentPoolController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<IndustryTalentPoolIndex> localizer,
        ILogger<IndustryTalentPoolController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new IndustryTalentPoolEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] IndustryTalentPoolEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/industry-talent-pool",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "IndustryTalentPool create failed.");
    ...
    private async Task<IndustryTalentPoolDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/industry-talent-pool/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<IndustryTalentPoolDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static IndustryTalentPoolSavePayload ToPayload(IndustryTalentPoolEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        IndustryTalentPoolReadinessState = model.IndustryTalentPoolReadinessState,
        PoolMembershipCatalogBoundaryState = model.PoolMembershipCatalogBoundaryState,
        CandidateInclusionIntakeBoundaryState = model.CandidateInclusionIntakeBoundaryState,
        EligibilityScopeBoundaryState = model.EligibilityScopeBoundaryState,
        VisibilityControlBoundaryState = model.VisibilityControlBoundaryState,
        PoolCurationReviewBoundaryState = model.PoolCurationReviewBoundaryState,
        ...
        EligibilityPolicyState = model.EligibilityPolicyState,
        SourceContractVersion = model.SourceContractVersion,
        IndustryTalentPoolReadinessVersion = model.IndustryTalentPoolReadinessVersion,

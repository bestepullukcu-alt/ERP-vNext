...
{
    private const string BackendSlug = "hris-sources";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly ILogger<HrisSourcesController> _logger;

    public HrisSourcesController(
        HttpClient httpClient,
        IConfiguration configuration,
        ILogger<HrisSourcesController> logger)
    {
    ...
    private async Task<IActionResult> ProxyJsonGatewayAsync(HttpMethod method, string targetUrl, bool readBody = false)
    {
        ...
        {
            _logger.LogError(ex, "Platform HRIS sources proxy request failed for {Method} {TargetUrl}.", method, targetUrl);

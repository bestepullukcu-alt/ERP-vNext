...
{
    private const string BackendSlug = "payroll-sources";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly ILogger<PayrollSourcesController> _logger;

    public PayrollSourcesController(
        HttpClient httpClient,
        IConfiguration configuration,
        ILogger<PayrollSourcesController> logger)
    {
    ...
    private async Task<IActionResult> ProxyJsonGatewayAsync(HttpMethod method, string targetUrl, bool readBody = false)
    {
        ...
        {
            _logger.LogError(ex, "Platform payroll sources proxy request failed for {Method} {TargetUrl}.", method, targetUrl);

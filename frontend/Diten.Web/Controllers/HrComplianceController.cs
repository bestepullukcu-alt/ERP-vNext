...
using Diten.Web.Models.HumanCapital.HrCompliance;
using Diten.Web.Views.HumanCapital.HrCompliance;
...
[Authorize]
[Route("HumanCapital/HrCompliance")]
public sealed class HrComplianceController : Controller
{
    private const string BaseView = "~/Views/HumanCapital/HrCompliance/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<HrComplianceIndex> _localizer;
    private readonly ILogger<HrComplianceController> _logger;
    ...
    public HrComplianceController(
        ...
        IStringLocalizer<HrComplianceIndex> localizer,
        ILogger<HrComplianceController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new HrComplianceEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] HrComplianceEditViewModel model)
    {
        ...
        {
            _logger.LogError(ex, "HrCompliance create failed.");
    ...
    private async Task<HrComplianceDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/hr-compliance/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<HrComplianceDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static HrComplianceSavePayload ToPayload(HrComplianceEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        HrComplianceReadinessState = model.HrComplianceReadinessState,
        ObligationCatalogBoundaryState = model.ObligationCatalogBoundaryState,
        ControlMappingBoundaryState = model.ControlMappingBoundaryState,
        StatutoryReportDefinitionBoundaryState = model.StatutoryReportDefinitionBoundaryState,
        FilingScheduleBoundaryState = model.FilingScheduleBoundaryState,
        AttestationClosureBoundaryState = model.AttestationClosureBoundaryState,
        AutomatedDecisionBoundaryState = model.AutomatedDecisionBoundaryState,
        RegulatorySourceDependencyState = model.RegulatorySourceDependencyState,
        HcmDataSourceDependencyState = model.HcmDataSourceDependencyState,
        ...
        HrComplianceReadinessVersion = model.HrComplianceReadinessVersion,

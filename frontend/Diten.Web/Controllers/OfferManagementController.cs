...
using Diten.Web.Models.HumanCapital.OfferManagement;
using Diten.Web.Views.HumanCapital.OfferManagement;
...
[Authorize]
[Route("HumanCapital/OfferManagement")]
public sealed class OfferManagementController : Controller
{
    private const string BaseView = "~/Views/HumanCapital/OfferManagement/";

    private readonly HttpClient _httpClient;
    private readonly string _gatewayUrl;
    private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
    private readonly IStringLocalizer<OfferManagementIndex> _localizer;
    ...
    public OfferManagementController(
        HttpClient httpClient,
        IConfiguration configuration,
        IStringLocalizer<SharedResource> sharedLocalizer,
        IStringLocalizer<OfferManagementIndex> localizer,
        ILogger<OfferManagementController> logger)
    {
    ...
    public IActionResult Create() =>
        View($"{BaseView}Create.cshtml", new OfferManagementEditViewModel());

    [HttpPost("Create")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create([FromForm] OfferManagementEditViewModel model)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(
                $"{_gatewayUrl}/api/offer-management",
                ToPayload(model),
                _jsonOptions);
        ...
        {
            _logger.LogError(ex, "OfferManagement create failed.");
    ...
    private async Task<OfferManagementDetailViewModel?> LoadApiModelAsync(Guid id)
    {
        if (!AddAuthHeaders())
            return null;

        var response = await _httpClient.GetAsync($"{_gatewayUrl}/api/offer-management/{id}");
        if (!response.IsSuccessStatusCode)
            return null;

        var payload = await response.Content
            .ReadFromJsonAsync<GatewayResponse<OfferManagementDetailViewModel>>(_jsonOptions);
        return payload?.Data;
    }

    private static OfferManagementSavePayload ToPayload(OfferManagementEditViewModel model) => new()
    {
        Code = model.Code,
        DisplayName = model.DisplayName,
        OfferReadinessState = model.OfferReadinessState,
        OfferWorkflowBoundaryState = model.OfferWorkflowBoundaryState,
        ApprovalWorkflowBoundaryState = model.ApprovalWorkflowBoundaryState,
        CandidateAcceptanceBoundaryState = model.CandidateAcceptanceBoundaryState,
        OfferDocumentBoundaryState = model.OfferDocumentBoundaryState,
        ...
        OfferReadinessVersion = model.OfferReadinessVersion,

...
describe("the two-column grid is the shape it claims to be", () => {
  ...
  test("six cards on the left, five on the right — and they are DIRECT children of their column", () => {
    /*
     ...
     *
     * The left column gained the CHECKLIST card, then the ATTACHMENTS card (WP-PSS-MOD0024-ATTACHMENTS-UX-01):
     * both describe the task's own content, not the governance decisions on the right. Both are create-only —
     * form-page.js removes them on edit, where a checklist and an attachment list are separate documents with
     * their own endpoints.
     */
    const row = parse().querySelector("form#taskForm > .row.g-4");
    const [left, right] = Array.from(row.children);

    expect(Array.from(left.children).filter((n) => n.tagName === "SECTION")).toHaveLength(6);

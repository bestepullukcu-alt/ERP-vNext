...
async function boot() {
  ...
  window.strategyEnterpriseMetaApi = {
    runtimeIdPreview: vi.fn().mockResolvedValue({ objectiveId: "O-000201" })
  };
  ...
  window.strategyKpisApi = {
    list: vi.fn().mockResolvedValue({
      items: [{ id: "KPI-1", name: "Scorecard Data Quality Index" }]
    })
  };
...
describe("objective create save", () => {
  beforeEach(async () => {
    await boot();
  });
  ...
  it("submits the canonical payload and redirects with the saved objective id", async () => {
    ...
    document.getElementById("objective-name").value = "Improve Quality";
    document.getElementById("objective-statement").value = "Lift quality score.";
    ...
    document.getElementById("objective-kpi-uom").value = "Currency";
    document.getElementById("objective-direction").value = "Increase";
    document.getElementById("objective-reporting-frequency").value = "Real Time";
    document.getElementById("objective-save").classList.remove("d-none");

    document.getElementById("objective-save").click();
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(window.strategyObjectivesApi.create).toHaveBeenCalledWith(
      expect.objectContaining({
        objectiveName: "Improve Quality",
        objectiveStatement: "Lift quality score.",
        objectiveTypeId: "Growth",
        strategicThemeId: "theme-growth",
        strategyPeriodId: "sp-1",
        ownerCompanyId: "comp-1",
    ...
    expect(document.getElementById("objective-id").value).toBe("O-2001");

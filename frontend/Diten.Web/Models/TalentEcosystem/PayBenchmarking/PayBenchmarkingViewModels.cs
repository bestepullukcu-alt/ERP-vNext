...
namespace Diten.Web.Models.TalentEcosystem.PayBenchmarking;

// Enum field values map 1:1 to the backend PayBenchmarkingReadinessState ordinals:
...
public sealed class PayBenchmarkingEditViewModel
{
    ...
    // Readiness & workflow
    public int PayBenchmarkingReadinessState { get; set; } = 0;                     // Draft
    public int ReferenceRangeCatalogBoundaryState { get; set; } = 3;                  // Blocked
    public int ContributionIntakeBoundaryState { get; set; } = 3;     // Blocked
    public int AggregationScopeBoundaryState { get; set; } = 3;              // Blocked
    public int VisibilityControlBoundaryState { get; set; } = 3;            // Blocked
    public int BenchmarkingReviewBoundaryState { get; set; } = 3;                   // Blocked
...
public sealed class PayBenchmarkingDetailViewModel
{
    public Guid Id { get; set; }
    public string Code { get; set; } = string.Empty;
    public string DisplayName { get; set; } = string.Empty;
    public int PayBenchmarkingReadinessState { get; set; }
    public int ReferenceRangeCatalogBoundaryState { get; set; }
    public int ContributionIntakeBoundaryState { get; set; }
    public int AggregationScopeBoundaryState { get; set; }
    public int VisibilityControlBoundaryState { get; set; }
    public int BenchmarkingReviewBoundaryState { get; set; }
    ...
    public string SourceContractVersion { get; set; } = string.Empty;
    public long PayBenchmarkingReadinessVersion { get; set; }
    public DateTimeOffset? LastEvaluatedAt { get; set; }
    public string? DeferredReason { get; set; }

    // Ordinal → canonical state name, mirroring PayBenchmarkingReadinessState.
...
// Sent to the gateway create endpoint. Property names match PayBenchmarkingReadinessCreateRequest.
// This module has NO manager/employee assessment UX fields; every scalar request field is included.
public sealed class PayBenchmarkingSavePayload
{
    public string Code { get; set; } = string.Empty;
    public string DisplayName { get; set; } = string.Empty;
    public int PayBenchmarkingReadinessState { get; set; }

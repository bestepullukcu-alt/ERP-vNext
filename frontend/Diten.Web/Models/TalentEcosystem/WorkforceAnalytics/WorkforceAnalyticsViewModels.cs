...
namespace Diten.Web.Models.TalentEcosystem.WorkforceAnalytics;

// Enum field values map 1:1 to the backend WorkforceAnalyticsReadinessState ordinals:
...
public sealed class WorkforceAnalyticsEditViewModel
{
    ...
    // Readiness & workflow
    public int WorkforceAnalyticsReadinessState { get; set; } = 0;                     // Draft
    public int AnalyticsCatalogBoundaryState { get; set; } = 3;                  // Blocked
    public int MetricBindingIntakeBoundaryState { get; set; } = 3;     // Blocked
    public int AggregationScopeBoundaryState { get; set; } = 3;              // Blocked
    public int VisibilityControlBoundaryState { get; set; } = 3;            // Blocked
    public int AnalyticsReviewBoundaryState { get; set; } = 3;                   // Blocked
...
public sealed class WorkforceAnalyticsDetailViewModel
{
    public Guid Id { get; set; }
    public string Code { get; set; } = string.Empty;
    public string DisplayName { get; set; } = string.Empty;
    public int WorkforceAnalyticsReadinessState { get; set; }
    public int AnalyticsCatalogBoundaryState { get; set; }
    public int MetricBindingIntakeBoundaryState { get; set; }
    public int AggregationScopeBoundaryState { get; set; }
    public int VisibilityControlBoundaryState { get; set; }
    public int AnalyticsReviewBoundaryState { get; set; }
    ...
    public string SourceContractVersion { get; set; } = string.Empty;
    public long WorkforceAnalyticsReadinessVersion { get; set; }
    public DateTimeOffset? LastEvaluatedAt { get; set; }
    public string? DeferredReason { get; set; }

    // Ordinal → canonical state name, mirroring WorkforceAnalyticsReadinessState.
...
// Sent to the gateway create endpoint. Property names match WorkforceAnalyticsReadinessCreateRequest.
// This module has NO manager/employee assessment UX fields; every scalar request field is included.
public sealed class WorkforceAnalyticsSavePayload
{
    public string Code { get; set; } = string.Empty;
    public string DisplayName { get; set; } = string.Empty;
    public int WorkforceAnalyticsReadinessState { get; set; }

/**
 * MOD-0141 Requisition - DataTables Index Script
 * Golden Compact aligned (>8 form fields): create/details use full MVC pages; submit lifecycle action on Details.
 * Direct-gateway profile: browser reads/deletes through the Gateway (window.API.procurement + /api/requisitions).
 */
'use strict';

const RequisitionsList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-requisitions');
    const apiUrl = window.API?.procurement;
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'Procurement', pageKey: 'Requisitions' };
    ...
    // Contract RequisitionStatus enum ordinals — the service serializes enums numerically
    // (no JsonStringEnumConverter). Normalize both numeric and string forms to canonical names.
    const REQUISITION_STATUS_NAMES = ['Draft', 'Submitted', 'Approved', 'Rejected', 'Converted', 'Cancelled'];
    ...
    const statusName = (v) => {
        if (typeof v === 'number') return REQUISITION_STATUS_NAMES[v] || String(v);
    ...
    const lineCount = (lines) => Array.isArray(lines) ? lines.length : 0;
    const earliestNeedBy = (lines) => {
        if (!Array.isArray(lines)) return '';
        const dates = lines
            .map((l) => normalizeString(l?.needBy))
            .filter(Boolean)
            .sort();
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[Requisitions SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const getStatusMap = () => ({
        ...
        'Submitted': { title: L.StatusSubmitted || 'Submitted', class: 'bg-label-primary' },
        'Approved': { title: L.StatusApproved || 'Approved', class: 'bg-label-success' },
        'Rejected': { title: L.StatusRejected || 'Rejected', class: 'bg-label-danger' },
        'Converted': { title: L.StatusConverted || 'Converted', class: 'bg-label-info' },
        'Cancelled': { title: L.StatusCancelled || 'Cancelled', class: 'bg-label-dark' }
    });
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/requisitions/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Requisitions/Details/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.requisitionId) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/requisitions/${encodeURIComponent(row.requisitionId)}`, {
                        method: 'DELETE',
                        credentials: 'include',
            ...
            }, { entityName: row.requisitionId, type: 'danger', confirmButtonText: L.Delete });
        }
    };
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        if (!apiUrl) { console.error('[Requisitions] window.API.procurement is required.'); return; }
        syncL10n();
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[Requisitions SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    { data: 'requisitionId', name: 'control' },
                    { data: 'requisitionId', name: 'checkbox' },
                    { data: 'requisitionId', name: 'requisitionId' },
                    { data: 'status', name: 'status' },
                    { data: 'lines', name: 'lineCount' },
                    { data: 'lines', name: 'needBy' },
                    { data: 'requisitionId', name: 'action' }
                ],
                columnDefs: [
                    ...
                    },
                    {
                        targets: 4,
                        searchable: false,
                        render: (data) => lineCount(data)
                    },
                    {
                        targets: 5,
                        searchable: false,
                        render: (data, type) => {
                            const nb = earliestNeedBy(data);
                            return type === 'display' ? (nb ? formatDate(nb) : '-') : (nb || '');
                        }
                    },
                    {
                        targets: -1,
                            ...
                            return window.DitenDataTable.renderActions([
                                {
                                    ...
                                    icon: 'bx bx-show',
                                    attrs: { 'data-id': full.requisitionId, 'title': L.QuickView }
                                },
                                {
                                    key: 'delete',
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/Requisitions/Create';
                    });
...
document.addEventListener('DOMContentLoaded', () => RequisitionsList.init());

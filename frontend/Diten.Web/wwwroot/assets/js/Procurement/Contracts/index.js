/**
 * MOD-0144 Contracting & Clause Library - DataTables Index Script
 * Golden Compact aligned (>8 form fields): create/edit/details use full MVC pages; contract lifecycle actions on Details.
 * Direct-gateway profile: browser reads/deletes through the Gateway (window.API.procurement + /api/contracts).
 * The clause library (read/create) uses the same-origin controller proxy (/Contracts/clauses) so the Idempotency-Key
 * stays server-side.
 */
'use strict';

const ContractsList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-contracts');
    const apiUrl = window.API?.procurement;
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'Procurement', pageKey: 'Contracts' };
    ...
    // ContractStatus enum ordinals — the service serializes enums numerically (no JsonStringEnumConverter).
    // Normalize both numeric and string forms to canonical names.
    const STATUS_NAMES = ['Draft', 'InReview', 'Active', 'Expired', 'Terminated'];
    ...
    const statusName = (v) => {
        if (typeof v === 'number') return STATUS_NAMES[v] || String(v);
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[Contracts SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const getAppliedFilterCount = () =>
        [appliedFilters.status].filter(hasFilterValue).length;
    ...
    const getStatusMap = () => ({
        'Draft': { title: L.StatusDraft || 'Draft', class: 'bg-label-secondary' },
        'InReview': { title: L.StatusInReview || 'In Review', class: 'bg-label-warning' },
        'Active': { title: L.StatusActive || 'Active', class: 'bg-label-success' },
        'Expired': { title: L.StatusExpired || 'Expired', class: 'bg-label-dark' },
        'Terminated': { title: L.StatusTerminated || 'Terminated', class: 'bg-label-danger' }
    });
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/contracts/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Contracts/Details/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.contractId) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/contracts/${encodeURIComponent(row.contractId)}`, {
                        method: 'DELETE',
                        credentials: 'include',
            ...
            }, { entityName: row.title, type: 'danger', confirmButtonText: L.Delete });
        }
    };

    // ── Clause library (read/create) — same-origin controller proxy keeps Idempotency-Key server-side ──
    const escapeHtml = (v) => String(v ?? '').replace(/[&<>"']/g, (c) => (
        { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]
    ));
    const clauseBody = () => document.getElementById('clauseLibraryBody');
    const renderClauses = (items) => {
        const body = clauseBody();
        if (!body) return;
        if (!Array.isArray(items) || !items.length) {
            body.innerHTML = `<tr><td colspan="4" class="text-muted text-center">${L.NotAvailable || '-'}</td></tr>`;
            return;
        }
        body.innerHTML = items.map((c) => `<tr>
            <td class="fw-medium text-heading">${escapeHtml(c.clauseId)}</td>
            <td>${escapeHtml(c.category)}</td>
            <td>${escapeHtml(c.title)}</td>
            <td class="text-truncate" style="max-width: 320px;" title="${escapeHtml(c.body)}">${escapeHtml(c.body)}</td>
        </tr>`).join('');
    };
    const loadClauses = async () => {
        ...
        if (!body) return;
        try {
            const res = await fetch('/Contracts/clauses', { method: 'GET', credentials: 'same-origin' });
            if (!res.ok) return;
            const payload = await res.json();
            const items = payload?.data?.items || payload?.data?.Items || payload?.items || [];
            renderClauses(items);
        } catch (error) {
            console.error('[Contracts] Failed to load clause library.', error);
        }
    };
    const bindClauseLibrary = () => {
        if (!clauseBody()) return;
        loadClauses();
        const submitBtn = document.getElementById('btnSubmitClause');
        submitBtn?.addEventListener('click', async () => {
            const category = document.getElementById('clauseCategory')?.value?.trim() || '';
            const title = document.getElementById('clauseTitle')?.value?.trim() || '';
            const bodyText = document.getElementById('clauseBody')?.value?.trim() || '';
            if (!category || !title || !bodyText) { window.showToast?.(L.FormValidationError || L.ErrorOccurred, 'error'); return; }
            try {
                const res = await fetch('/Contracts/clauses', {
                    method: 'POST',
                ...
                if (!res.ok) {
                    ...
                    const msg = (err?.errors || err?.Errors || [])[0] || err?.error?.code || L.ErrorOccurred;
                    window.showToast?.(msg, 'error');
                    return;
                }
                window.showToast?.(L.ClauseCreated || L.RecordCreated, 'success');
                bootstrap.Modal.getInstance(document.getElementById('clauseModal'))?.hide();
                ['clauseCategory', 'clauseTitle', 'clauseBody'].forEach((id) => { const el = document.getElementById(id); if (el) el.value = ''; });
                loadClauses();
            } catch (error) {
                console.error(error);
                window.showToast?.(L.ErrorOccurred, 'error');
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        if (!apiUrl) { console.error('[Contracts] window.API.procurement is required.'); return; }
        syncL10n();
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[Contracts SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
                            ...
                            return window.DitenDataTable.renderActions([
                                {
                                    ...
                                    icon: 'bx bx-show',
                                    attrs: { 'data-id': full.contractId, 'title': L.QuickView }
                                },
                                {
                                    key: 'delete',
...
document.addEventListener('DOMContentLoaded', () => ContractsList.init());

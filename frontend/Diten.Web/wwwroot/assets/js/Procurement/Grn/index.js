/**
 * MOD-0142 Receiving (GRN) - DataTables Index Script
 * Golden Compact aligned (>8 form fields): create/details use full MVC pages; the GRN reverse action lives on Details.
 * Direct-gateway profile: browser reads/deletes through the Gateway (window.API.procurement + /api/grn).
 * GRN is a receipt document — the list shows the receipt + status only; NO inventory balance is shown as GRN's own.
 */
'use strict';

const GrnList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-grn');
    const apiUrl = window.API?.procurement;
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'Procurement', pageKey: 'Grn' };
    ...
    // GrnStatus enum ordinals — the service serializes enums numerically (no JsonStringEnumConverter).
    // Normalize both numeric and string forms to canonical names.
    const GRN_STATUS_NAMES = ['Draft', 'Posted', 'Reversed'];
    ...
    const statusName = (v) => {
        if (typeof v === 'number') return GRN_STATUS_NAMES[v] || String(v);
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[Grn SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const getStatusMap = () => ({
        ...
        'Posted': { title: L.StatusPosted || 'Posted', class: 'bg-label-success' },
        'Reversed': { title: L.StatusReversed || 'Reversed', class: 'bg-label-danger' }
    });
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/grn/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Grn/Details/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.grnId) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/grn/${encodeURIComponent(row.grnId)}`, {
                        method: 'DELETE',
                        credentials: 'include',
            ...
            }, { entityName: row.grnId, type: 'danger', confirmButtonText: L.Delete });
        }
    };
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        if (!apiUrl) { console.error('[Grn] window.API.procurement is required.'); return; }
        syncL10n();
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[Grn SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    { data: 'grnId', name: 'control' },
                    { data: 'grnId', name: 'checkbox' },
                    { data: 'grnId', name: 'grnId' },
                    { data: 'poId', name: 'poId' },
                    { data: 'receivedAt', name: 'receivedAt' },
                    { data: 'status', name: 'status' },
                    { data: 'grnId', name: 'action' }
                ],
                columnDefs: [
                            ...
                            return window.DitenDataTable.renderActions([
                                {
                                    ...
                                    icon: 'bx bx-show',
                                    attrs: { 'data-id': full.grnId, 'title': L.QuickView }
                                },
                                {
                                    key: 'delete',
...
document.addEventListener('DOMContentLoaded', () => GrnList.init());

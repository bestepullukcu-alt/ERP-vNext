/**
 * MOD-0141 Purchase Order - DataTables Index Script
 * Golden Compact aligned (>8 form fields): create/details use full MVC pages; approve lifecycle action on Details.
 * Direct-gateway profile: browser reads/deletes through the Gateway (window.API.procurement + /api/purchase-orders).
 * Money is rendered as the raw Decimal string (float YASAK — never parseFloat).
 */
'use strict';

const PurchaseOrdersList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-purchase-orders');
    const apiUrl = window.API?.procurement;
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'Procurement', pageKey: 'PurchaseOrders' };
    ...
    // Contract PoStatus enum ordinals — the service serializes enums numerically
    // (no JsonStringEnumConverter). Normalize both numeric and string forms to canonical names.
    const PO_STATUS_NAMES = ['Draft', 'Approved', 'PartiallyReceived', 'Received', 'Closed', 'Cancelled'];
    ...
    const statusName = (v) => {
        if (typeof v === 'number') return PO_STATUS_NAMES[v] || String(v);
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[PurchaseOrders SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const getStatusMap = () => ({
        ...
        'PartiallyReceived': { title: L.StatusPartiallyReceived || 'Partially Received', class: 'bg-label-warning' },
        'Received': { title: L.StatusReceived || 'Received', class: 'bg-label-primary' },
    ...
    // Money is a Decimal string from the contract — never coerce to float; escape and render as-is.
    const escapeHtml = (v) => String(v ?? '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
    const renderMoney = (v) => v == null || v === '' ? '-' : `<span class="fw-medium">${escapeHtml(v)}</span>`;
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/purchase-orders/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/PurchaseOrders/Details/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.poId) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/purchase-orders/${encodeURIComponent(row.poId)}`, {
                        method: 'DELETE',
                        credentials: 'include',
            ...
            }, { entityName: row.poId, type: 'danger', confirmButtonText: L.Delete });
        }
    };
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        if (!apiUrl) { console.error('[PurchaseOrders] window.API.procurement is required.'); return; }
        syncL10n();
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[PurchaseOrders SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            bulk: bulkOptions,
            ajax: {
                url: apiUrl + '/api/purchase-orders',
                type: 'GET',
            ...
            config: {
                ...
                columns: [
                    { data: 'poId', name: 'control' },
                    { data: 'poId', name: 'checkbox' },
                    { data: 'poId', name: 'poId' },
                    { data: 'supplierId', name: 'supplierId' },
                    { data: 'status', name: 'status' },
                    { data: 'totalAmount', name: 'totalAmount' },
                    { data: 'currency', name: 'currency' },
                    { data: 'poId', name: 'action' }
                ],
                columnDefs: [
                    ...
                    },
                    {
                        targets: 5,
                        className: 'text-end',
                        render: (data, type) => type === 'display' ? renderMoney(data) : (data || '')
                    },
                            ...
                            return window.DitenDataTable.renderActions([
                                {
                                    ...
                                    icon: 'bx bx-show',
                                    attrs: { 'data-id': full.poId, 'title': L.QuickView }
                                },
                                {
                                    key: 'delete',
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/PurchaseOrders/Create';
                    });
...
document.addEventListener('DOMContentLoaded', () => PurchaseOrdersList.init());

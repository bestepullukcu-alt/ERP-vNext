/**
 * MOD-0145 Sourcing (RFQ/RFP) - DataTables Index Script
 * Golden Compact aligned (>8 form fields): create/details use full MVC pages; RFx lifecycle actions on Details.
 * Direct-gateway profile: browser reads/deletes through the Gateway (window.API.procurement + /api/sourcing/events).
 */
'use strict';

const SourcingList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-sourcing');
    const apiUrl = window.API?.procurement;
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'Procurement', pageKey: 'Sourcing' };
    ...
    let appliedFilters = { status: [], type: [] };
    let L = window.L10n || {};

    // Contract RfxType / RfxStatus enum ordinals — the service serializes enums numerically
    // (no JsonStringEnumConverter). Normalize both numeric and string forms to canonical names.
    const RFX_TYPE_NAMES = ['RFQ', 'RFP', 'RFI'];
    const RFX_STATUS_NAMES = ['Draft', 'Published', 'Evaluating', 'Awarded', 'Cancelled', 'Closed'];
    ...
    const normalizeFilters = (filters) => {
        ...
        return {
            ...
            type: normalizeArray(source.type)
        };
    };
    const hasFilterValue = (v) => Array.isArray(v) ? normalizeArray(v).length > 0 : normalizeString(v).length > 0;

    const typeName = (v) => {
        if (typeof v === 'number') return RFX_TYPE_NAMES[v] || String(v);
        return normalizeString(v);
    };
    const statusName = (v) => {
        if (typeof v === 'number') return RFX_STATUS_NAMES[v] || String(v);
        return normalizeString(v);
    };
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[Sourcing SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const registerTableFilters = () => {
        ...
        $.fn.dataTable.ext.search.push((settings, _sd, dataIndex, rowData) => {
            ...
            return matchesEnumFilter(appliedFilters.status, statusName(row.status))
                && matchesEnumFilter(appliedFilters.type, typeName(row.type));
        });
    };
    ...
    const initSelect2Filters = () => {
        ...
        $('#filterStatus, #filterType').each(function () {
            const $s = $(this);
    ...
    const getAppliedFilterCount = () =>
        [appliedFilters.status, appliedFilters.type].filter(hasFilterValue).length;
    ...
    const getStatusMap = () => ({
        ...
        'Published': { title: L.StatusPublished || 'Published', class: 'bg-label-primary' },
        'Evaluating': { title: L.StatusEvaluating || 'Evaluating', class: 'bg-label-warning' },
        'Awarded': { title: L.StatusAwarded || 'Awarded', class: 'bg-label-success' },
        'Cancelled': { title: L.StatusCancelled || 'Cancelled', class: 'bg-label-danger' },
        'Closed': { title: L.StatusClosed || 'Closed', class: 'bg-label-dark' }
    });
    ...
    const renderType = (type) => {
        ...
        const title = getTypeMap()[name] || name || '';
        return title ? `<span class="badge bg-label-info">${title}</span>` : '';
    };
    const formatDate = (v) => {
        if (!v) return '';
        const d = new Date(v);
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/sourcing/events/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Sourcing/Details/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.rfxId) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/sourcing/events/${encodeURIComponent(row.rfxId)}`, {
                        method: 'DELETE',
                        credentials: 'include',
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        if (!apiUrl) { console.error('[Sourcing] window.API.procurement is required.'); return; }
        syncL10n();
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[Sourcing SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            bulk: bulkOptions,
            ajax: {
                url: apiUrl + '/api/sourcing/events',
                type: 'GET',
            ...
            config: {
                ...
                columns: [
                    { data: 'rfxId', name: 'control' },
                    { data: 'rfxId', name: 'checkbox' },
                    { data: 'rfxId', name: 'rfxId' },
                    { data: 'type', name: 'type' },
                    { data: 'title', name: 'title' },
                    { data: 'status', name: 'status' },
                    { data: 'closesAt', name: 'closesAt' },
                    { data: 'rfxId', name: 'action' }
                ],
                columnDefs: [
                    ...
                    {
                        targets: 3,
                        render: (data, type) => type === 'display' ? renderType(data) : typeName(data)
                    },
                    {
                        targets: 5,
                            ...
                            return window.DitenDataTable.renderActions([
                                {
                                    ...
                                    icon: 'bx bx-show',
                                    attrs: { 'data-id': full.rfxId, 'title': L.QuickView }
                                },
                                {
                                    key: 'delete',
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/Sourcing/Create';
                    });
...
document.addEventListener('DOMContentLoaded', () => SourcingList.init());

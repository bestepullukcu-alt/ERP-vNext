/**
 * DataWarehouseLakehouse & High-Potential Tracking - DataTables Index Script
...
const DataWarehouseLakehouseList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('data-warehouse-lakehouse-shell');
    const tableEl = doc.querySelector('.datatables-data-warehouse-lakehouse');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/data-warehouse-lakehouse`;
    const detailsBaseUrl = '/DataKnowledge/DataWarehouseLakehouse/Details';
    const createUrl = '/DataKnowledge/DataWarehouseLakehouse/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[DataWarehouseLakehouse] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[DataWarehouseLakehouse] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[DataWarehouseLakehouse] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[DataWarehouseLakehouse] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'dataWarehouseLakehouseReadinessState', name: 'dataWarehouseLakehouseReadinessState' },
                    { data: 'storageLayerCatalogBoundaryState', name: 'storageLayerCatalogBoundaryState' },
                    { data: 'vaultDependencyState', name: 'vaultDependencyState' },
                    { data: 'lineageControlBoundaryState', name: 'lineageControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => DataWarehouseLakehouseList.init());

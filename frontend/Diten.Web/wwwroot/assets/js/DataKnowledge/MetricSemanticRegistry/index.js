/**
 * MetricSemanticRegistry & High-Potential Tracking - DataTables Index Script
...
const MetricSemanticRegistryList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('metric-semantic-registry-shell');
    const tableEl = doc.querySelector('.datatables-metric-semantic-registry');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/metric-semantic-registry`;
    const detailsBaseUrl = '/DataKnowledge/MetricSemanticRegistry/Details';
    const createUrl = '/DataKnowledge/MetricSemanticRegistry/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[MetricSemanticRegistry] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[MetricSemanticRegistry] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[MetricSemanticRegistry] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[MetricSemanticRegistry] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'metricSemanticRegistryReadinessState', name: 'metricSemanticRegistryReadinessState' },
                    { data: 'metricIdentityCatalogBoundaryState', name: 'metricIdentityCatalogBoundaryState' },
                    { data: 'dataSourceRegistryDependencyState', name: 'dataSourceRegistryDependencyState' },
                    { data: 'semanticBindingControlBoundaryState', name: 'semanticBindingControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => MetricSemanticRegistryList.init());

/**
 * BaselineExperimentMeasurement & High-Potential Tracking - DataTables Index Script
...
const BaselineExperimentMeasurementList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('baseline-experiment-measurement-shell');
    const tableEl = doc.querySelector('.datatables-baseline-experiment-measurement');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/baseline-experiment-measurement`;
    const detailsBaseUrl = '/DataKnowledge/BaselineExperimentMeasurement/Details';
    const createUrl = '/DataKnowledge/BaselineExperimentMeasurement/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[BaselineExperimentMeasurement] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[BaselineExperimentMeasurement] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[BaselineExperimentMeasurement] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[BaselineExperimentMeasurement] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'baselineExperimentMeasurementReadinessState', name: 'baselineExperimentMeasurementReadinessState' },
                    { data: 'baselineCatalogBoundaryState', name: 'baselineCatalogBoundaryState' },
                    { data: 'metricSemanticRegistrySourceDependencyState', name: 'metricSemanticRegistrySourceDependencyState' },
                    { data: 'resultPublicationControlBoundaryState', name: 'resultPublicationControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => BaselineExperimentMeasurementList.init());

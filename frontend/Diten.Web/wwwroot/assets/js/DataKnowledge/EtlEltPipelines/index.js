/**
 * EtlEltPipelines & High-Potential Tracking - DataTables Index Script
...
const EtlEltPipelinesList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('etl-elt-pipelines-shell');
    const tableEl = doc.querySelector('.datatables-etl-elt-pipelines');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/etl-elt-pipelines`;
    const detailsBaseUrl = '/DataKnowledge/EtlEltPipelines/Details';
    const createUrl = '/DataKnowledge/EtlEltPipelines/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[EtlEltPipelines] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[EtlEltPipelines] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[EtlEltPipelines] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[EtlEltPipelines] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'etlEltPipelinesReadinessState', name: 'etlEltPipelinesReadinessState' },
                    { data: 'pipelineDefinitionCatalogBoundaryState', name: 'pipelineDefinitionCatalogBoundaryState' },
                    { data: 'lakehouseSourceDependencyState', name: 'lakehouseSourceDependencyState' },
                    { data: 'loadControlBoundaryState', name: 'loadControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => EtlEltPipelinesList.init());

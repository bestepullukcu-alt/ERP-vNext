/**
 * MetricDefinitionsOwnership & High-Potential Tracking - DataTables Index Script
...
const MetricDefinitionsOwnershipList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('metric-definitions-ownership-shell');
    const tableEl = doc.querySelector('.datatables-metric-definitions-ownership');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/metric-definitions-ownership`;
    const detailsBaseUrl = '/DataKnowledge/MetricDefinitionsOwnership/Details';
    const createUrl = '/DataKnowledge/MetricDefinitionsOwnership/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[MetricDefinitionsOwnership] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[MetricDefinitionsOwnership] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[MetricDefinitionsOwnership] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[MetricDefinitionsOwnership] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'metricDefinitionsOwnershipReadinessState', name: 'metricDefinitionsOwnershipReadinessState' },
                    { data: 'definitionCatalogBoundaryState', name: 'definitionCatalogBoundaryState' },
                    { data: 'metricSemanticRegistrySourceDependencyState', name: 'metricSemanticRegistrySourceDependencyState' },
                    { data: 'approvalControlBoundaryState', name: 'approvalControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => MetricDefinitionsOwnershipList.init());

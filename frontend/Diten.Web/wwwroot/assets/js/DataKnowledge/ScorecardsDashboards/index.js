/**
 * ScorecardsDashboards & High-Potential Tracking - DataTables Index Script
...
const ScorecardsDashboardsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('scorecards-dashboards-shell');
    const tableEl = doc.querySelector('.datatables-scorecards-dashboards');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/scorecards-dashboards`;
    const detailsBaseUrl = '/DataKnowledge/ScorecardsDashboards/Details';
    const createUrl = '/DataKnowledge/ScorecardsDashboards/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[ScorecardsDashboards] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[ScorecardsDashboards] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[ScorecardsDashboards] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[ScorecardsDashboards] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'scorecardsDashboardsReadinessState', name: 'scorecardsDashboardsReadinessState' },
                    { data: 'scorecardCatalogBoundaryState', name: 'scorecardCatalogBoundaryState' },
                    { data: 'metricSemanticRegistrySourceDependencyState', name: 'metricSemanticRegistrySourceDependencyState' },
                    { data: 'publicationControlBoundaryState', name: 'publicationControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => ScorecardsDashboardsList.init());

/**
 * KpiCatalog & High-Potential Tracking - DataTables Index Script
...
const KpiCatalogList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('kpi-catalog-shell');
    const tableEl = doc.querySelector('.datatables-kpi-catalog');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/kpi-catalog`;
    const detailsBaseUrl = '/DataKnowledge/KpiCatalog/Details';
    const createUrl = '/DataKnowledge/KpiCatalog/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[KpiCatalog] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[KpiCatalog] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[KpiCatalog] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[KpiCatalog] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'kpiCatalogReadinessState', name: 'kpiCatalogReadinessState' },
                    { data: 'kpiIdentityCatalogBoundaryState', name: 'kpiIdentityCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => KpiCatalogList.init());

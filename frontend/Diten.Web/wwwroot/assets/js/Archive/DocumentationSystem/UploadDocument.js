...
function fetchFolders(domain, userId, selectedFolderId, folderSelect) {
    fetch(`${window.ApiBaseUrl}/services/PvDocumentManagement/Folder/GetFolderByUserId?id=${userId}`)
        .then(res => res.json())
        .then(data => {
...
function setupFormSubmit(domain, userId, { form, fileInput, filePreview, dropDefault, folderSelect, tagContainer }, selectedFolderId) {
    form.addEventListener("submit", async function (e) {
        ...
        try {
            const response = await fetch(`${window.ApiBaseUrl}/services/PvDocumentManagement/Document/CreateDocument`, {
                method: "POST",
                body: formData
            });

...
document.addEventListener('DOMContentLoaded', function (e) {
    // Sayfa hazır olunca çalışacak kodlar buraya yazılır
    ...
    const dt_country_table = document.querySelector('.datatables-country');
    //    select2 = $('.select2'),
    ...
    if (dt_country_table) {
        var dt_country = new DataTable(dt_country_table, {
            ajax: {
                url: `${window.ApiBaseUrl}/services/PvTenant/Tenant/GetCountriesByTenantId`,
                method: 'GET',
                dataSrc: 'data'
            },
            columns: [
            ...
            columnDefs: [
                ...
                {
                    targets: 3, // 4. sütun (Action) için
                    render: function (data, type, full, meta) {
                        // Iconları ekliyoruz
                        return `<a href="javascript:;" class="btn btn-icon activate-disabled-record" data-id="${full.id}" data-status="${full.isActiveStr}">
...
function initActiveDisableCountry() {
    $(document).on('click', '.activate-disabled-record', function () {
        ...
        fetch(`${window.ApiBaseUrl}/services/PvTenant/Tenant/activate-disable`, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.isSuccessful) {
                    const table = $('.datatables-country').DataTable();

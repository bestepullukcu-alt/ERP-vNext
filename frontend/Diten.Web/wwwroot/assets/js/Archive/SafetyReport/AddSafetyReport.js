...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const statusUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSafetyReportStatus`;
    fetchDropdowns(statusUrl, "ddl-safety-report-status");

    const reportTypeUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSafetyReportTypes`;
    fetchDropdowns(reportTypeUrl, "ddl-report-type");

    const globalSkuUrl = `${window.ApiBaseUrl}/services/PvOrganization/GlobalSku/GetGlobalSkus`;
    fetchGlobalSku(globalSkuUrl, "ddl-global-sku");

    const subjectRaceUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSubjectRace`;
    fetchDropdowns(subjectRaceUrl, "ddl-study-subject-race");

    const subjectInformationSourceUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSubjectInformationSource`;
    fetchDropdowns(subjectInformationSourceUrl, "ddl-source-information");

    const submissionStatusUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSafetySubmissionStatus`;
    fetchDropdowns(submissionStatusUrl, "ddl-safety-submission-status");

    const safetyDataTypeUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSafetyDataType`;
    fetchDropdowns(safetyDataTypeUrl, "ddl-safety-data-type");

    const causalityAssessmentUrl = `${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetCausalityAssessment`;
    ...
    const organizationUrl = `${window.ApiBaseUrl}/services/PvOrganization/OrganizationControlller/GetOrganizationsByTenantId`;
...
const disableStatus = urlParams.get('disabledStatus') ?? 0;
    ...
    if (id) {
        ...
        fetch(`${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/UpdateSafetyReport`, {
            method: 'POST',
            headers: {
        ...
        fetch(`${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/CreateSafetyReport`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function UploadDocument(SafetyReportId) {
    ...
    fetch(`${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/CreateSafetyReportDocuments`, {
        method: 'POST',
        body: formUploadData
    })
        .then(response => response.json())
...
async function loadSafetyReport() {
    ...
    const id = urlParams.get('id');
    const disableStatus = urlParams.get('disabledStatus') ?? 0;
    const response = await fetch(`${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/GetSafetyReportById/${id}`); // API adresin 
...
function UpdateUploadDocument(SafetyId) {
    ...
    fetch(`${window.ApiBaseUrl}/services/PvOrganization/SafetyReport/UpdateSafetyDocuments`, {
        method: 'POST',
        body: formUploadData
    })
        .then(response => response.json())

/**
 * Employee Onboarding - DataTables Index Script
...
const EmployeeOnboardingList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('employee-onboarding-shell');
    const tableEl = doc.querySelector('.datatables-employee-onboarding');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/employee-onboarding`;
    const detailsBaseUrl = '/HumanCapital/EmployeeOnboarding/Details';
    const createUrl = '/HumanCapital/EmployeeOnboarding/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[EmployeeOnboarding] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[EmployeeOnboarding] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[EmployeeOnboarding] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[EmployeeOnboarding] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'onboardingReadinessState', name: 'onboardingReadinessState' },
                    { data: 'lifecycleBoundaryState', name: 'lifecycleBoundaryState' },
                    { data: 'checklistBoundaryState', name: 'checklistBoundaryState' },
                    { data: 'managerActionBoundaryState', name: 'managerActionBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => EmployeeOnboardingList.init());

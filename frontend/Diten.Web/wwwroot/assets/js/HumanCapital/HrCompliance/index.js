/**
 * HrCompliance & High-Potential Tracking - DataTables Index Script
...
const HrComplianceList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('hr-compliance-shell');
    const tableEl = doc.querySelector('.datatables-hr-compliance');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/hr-compliance`;
    const detailsBaseUrl = '/HumanCapital/HrCompliance/Details';
    const createUrl = '/HumanCapital/HrCompliance/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HrCompliance] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HrCompliance] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HrCompliance] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HrCompliance] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'hrComplianceReadinessState', name: 'hrComplianceReadinessState' },
                    { data: 'obligationCatalogBoundaryState', name: 'obligationCatalogBoundaryState' },
                    { data: 'regulatorySourceDependencyState', name: 'regulatorySourceDependencyState' },
                    { data: 'filingScheduleBoundaryState', name: 'filingScheduleBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HrComplianceList.init());

/**
 * SelfService & High-Potential Tracking - DataTables Index Script
...
const SelfServiceList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('self-service-shell');
    const tableEl = doc.querySelector('.datatables-self-service');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/self-service`;
    const detailsBaseUrl = '/HumanCapital/SelfService/Details';
    const createUrl = '/HumanCapital/SelfService/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[SelfService] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[SelfService] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[SelfService] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[SelfService] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'selfServiceReadinessState', name: 'selfServiceReadinessState' },
                    { data: 'requestIntakeBoundaryState', name: 'requestIntakeBoundaryState' },
                    { data: 'identityDirectoryDependencyState', name: 'identityDirectoryDependencyState' },
                    { data: 'profileSelfUpdateBoundaryState', name: 'profileSelfUpdateBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => SelfServiceList.init());

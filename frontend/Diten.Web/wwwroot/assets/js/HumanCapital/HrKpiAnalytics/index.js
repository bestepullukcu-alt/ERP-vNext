/**
 * HrKpiAnalytics & High-Potential Tracking - DataTables Index Script
...
const HrKpiAnalyticsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('hr-kpi-analytics-shell');
    const tableEl = doc.querySelector('.datatables-hr-kpi-analytics');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/hr-kpi-analytics`;
    const detailsBaseUrl = '/HumanCapital/HrKpiAnalytics/Details';
    const createUrl = '/HumanCapital/HrKpiAnalytics/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HrKpiAnalytics] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HrKpiAnalytics] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HrKpiAnalytics] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HrKpiAnalytics] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'hrKpiAnalyticsReadinessState', name: 'hrKpiAnalyticsReadinessState' },
                    { data: 'kpiCatalogBoundaryState', name: 'kpiCatalogBoundaryState' },
                    { data: 'analyticsPlatformDependencyState', name: 'analyticsPlatformDependencyState' },
                    { data: 'analyticsQueryBoundaryState', name: 'analyticsQueryBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HrKpiAnalyticsList.init());

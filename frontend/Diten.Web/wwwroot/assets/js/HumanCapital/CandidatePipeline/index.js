/**
 * Candidate Pipeline - DataTables Index Script
...
const CandidatePipelineList = (function () {
    ...
    const shell = doc.getElementById('candidate-pipeline-shell');
    ...
    const detailsBaseUrl = '/HumanCapital/CandidatePipeline/Details';
    const createUrl = '/HumanCapital/CandidatePipeline/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[CandidatePipeline] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[CandidatePipeline] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[CandidatePipeline] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[CandidatePipeline] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'pipelineReadinessState', name: 'pipelineReadinessState' },
                    { data: 'pipelineStageGovernanceState', name: 'pipelineStageGovernanceState' },
                    { data: 'interviewSchedulingReadinessState', name: 'interviewSchedulingReadinessState' },
                    { data: 'interviewerAssignmentReadinessState', name: 'interviewerAssignmentReadinessState' },
                    { data: 'evaluationGovernanceState', name: 'evaluationGovernanceState' },

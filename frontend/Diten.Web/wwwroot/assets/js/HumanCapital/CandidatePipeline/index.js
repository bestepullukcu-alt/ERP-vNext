...
const CandidatePipelineList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const root = doc.getElementById('candidate-pipeline-shell');
    const tableEl = doc.querySelector('.datatables-candidate-pipeline');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/candidate-pipeline`;
    const readinessStateNames = ['Draft', 'Ready', 'Deferred', 'Blocked', 'NotRequired', 'Archived'];
    ...
    const updateSummary = (items) => {
        ...
        const counts = rows.reduce((acc, item) => {
            const state = normalizeState(item?.pipelineReadinessState ?? item?.PipelineReadinessState);
            acc.total += 1;
        ...
        Object.entries(counts).forEach(([key, value]) => {
            const node = doc.querySelector(`[data-candidate-pipeline-summary="${key}"]`);
    ...
    const updateActionVisibility = () => {
        const evaluateButton = doc.querySelector('[data-candidate-pipeline-action="evaluate"]');
        const manageButton = doc.querySelector('[data-candidate-pipeline-action="manage"]');
    ...
    const setAudit = (item) => {
        const block = doc.getElementById('candidatePipelineAuditBlock');
        if (!block) return;
    ...
    const loadAudit = async (id) => {
        ...
        try {
            ...
            console.error('[CandidatePipeline] Audit metadata load failed.', error);
            setAudit(null);
        }
    };

    const setDetails = (item) => {
        const empty = doc.getElementById('candidatePipelineDetailsEmpty');
        const list = doc.getElementById('candidatePipelineDetailsList');
        if (!empty || !list) return;

        const values = {
            code: item?.code ?? item?.Code,
            displayName: item?.displayName ?? item?.DisplayName,
            pipelineReadinessState: normalizeState(item?.pipelineReadinessState ?? item?.PipelineReadinessState),
            pipelineStageGovernanceState: normalizeState(item?.pipelineStageGovernanceState ?? item?.PipelineStageGovernanceState),
            interviewSchedulingReadinessState: normalizeState(item?.interviewSchedulingReadinessState ?? item?.InterviewSchedulingReadinessState),
            interviewerAssignmentReadinessState: normalizeState(item?.interviewerAssignmentReadinessState ?? item?.InterviewerAssignmentReadinessState),
            evaluationGovernanceState: normalizeState(item?.evaluationGovernanceState ?? item?.EvaluationGovernanceState),
            candidateCommunicationBoundaryState: normalizeState(item?.candidateCommunicationBoundaryState ?? item?.CandidateCommunicationBoundaryState),
            ...
            retentionPolicyState: normalizeState(item?.retentionPolicyState ?? item?.RetentionPolicyState),
            evidencePolicyState: normalizeState(item?.evidencePolicyState ?? item?.EvidencePolicyState),
            calendarDependencyState: normalizeState(item?.calendarDependencyState ?? item?.CalendarDependencyState),
            notificationDependencyState: normalizeState(item?.notificationDependencyState ?? item?.NotificationDependencyState),
            documentDependencyState: normalizeState(item?.documentDependencyState ?? item?.DocumentDependencyState),
            automatedDecisionBoundaryState: normalizeState(item?.automatedDecisionBoundaryState ?? item?.AutomatedDecisionBoundaryState),
            ...
            pipelineReadinessVersion: item?.pipelineReadinessVersion ?? item?.PipelineReadinessVersion,
            lastEvaluatedAt: formatDateTime(item?.lastEvaluatedAt ?? item?.LastEvaluatedAt),
    ...
    const showDetails = async (id) => {
        try {
            ...
            setDetails(item);
            await loadAudit(id);
            const panel = doc.getElementById('offcanvasCandidatePipelineDetails');
            if (panel) bootstrap.Offcanvas.getOrCreateInstance(panel).show();
        } catch (error) {
            console.error('[CandidatePipeline] Detail load failed.', error);
        ...
        dt = new DataTable(tableEl, {
            ajax: async (_data, callback) => {
                try {
                    ...
                    console.error('[CandidatePipeline] List load failed.', error);
                    updateSummary([]);
                    callback({ data: [] });
                    window.notyf?.error?.(L.ErrorOccurred || 'An error occurred.');
                } finally {
                    doc.getElementById('candidate-pipeline-skeleton-loader')?.classList.add('d-none');
                }
            },
            columns: [
                ...
                { data: 'pipelineReadinessState', render: (data, _type, item) => renderBadge(data ?? item?.PipelineReadinessState) },
                { data: 'pipelineStageGovernanceState', render: (data, _type, item) => renderBadge(data ?? item?.PipelineStageGovernanceState) },
                { data: 'interviewSchedulingReadinessState', render: (data, _type, item) => renderBadge(data ?? item?.InterviewSchedulingReadinessState) },
                { data: 'interviewerAssignmentReadinessState', render: (data, _type, item) => renderBadge(data ?? item?.InterviewerAssignmentReadinessState) },
                { data: 'evaluationGovernanceState', render: (data, _type, item) => renderBadge(data ?? item?.EvaluationGovernanceState) },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', CandidatePipelineList.init);

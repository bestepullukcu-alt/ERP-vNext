/**
 * HeadcountBudget & High-Potential Tracking - DataTables Index Script
...
const HeadcountBudgetList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('headcount-budget-shell');
    const tableEl = doc.querySelector('.datatables-headcount-budget');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/headcount-budget`;
    const detailsBaseUrl = '/HumanCapital/HeadcountBudget/Details';
    const createUrl = '/HumanCapital/HeadcountBudget/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HeadcountBudget] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HeadcountBudget] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HeadcountBudget] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HeadcountBudget] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'headcountBudgetReadinessState', name: 'headcountBudgetReadinessState' },
                    { data: 'headcountRequisitionBoundaryState', name: 'headcountRequisitionBoundaryState' },
                    { data: 'organizationStructureDependencyState', name: 'organizationStructureDependencyState' },
                    { data: 'budgetApprovalBoundaryState', name: 'budgetApprovalBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HeadcountBudgetList.init());

/**
 * Offer Management - DataTables Index Script
...
const OfferManagementList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('offer-management-shell');
    const tableEl = doc.querySelector('.datatables-offer-management');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/offer-management`;
    const detailsBaseUrl = '/HumanCapital/OfferManagement/Details';
    const createUrl = '/HumanCapital/OfferManagement/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[OfferManagement] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[OfferManagement] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[OfferManagement] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[OfferManagement] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'offerReadinessState', name: 'offerReadinessState' },
                    { data: 'offerWorkflowBoundaryState', name: 'offerWorkflowBoundaryState' },
                    { data: 'approvalWorkflowBoundaryState', name: 'approvalWorkflowBoundaryState' },
                    { data: 'candidateAcceptanceBoundaryState', name: 'candidateAcceptanceBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => OfferManagementList.init());

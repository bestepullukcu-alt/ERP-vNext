/**
 * Employment Changes - DataTables Index Script
...
const EmploymentChangesList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('employment-changes-shell');
    const tableEl = doc.querySelector('.datatables-employment-changes');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/employment-changes`;
    const detailsBaseUrl = '/HumanCapital/EmploymentChanges/Details';
    const createUrl = '/HumanCapital/EmploymentChanges/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[EmploymentChanges] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[EmploymentChanges] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[EmploymentChanges] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[EmploymentChanges] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'employmentChangeReadinessState', name: 'employmentChangeReadinessState' },
                    { data: 'changeLifecycleBoundaryState', name: 'changeLifecycleBoundaryState' },
                    { data: 'approvalBoundaryState', name: 'approvalBoundaryState' },
                    { data: 'positionAssignmentBoundaryState', name: 'positionAssignmentBoundaryState' },
                ...
                columnDefs: [
                    ...
                    { targets: 8, render: (data) => formatDateTime(data) },
                    {
                        targets: -1,
                        title: L.Actions,
                        orderable: false,
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => EmploymentChangesList.init());

/**
 * HrDocumentation & High-Potential Tracking - DataTables Index Script
...
const HrDocumentationList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('hr-documentation-shell');
    const tableEl = doc.querySelector('.datatables-hr-documentation');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/hr-documentation`;
    const detailsBaseUrl = '/HumanCapital/HrDocumentation/Details';
    const createUrl = '/HumanCapital/HrDocumentation/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HrDocumentation] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HrDocumentation] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HrDocumentation] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HrDocumentation] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'hrDocumentationReadinessState', name: 'hrDocumentationReadinessState' },
                    { data: 'documentWorkspaceBoundaryState', name: 'documentWorkspaceBoundaryState' },
                    { data: 'documentRepositoryDependencyState', name: 'documentRepositoryDependencyState' },
                    { data: 'legalHoldBoundaryState', name: 'legalHoldBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HrDocumentationList.init());

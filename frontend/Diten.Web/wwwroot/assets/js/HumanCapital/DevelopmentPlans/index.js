/**
 * Development Plans - DataTables Index Script
...
const DevelopmentPlansList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('development-plans-shell');
    const tableEl = doc.querySelector('.datatables-development-plans');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/development-plans`;
    const detailsBaseUrl = '/HumanCapital/DevelopmentPlans/Details';
    const createUrl = '/HumanCapital/DevelopmentPlans/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[DevelopmentPlans] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[DevelopmentPlans] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[DevelopmentPlans] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[DevelopmentPlans] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'developmentPlanReadinessState', name: 'developmentPlanReadinessState' },
                    { data: 'developmentPlanWorkflowBoundaryState', name: 'developmentPlanWorkflowBoundaryState' },
                    { data: 'learningAssignmentBoundaryState', name: 'learningAssignmentBoundaryState' },
                    { data: 'skillGapScoringBoundaryState', name: 'skillGapScoringBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => DevelopmentPlansList.init());

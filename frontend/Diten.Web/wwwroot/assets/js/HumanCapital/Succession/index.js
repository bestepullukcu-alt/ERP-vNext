/**
 * Succession & High-Potential Tracking - DataTables Index Script
...
const SuccessionList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('succession-shell');
    const tableEl = doc.querySelector('.datatables-succession');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/succession`;
    const detailsBaseUrl = '/HumanCapital/Succession/Details';
    const createUrl = '/HumanCapital/Succession/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[Succession] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[Succession] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[Succession] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[Succession] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'successionReadinessState', name: 'successionReadinessState' },
                    { data: 'successionPoolBoundaryState', name: 'successionPoolBoundaryState' },
                    { data: 'talentProfileDependencyState', name: 'talentProfileDependencyState' },
                    { data: 'readinessAssessmentBoundaryState', name: 'readinessAssessmentBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => SuccessionList.init());

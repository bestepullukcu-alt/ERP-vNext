/**
 * Applicant Intake - DataTables Index Script
...
const ApplicantIntakeList = (function () {
    ...
    const shell = doc.getElementById('applicant-intake-shell');
    ...
    const detailsBaseUrl = '/HumanCapital/ApplicantIntake/Details';
    const createUrl = '/HumanCapital/ApplicantIntake/Create';
    ...
    // Ordinal order follows ApplicantIntakeReadinessState:
    ...
    const stateClasses = ['bg-label-warning', 'bg-label-info', 'bg-label-success', 'bg-label-danger', 'bg-label-secondary', 'bg-label-secondary'];
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[ApplicantIntake] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[ApplicantIntake] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[ApplicantIntake] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[ApplicantIntake] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'intakeState', name: 'intakeState' },
                    { data: 'sourceChannelState', name: 'sourceChannelState' },
                    { data: 'consentPreconditionState', name: 'consentPreconditionState' },
                    { data: 'dataMinimizationState', name: 'dataMinimizationState' },
                    { data: 'duplicateHandlingState', name: 'duplicateHandlingState' },

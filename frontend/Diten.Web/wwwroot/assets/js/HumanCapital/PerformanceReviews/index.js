/**
 * Performance Reviews - DataTables Index Script
...
const PerformanceReviewsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('performance-reviews-shell');
    const tableEl = doc.querySelector('.datatables-performance-reviews');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/performance-reviews`;
    const detailsBaseUrl = '/HumanCapital/PerformanceReviews/Details';
    const createUrl = '/HumanCapital/PerformanceReviews/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[PerformanceReviews] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[PerformanceReviews] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[PerformanceReviews] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[PerformanceReviews] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'performanceReviewReadinessState', name: 'performanceReviewReadinessState' },
                    { data: 'reviewCycleBoundaryState', name: 'reviewCycleBoundaryState' },
                    { data: 'scoringBoundaryState', name: 'scoringBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => PerformanceReviewsList.init());

/**
 * Competency & Skills - DataTables Index Script
...
const CompetencySkillsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('competency-skills-shell');
    const tableEl = doc.querySelector('.datatables-competency-skills');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/competency-skills`;
    const detailsBaseUrl = '/HumanCapital/CompetencySkills/Details';
    const createUrl = '/HumanCapital/CompetencySkills/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[CompetencySkills] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[CompetencySkills] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[CompetencySkills] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[CompetencySkills] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'competencySkillsReadinessState', name: 'competencySkillsReadinessState' },
                    { data: 'assessmentWorkflowBoundaryState', name: 'assessmentWorkflowBoundaryState' },
                    { data: 'skillTaxonomyDependencyState', name: 'skillTaxonomyDependencyState' },
                    { data: 'skillScoringBoundaryState', name: 'skillScoringBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => CompetencySkillsList.init());

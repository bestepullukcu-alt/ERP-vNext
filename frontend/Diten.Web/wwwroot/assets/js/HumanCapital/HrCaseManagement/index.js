/**
 * HrCaseManagement & High-Potential Tracking - DataTables Index Script
...
const HrCaseManagementList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('hr-case-management-shell');
    const tableEl = doc.querySelector('.datatables-hr-case-management');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/hr-case-management`;
    const detailsBaseUrl = '/HumanCapital/HrCaseManagement/Details';
    const createUrl = '/HumanCapital/HrCaseManagement/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HrCaseManagement] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HrCaseManagement] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HrCaseManagement] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HrCaseManagement] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'hrCaseManagementReadinessState', name: 'hrCaseManagementReadinessState' },
                    { data: 'caseIntakeBoundaryState', name: 'caseIntakeBoundaryState' },
                    { data: 'employeeRecordDependencyState', name: 'employeeRecordDependencyState' },
                    { data: 'disciplinaryActionBoundaryState', name: 'disciplinaryActionBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HrCaseManagementList.init());

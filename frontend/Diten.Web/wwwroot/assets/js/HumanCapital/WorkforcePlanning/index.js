/**
 * WorkforcePlanning & High-Potential Tracking - DataTables Index Script
...
const WorkforcePlanningList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('workforce-planning-shell');
    const tableEl = doc.querySelector('.datatables-workforce-planning');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/workforce-planning`;
    const detailsBaseUrl = '/HumanCapital/WorkforcePlanning/Details';
    const createUrl = '/HumanCapital/WorkforcePlanning/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[WorkforcePlanning] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[WorkforcePlanning] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[WorkforcePlanning] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[WorkforcePlanning] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'workforcePlanningReadinessState', name: 'workforcePlanningReadinessState' },
                    { data: 'headcountPlanBoundaryState', name: 'headcountPlanBoundaryState' },
                    { data: 'organizationStructureDependencyState', name: 'organizationStructureDependencyState' },
                    { data: 'gapAnalysisBoundaryState', name: 'gapAnalysisBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => WorkforcePlanningList.init());

/**
 * Learning / Training - DataTables Index Script
...
const LearningTrainingList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('learning-training-shell');
    const tableEl = doc.querySelector('.datatables-learning-training');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/learning-training`;
    const detailsBaseUrl = '/HumanCapital/LearningTraining/Details';
    const createUrl = '/HumanCapital/LearningTraining/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[LearningTraining] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[LearningTraining] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[LearningTraining] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[LearningTraining] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'learningTrainingReadinessState', name: 'learningTrainingReadinessState' },
                    { data: 'courseCatalogBoundaryState', name: 'courseCatalogBoundaryState' },
                    { data: 'skillTaxonomyDependencyState', name: 'skillTaxonomyDependencyState' },
                    { data: 'assessmentScoringBoundaryState', name: 'assessmentScoringBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => LearningTrainingList.init());

/**
 * CompensationBenefits & High-Potential Tracking - DataTables Index Script
...
const CompensationBenefitsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('compensation-benefits-shell');
    const tableEl = doc.querySelector('.datatables-compensation-benefits');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/compensation-benefits`;
    const detailsBaseUrl = '/HumanCapital/CompensationBenefits/Details';
    const createUrl = '/HumanCapital/CompensationBenefits/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[CompensationBenefits] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[CompensationBenefits] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[CompensationBenefits] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[CompensationBenefits] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'compensationBenefitsReadinessState', name: 'compensationBenefitsReadinessState' },
                    { data: 'compensationPlanBoundaryState', name: 'compensationPlanBoundaryState' },
                    { data: 'compensationSourceDependencyState', name: 'compensationSourceDependencyState' },
                    { data: 'benefitEnrollmentBoundaryState', name: 'benefitEnrollmentBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => CompensationBenefitsList.init());

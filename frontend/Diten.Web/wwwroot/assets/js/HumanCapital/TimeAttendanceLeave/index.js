/**
 * TimeAttendanceLeave & High-Potential Tracking - DataTables Index Script
...
const TimeAttendanceLeaveList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('time-attendance-leave-shell');
    const tableEl = doc.querySelector('.datatables-time-attendance-leave');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/time-attendance-leave`;
    const detailsBaseUrl = '/HumanCapital/TimeAttendanceLeave/Details';
    const createUrl = '/HumanCapital/TimeAttendanceLeave/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[TimeAttendanceLeave] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[TimeAttendanceLeave] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[TimeAttendanceLeave] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[TimeAttendanceLeave] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'timeAttendanceLeaveReadinessState', name: 'timeAttendanceLeaveReadinessState' },
                    { data: 'timesheetIntakeBoundaryState', name: 'timesheetIntakeBoundaryState' },
                    { data: 'timeAttendanceSourceDependencyState', name: 'timeAttendanceSourceDependencyState' },
                    { data: 'leaveBalanceBoundaryState', name: 'leaveBalanceBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => TimeAttendanceLeaveList.init());

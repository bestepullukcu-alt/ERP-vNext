(function (window, document) {
    ...
    // Golden Compact form init — Select2 on .select2 (mirrors GoldenReferenceCompact/form.js).
    const jq = window.jQuery;
    if (jq && jq.fn && jq.fn.select2) {
        jq('#consentForm .select2').each(function () {
            const $this = jq(this);
    ...
    if (jq && jq.fn && jq.fn.select2 && subjectSel && subjectSel.tagName === 'SELECT' && typeSel) {
        ...
        const buildPicker = () => {
            ...
            const opts = {
                ...
                tags: true, // a raw GUID is always accepted (fallback for hcp/hco/account-contact-link or no match)
    ...
    // Zero references is valid: nothing is rendered into #externalReferencesHost, so the form posts an empty collection
    // and the record saves fine. Rows are held in `refs` and mirrored into hidden inputs for MVC model binding.
    (function externalReferences() {
        ...
        // Actions column rendered exactly like the Golden Slim datatable (DitenDataTable.renderActions).
    ...
    // Withdrawal reason hint when status = withdrawn (backend stays authoritative on the requirement).
    const statusSelect = document.getElementById('consentStatusSelect');
    const withdrawal = document.getElementById('withdrawalReason');
    const syncWithdrawal = () => { if (withdrawal) withdrawal.classList.toggle('is-invalid', statusSelect?.value === 'withdrawn' && !withdrawal.value.trim()); };
    statusSelect?.addEventListener('change', syncWithdrawal);
    withdrawal?.addEventListener('input', syncWithdrawal);

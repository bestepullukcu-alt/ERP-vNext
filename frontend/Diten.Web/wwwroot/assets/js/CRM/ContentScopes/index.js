/**
 * SCMM-14-UI (CAND-CAP-0011) ContentScope console — DataTables Index (Golden aligned, mirrors the Claims console).
 * Reusable, versioned context/scope; no delete surface — closing a scope is Archive.
 */
(function (window, document) {
    'use strict';
    const tableEl = document.getElementById('dt-content-scopes');
    if (!tableEl) return;

    const endpoint = '/CRM/ContentScopes/api';
    const filterCollapseId = 'inlineFilterCollapse';
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'CRM', pageKey: 'ContentScopes' };
    const saveViewColumnIndexes = [1, 2, 3, 4, 5, 6, 7, 8];
    const totalColumnCount = 10;
    const baseOrder = [[8, 'desc']];
    const SCOPE_STATUSES = ['draft', 'active', 'archived'];

    let L = window.ScopeL10n || window.L10n || {};
    let dt = null;
    ...
    const statusTone = s => ({ draft: 'primary', active: 'success', archived: 'secondary' }[s] || 'primary');
    ...
    const loadFilterOptions = () => {
        fillSelect('filterStatus', SCOPE_STATUSES.map(v => ({ value: v, text: statusLabel(v) })), false);
        initSelect2();
    };
    ...
    const loadDefaultView = async () => {
        ...
        } catch (e) { if (!e?.authHandled) console.error('[Scope SaveView] load failed', e); }
    };
    ...
    const rowById = {};
    const actions = row => {
        const id = esc(row.contentScopeId);
        rowById[row.contentScopeId] = row;
        const items = [{ className: 'js-quick-view me-1', icon: 'bx bx-show', attrs: { 'data-id': id, title: L.QuickView } }];
        items.push({ className: 'js-edit-scope', icon: 'bx bx-edit', text: L.EditScope, attrs: { 'data-id': id } });
        if (!row.isArchived) {
            items.push({ className: 'js-archive-scope text-warning', icon: 'bx bx-archive-in', text: L.ArchiveScope, attrs: { 'data-id': id, 'data-name': esc(row.scopeName) } });
        }
        return window.DitenDataTable?.renderActions ? window.DitenDataTable.renderActions(items) : '';
    };

    const openScopePreview = id => {
        const row = rowById[id];
        if (!row) return;
        const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.textContent = val ?? '—'; };
        set('scopePreviewTitle', row.scopeName || row.scopeCode || '—');
        set('scopePreviewSubtitle', row.scopeCode || '—');
        set('pvStatus', statusLabel(row.status));
        set('pvVersion', row.scopeVersion || '—');
        set('pvChannel', row.channel || '—');
        set('pvLanguage', row.languageCode || '—');
        set('pvProducts', (row.productRefs || []).join(', ') || '—');
        set('pvMarkets', (row.marketRefs || []).join(', ') || '—');
        set('pvAudiences', (row.audienceRefs || []).join(', ') || '—');
        const editLink = document.getElementById('scopePreviewEdit');
        if (editLink) editLink.setAttribute('href', `/CRM/ContentScopes/Edit/${id}`);
    ...
    const buildConfig = () => ({
        ...
        order: baseOrder,
        columns: [
            { data: null, defaultContent: '' }, { data: 'scopeCode' }, { data: 'scopeName' }, { data: 'scopeVersion' },
            { data: 'status' }, { data: 'channel' }, { data: 'languageCode' }, { data: 'isArchived' },
        ...
        columnDefs: [
            ...
            { targets: [5, 6], render: v => esc(v || '—') },
            { targets: 7, render: v => badge(v ? L.Yes : L.No, v ? 'warning' : 'success') },
            { targets: 8, render: v => date(v) },
        ...
        buttons: window.DtDefaults.exportButtons(L.CreateScope, { href: '/CRM/ContentScopes/Create' }, {
        ...
        initComplete: function () {
            ...
            if (!addNewBound) { nodeContainer(api).querySelector('.add-new')?.addEventListener('click', e => { e.preventDefault(); window.location.href = '/CRM/ContentScopes/Create'; }); addNewBound = true; }
    ...
    const loadRows = async () =>
        (await envelope(await fetch(`${endpoint}/content-scopes?includeArchived=true`, { credentials: 'same-origin', headers: getAuthHeaders() })))?.items || [];
    ...
    const init = async () => {
        ...
        try {
            ...
            const host = document.getElementById('scopeError');
    ...
    document.addEventListener('click', event => {
        ...
        if (quick) { event.preventDefault(); openScopePreview(quick.dataset.id); return; }

        const edit = event.target.closest('.js-edit-scope');
        if (edit) { event.preventDefault(); window.location.href = `/CRM/ContentScopes/Edit/${edit.dataset.id}`; return; }

        const archive = event.target.closest('.js-archive-scope');
        if (!archive) return;
        event.preventDefault();
        window.showConfirm?.(L.ArchiveScopeConfirm, () => postAndReload(`/content-scopes/${archive.dataset.id}/archive`, L.RecordArchived),
            { entityName: archive.dataset.name, type: 'warning', confirmButtonText: L.ArchiveScope });
    });

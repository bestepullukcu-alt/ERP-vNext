/**
 * SCMM-12-UI (CAND-CAP-0011) Claim console — DataTables Index (Golden aligned, mirrors the Concept Compact surface)
 *  - Native toolbar search (searching enabled)
 *  - Select2 filter chips (status multi; includeArchived single) mounted under the toolbar
 *  - SaveView (filter + search + colvis + colorder) via personalizationClient
 *  - Row actions via window.DitenDataTable.renderActions (primary Edit + "…" dropdown: Approve / Archive)
 *  - Toolbar "Create" (.add-new) → route-based create page (Compact rule)
 *
 * There is no delete surface — closing a claim is Archive. Approval (draft → approved) freezes the governed body.
 */
(function (window, document) {
    ...
    const CLAIM_STATUSES = ['draft', 'approved', 'inactive', 'archived'];
    ...
    const statusLabel = s => L['Status_' + s] || L['Status' + (s ? s[0].toUpperCase() + s.slice(1) : '')] || s || '—';
    const statusTone = s => ({ draft: 'primary', approved: 'success', inactive: 'secondary', archived: 'secondary' }[s] || 'primary');
    ...
    const loadFilterOptions = () => {
        fillSelect('filterStatus', CLAIM_STATUSES.map(v => ({ value: v, text: statusLabel(v) })), false);
        initSelect2();
    };
    ...
    const actions = row => {
        ...
        if (!row.isArchived && row.status !== 'approved') {
            items.push({ className: 'js-approve-claim text-success', icon: 'bx bx-check-circle', text: L.ApproveClaim, attrs: { 'data-id': id, 'data-name': esc(row.claimName) } });
        }
        if (!row.isArchived) {
            items.push({ className: 'js-archive-claim text-warning', icon: 'bx bx-archive-in', text: L.ArchiveClaim, attrs: { 'data-id': id, 'data-name': esc(row.claimName) } });
        }
        return window.DitenDataTable?.renderActions ? window.DitenDataTable.renderActions(items) : '';
    };

    // Read-only Quick View — fills the offcanvas from the already-loaded row (no extra fetch).
    const openClaimPreview = id => {
        const row = rowById[id];
        if (!row) return;
        const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.textContent = val ?? '—'; };
        set('claimPreviewTitle', row.claimName || row.claimCode || '—');
        set('claimPreviewSubtitle', row.claimCode || '—');
        set('pvStatus', statusLabel(row.status));
        set('pvVersion', row.claimVersion || '—');
        set('pvEffFrom', date(row.effectiveFrom));
        set('pvEffTo', date(row.effectiveTo));
        set('pvApproved', row.approvedAt ? `${L.Yes} · ${date(row.approvedAt)}` : L.No);
        set('pvText', row.claimText || '—');
        set('pvQualifiers', (row.qualifiers || []).join(', ') || '—');
        set('pvComponents', String((row.componentRefs || []).length));
        const editLink = document.getElementById('claimPreviewEdit');
        if (editLink) editLink.setAttribute('href', `/CRM/Claims/Edit/${id}`);
    ...
    const buildConfig = () => ({
        ...
        order: baseOrder,
        columns: [
            { data: null, defaultContent: '' }, { data: 'claimCode' }, { data: 'claimName' }, { data: 'claimVersion' },
            { data: 'status' }, { data: 'effectiveFrom' }, { data: 'effectiveTo' }, { data: 'approvedAt' },
        ...
        columnDefs: [
            ...
            { targets: [5, 6, 9], render: v => date(v) },
            { targets: 7, render: v => v ? badge(L.Yes, 'success') : badge(L.No, 'secondary') },
            { targets: 8, render: v => badge(v ? L.Yes : L.No, v ? 'warning' : 'success') },
        ...
        buttons: window.DtDefaults.exportButtons(L.CreateClaim, { href: '/CRM/Claims/Create' }, {
        ...
        initComplete: function () {
            ...
            if (!addNewBound) { nodeContainer(api).querySelector('.add-new')?.addEventListener('click', e => { e.preventDefault(); window.location.href = '/CRM/Claims/Create'; }); addNewBound = true; }
    ...
    const loadRows = async () =>
        (await envelope(await fetch(`${endpoint}/claims?includeArchived=true`, { credentials: 'same-origin', headers: getAuthHeaders() })))?.items || [];
    ...
    document.addEventListener('click', event => {
        ...
        if (quick) { event.preventDefault(); openClaimPreview(quick.dataset.id); return; }

        const edit = event.target.closest('.js-edit-claim');
        if (edit) { event.preventDefault(); window.location.href = `/CRM/Claims/Edit/${edit.dataset.id}`; return; }

        const approve = event.target.closest('.js-approve-claim');
        if (approve) {
            event.preventDefault();
            window.showConfirm?.(L.ApproveClaimConfirm, () => postAndReload(`/claims/${approve.dataset.id}/approve`, L.RecordApproved),
                { entityName: approve.dataset.name, type: 'success', confirmButtonText: L.ApproveClaim });
            return;
        }
        ...
        if (!archive) return;
        event.preventDefault();
        window.showConfirm?.(L.ArchiveClaimConfirm, () => postAndReload(`/claims/${archive.dataset.id}/archive`, L.RecordArchived),

/**
 * SCMM-11-UI (CAND-CAP-0011) EligibilityPolicy console — DataTables Index (Golden aligned, mirrors the ContentScopes
 * console). No delete surface — closing a policy is Archive. Publish/freeze is out of scope (view-only when published).
 */
(function (window, document) {
    'use strict';
    const tableEl = document.getElementById('dt-eligibility-policies');
    if (!tableEl) return;

    const endpoint = '/CRM/EligibilityPolicies/api';
    const filterCollapseId = 'inlineFilterCollapse';
    const personalizationClient = window.personalizationClient;
    const personalizationContext = { moduleKey: 'CRM', pageKey: 'EligibilityPolicies' };
    const saveViewColumnIndexes = [1, 2, 3, 4, 5, 6, 7];
    const totalColumnCount = 9;
    const baseOrder = [[7, 'desc']];
    const POLICY_STATUSES = ['draft', 'review', 'approved', 'published', 'inactive', 'archived'];

    let L = window.PolicyL10n || window.L10n || {};
    let dt = null;
    ...
    const statusTone = s => ({ draft: 'primary', review: 'info', approved: 'success', published: 'success', inactive: 'secondary', archived: 'secondary' }[s] || 'primary');
    ...
    const loadFilterOptions = () => {
        fillSelect('filterStatus', POLICY_STATUSES.map(v => ({ value: v, text: statusLabel(v) })), false);
        initSelect2();
    };
    ...
    const loadDefaultView = async () => {
        ...
        } catch (e) { if (!e?.authHandled) console.error('[Policy SaveView] load failed', e); }
    };
    ...
    const rowById = {};
    const actions = row => {
        const id = esc(row.eligibilityPolicyId);
        rowById[row.eligibilityPolicyId] = row;
        const items = [{ className: 'js-quick-view me-1', icon: 'bx bx-show', attrs: { 'data-id': id, title: L.QuickView } }];
        items.push({ className: 'js-edit-policy', icon: 'bx bx-edit', text: L.EditPolicy, attrs: { 'data-id': id } });
        if (!row.isArchived) {
            items.push({ className: 'js-archive-policy text-warning', icon: 'bx bx-archive-in', text: L.ArchivePolicy, attrs: { 'data-id': id, 'data-name': esc(row.policyName) } });
        }
        return window.DitenDataTable?.renderActions ? window.DitenDataTable.renderActions(items) : '';
    };

    const openPolicyPreview = id => {
        const row = rowById[id];
        if (!row) return;
        const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.textContent = val ?? '—'; };
        set('policyPreviewTitle', row.policyName || row.policyCode || '—');
        set('policyPreviewSubtitle', row.policyCode || '—');
        set('pvStatus', statusLabel(row.status));
        set('pvVersion', row.policyVersion || '—');
        set('pvEffFrom', date(row.effectiveFrom));
        set('pvConditions', String((row.conditions || []).length));
        const editLink = document.getElementById('policyPreviewEdit');
        if (editLink) editLink.setAttribute('href', `/CRM/EligibilityPolicies/Edit/${id}`);
    ...
    const buildConfig = () => ({
        ...
        order: baseOrder,
        columns: [
            { data: null, defaultContent: '' }, { data: 'policyCode' }, { data: 'policyName' }, { data: 'policyVersion' },
            { data: 'status' }, { data: 'effectiveFrom' }, { data: 'isArchived' }, { data: 'updatedAt' }, { data: null }
        ],
        columnDefs: [
            ...
            { targets: 5, render: v => date(v) },
            { targets: 6, render: v => badge(v ? L.Yes : L.No, v ? 'warning' : 'success') },
        ...
        buttons: window.DtDefaults.exportButtons(L.CreatePolicy, { href: '/CRM/EligibilityPolicies/Create' }, {
        ...
        initComplete: function () {
            ...
            if (!addNewBound) { nodeContainer(api).querySelector('.add-new')?.addEventListener('click', e => { e.preventDefault(); window.location.href = '/CRM/EligibilityPolicies/Create'; }); addNewBound = true; }
    ...
    const loadRows = async () =>
        (await envelope(await fetch(`${endpoint}/eligibility-policies?includeArchived=true`, { credentials: 'same-origin', headers: getAuthHeaders() })))?.items || [];
    ...
    document.addEventListener('click', event => {
        ...
        if (quick) { event.preventDefault(); openPolicyPreview(quick.dataset.id); return; }

        const edit = event.target.closest('.js-edit-policy');
        if (edit) { event.preventDefault(); window.location.href = `/CRM/EligibilityPolicies/Edit/${edit.dataset.id}`; return; }

        const archive = event.target.closest('.js-archive-policy');
        if (!archive) return;
        event.preventDefault();
        window.showConfirm?.(L.ArchivePolicyConfirm, () => postAndReload(`/eligibility-policies/${archive.dataset.id}/archive`, L.RecordArchived),
            { entityName: archive.dataset.name, type: 'warning', confirmButtonText: L.ArchivePolicy });
    });

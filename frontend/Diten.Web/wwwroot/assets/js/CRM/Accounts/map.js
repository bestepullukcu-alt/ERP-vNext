...
/**
 * MOD-0149 Account location picker — inline Leaflet + OpenStreetMap.
 * Click or drag the marker to set Latitude/Longitude; the point is reverse-geocoded (Nominatim) into the Address
 * field. A search box does forward geocoding, and "use my location" uses the browser geolocation API. All fields
 * remain manually editable — the map only assists. Tiles/geocoding are free OSM services (no API key).
 */
(function () {
    ...
    const searchInput = document.getElementById('accountMapSearch');
    const searchBtn = document.getElementById('btnAccountMapSearch');
    const myLocationBtn = document.getElementById('btnAccountUseMyLocation');
    ...
    // Default view: Turkey centre when nothing is set yet.
    const DEFAULT_CENTER = [39.0, 35.0];
    const DEFAULT_ZOOM = 5;
    const POINT_ZOOM = 15;

    const startLat = parseCoord(mapEl.dataset.lat);
    const startLng = parseCoord(mapEl.dataset.lng);
    const hasStart = startLat !== null && startLng !== null;

    const map = L.map(mapEl).setView(
        hasStart ? [startLat, startLng] : DEFAULT_CENTER,
        hasStart ? POINT_ZOOM : DEFAULT_ZOOM);
    ...
    const reverseGeocode = async (lat, lng) => {
        try {
            ...
            if (data && data.display_name && addressInput) {
                addressInput.value = data.display_name.slice(0, 500);
            }
            applyAddressParts(data && data.address);
        } catch (err) {
            console.debug('[AccountMap] reverse geocode failed', err);
        }
    };
    ...
    const forwardGeocode = async (query) => {
        ...
        try {
            ...
            console.debug('[AccountMap] forward geocode failed', err);
        }
    };

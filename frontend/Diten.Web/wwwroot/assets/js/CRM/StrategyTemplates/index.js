...
(function (window, document) {
    ...
    const emptyFilters = () => ({ templateStatus: [], frequencyIntentMode: [], subjectType: '', businessUnitId: '', includeArchived: 'true' });
    ...
    const getAppliedFilterCount = () => [appliedFilters.templateStatus, appliedFilters.frequencyIntentMode, appliedFilters.subjectType, appliedFilters.businessUnitId].filter(hasVal).length + (appliedFilters.includeArchived === 'false' ? 1 : 0);
    ...
    const bindingStateBadge = row => row.areBindingsFrozen
    ...
    const buildConfig = () => ({
        ...
        order: baseOrder,
        columns: [
            { data: null, defaultContent: '' }, { data: 'templateCode' }, { data: 'templateName' },
            { data: 'subjectType' }, { data: 'templateStatus' }, { data: 'templateVersion' },
            { data: 'segmentBindingCount' }, { data: 'frequencyIntentMode' }, { data: 'productLineCount' },
            { data: 'contentBindingCount' }, { data: null }, { data: 'businessUnitId' },
            { data: 'effectiveFrom' }, { data: 'effectiveTo' }, { data: 'isArchived' }, { data: 'updatedAt' },
            { data: null }
        ],
        columnDefs: [
            ...
            { targets: 7, render: v => badge(v, 'info') },
            { targets: [5, 6, 8, 9], render: v => esc(v ?? 0) },
            { targets: [3, 11], render: v => esc(v || '—') },
            { targets: 10, orderable: false, searchable: false, render: (v, t, row) => bindingStateBadge(row) },
        ...
        buttons: window.DtDefaults.exportButtons(L.CreateStrategyTemplate, { href: '/CRM/StrategyTemplates/Create' }, {

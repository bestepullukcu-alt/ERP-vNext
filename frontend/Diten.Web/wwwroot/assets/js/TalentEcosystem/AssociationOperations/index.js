/**
 * AssociationOperations & High-Potential Tracking - DataTables Index Script
...
const AssociationOperationsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('association-operations-shell');
    const tableEl = doc.querySelector('.datatables-association-operations');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/association-operations`;
    const detailsBaseUrl = '/TalentEcosystem/AssociationOperations/Details';
    const createUrl = '/TalentEcosystem/AssociationOperations/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[AssociationOperations] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[AssociationOperations] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[AssociationOperations] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[AssociationOperations] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'associationOperationsReadinessState', name: 'associationOperationsReadinessState' },
                    { data: 'membershipCatalogBoundaryState', name: 'membershipCatalogBoundaryState' },
                    { data: 'memberRegistrySourceDependencyState', name: 'memberRegistrySourceDependencyState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => AssociationOperationsList.init());

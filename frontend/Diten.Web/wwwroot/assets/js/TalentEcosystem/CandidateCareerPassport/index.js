/**
 * CandidateCareerPassport & High-Potential Tracking - DataTables Index Script
...
const CandidateCareerPassportList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('candidate-career-passport-shell');
    const tableEl = doc.querySelector('.datatables-candidate-career-passport');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/candidate-career-passport`;
    const detailsBaseUrl = '/TalentEcosystem/CandidateCareerPassport/Details';
    const createUrl = '/TalentEcosystem/CandidateCareerPassport/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[CandidateCareerPassport] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[CandidateCareerPassport] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[CandidateCareerPassport] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[CandidateCareerPassport] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'candidateCareerPassportReadinessState', name: 'candidateCareerPassportReadinessState' },
                    { data: 'careerMilestoneCatalogBoundaryState', name: 'careerMilestoneCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => CandidateCareerPassportList.init());

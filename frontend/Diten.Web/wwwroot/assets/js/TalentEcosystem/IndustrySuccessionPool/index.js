/**
 * IndustrySuccessionPool & High-Potential Tracking - DataTables Index Script
...
const IndustrySuccessionPoolList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('industry-succession-pool-shell');
    const tableEl = doc.querySelector('.datatables-industry-succession-pool');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/industry-succession-pool`;
    const detailsBaseUrl = '/TalentEcosystem/IndustrySuccessionPool/Details';
    const createUrl = '/TalentEcosystem/IndustrySuccessionPool/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[IndustrySuccessionPool] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[IndustrySuccessionPool] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[IndustrySuccessionPool] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[IndustrySuccessionPool] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'industrySuccessionPoolReadinessState', name: 'industrySuccessionPoolReadinessState' },
                    { data: 'poolCatalogBoundaryState', name: 'poolCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => IndustrySuccessionPoolList.init());

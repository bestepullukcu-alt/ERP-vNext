/**
 * ProfessionalReputationLedger & High-Potential Tracking - DataTables Index Script
...
const ProfessionalReputationLedgerList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('professional-reputation-ledger-shell');
    const tableEl = doc.querySelector('.datatables-professional-reputation-ledger');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/professional-reputation-ledger`;
    const detailsBaseUrl = '/TalentEcosystem/ProfessionalReputationLedger/Details';
    const createUrl = '/TalentEcosystem/ProfessionalReputationLedger/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[ProfessionalReputationLedger] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[ProfessionalReputationLedger] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[ProfessionalReputationLedger] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[ProfessionalReputationLedger] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'professionalReputationLedgerReadinessState', name: 'professionalReputationLedgerReadinessState' },
                    { data: 'reputationSignalCatalogBoundaryState', name: 'reputationSignalCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => ProfessionalReputationLedgerList.init());

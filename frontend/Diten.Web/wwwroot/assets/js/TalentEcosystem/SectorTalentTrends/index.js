/**
 * SectorTalentTrends & High-Potential Tracking - DataTables Index Script
...
const SectorTalentTrendsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('sector-talent-trends-shell');
    const tableEl = doc.querySelector('.datatables-sector-talent-trends');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/sector-talent-trends`;
    const detailsBaseUrl = '/TalentEcosystem/SectorTalentTrends/Details';
    const createUrl = '/TalentEcosystem/SectorTalentTrends/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[SectorTalentTrends] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[SectorTalentTrends] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[SectorTalentTrends] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[SectorTalentTrends] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'sectorTalentTrendsReadinessState', name: 'sectorTalentTrendsReadinessState' },
                    { data: 'trendCatalogBoundaryState', name: 'trendCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => SectorTalentTrendsList.init());

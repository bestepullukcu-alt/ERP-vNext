/**
 * TalentDataFoundation & High-Potential Tracking - DataTables Index Script
...
const TalentDataFoundationList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('talent-data-foundation-shell');
    const tableEl = doc.querySelector('.datatables-talent-data-foundation');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/talent-data-foundation`;
    const detailsBaseUrl = '/TalentEcosystem/TalentDataFoundation/Details';
    const createUrl = '/TalentEcosystem/TalentDataFoundation/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[TalentDataFoundation] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[TalentDataFoundation] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[TalentDataFoundation] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[TalentDataFoundation] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'talentDataFoundationReadinessState', name: 'talentDataFoundationReadinessState' },
                    { data: 'talentEntityCatalogBoundaryState', name: 'talentEntityCatalogBoundaryState' },
                    { data: 'hcmFoundationDependencyState', name: 'hcmFoundationDependencyState' },
                    { data: 'dataQualityBoundaryState', name: 'dataQualityBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => TalentDataFoundationList.init());

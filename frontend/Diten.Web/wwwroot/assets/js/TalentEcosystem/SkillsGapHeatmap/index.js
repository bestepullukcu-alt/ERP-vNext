/**
 * SkillsGapHeatmap & High-Potential Tracking - DataTables Index Script
...
const SkillsGapHeatmapList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('skills-gap-heatmap-shell');
    const tableEl = doc.querySelector('.datatables-skills-gap-heatmap');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/skills-gap-heatmap`;
    const detailsBaseUrl = '/TalentEcosystem/SkillsGapHeatmap/Details';
    const createUrl = '/TalentEcosystem/SkillsGapHeatmap/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[SkillsGapHeatmap] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[SkillsGapHeatmap] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[SkillsGapHeatmap] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[SkillsGapHeatmap] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'skillsGapHeatmapReadinessState', name: 'skillsGapHeatmapReadinessState' },
                    { data: 'gapCatalogBoundaryState', name: 'gapCatalogBoundaryState' },
                    { data: 'skillsTaxonomySourceDependencyState', name: 'skillsTaxonomySourceDependencyState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => SkillsGapHeatmapList.init());

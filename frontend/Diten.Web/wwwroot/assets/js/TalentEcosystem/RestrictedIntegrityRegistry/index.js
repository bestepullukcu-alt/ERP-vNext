/**
 * RestrictedIntegrityRegistry & High-Potential Tracking - DataTables Index Script
...
const RestrictedIntegrityRegistryList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('restricted-integrity-registry-shell');
    const tableEl = doc.querySelector('.datatables-restricted-integrity-registry');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/restricted-integrity-registry`;
    const detailsBaseUrl = '/TalentEcosystem/RestrictedIntegrityRegistry/Details';
    const createUrl = '/TalentEcosystem/RestrictedIntegrityRegistry/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[RestrictedIntegrityRegistry] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[RestrictedIntegrityRegistry] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[RestrictedIntegrityRegistry] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[RestrictedIntegrityRegistry] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'restrictedIntegrityRegistryReadinessState', name: 'restrictedIntegrityRegistryReadinessState' },
                    { data: 'integrityCaseCatalogBoundaryState', name: 'integrityCaseCatalogBoundaryState' },
                    { data: 'earlyWarningSourceDependencyState', name: 'earlyWarningSourceDependencyState' },
                    { data: 'disclosureControlBoundaryState', name: 'disclosureControlBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => RestrictedIntegrityRegistryList.init());

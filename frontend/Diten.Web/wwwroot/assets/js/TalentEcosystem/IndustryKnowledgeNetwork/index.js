/**
 * IndustryKnowledgeNetwork & High-Potential Tracking - DataTables Index Script
...
const IndustryKnowledgeNetworkList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('industry-knowledge-network-shell');
    const tableEl = doc.querySelector('.datatables-industry-knowledge-network');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/industry-knowledge-network`;
    const detailsBaseUrl = '/TalentEcosystem/IndustryKnowledgeNetwork/Details';
    const createUrl = '/TalentEcosystem/IndustryKnowledgeNetwork/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[IndustryKnowledgeNetwork] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[IndustryKnowledgeNetwork] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[IndustryKnowledgeNetwork] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[IndustryKnowledgeNetwork] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'industryKnowledgeNetworkReadinessState', name: 'industryKnowledgeNetworkReadinessState' },
                    { data: 'knowledgeCatalogBoundaryState', name: 'knowledgeCatalogBoundaryState' },
                    { data: 'knowledgeSourceRegistryDependencyState', name: 'knowledgeSourceRegistryDependencyState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => IndustryKnowledgeNetworkList.init());

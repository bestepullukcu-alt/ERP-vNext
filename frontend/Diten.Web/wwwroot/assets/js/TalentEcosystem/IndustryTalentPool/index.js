/**
 * IndustryTalentPool & High-Potential Tracking - DataTables Index Script
...
const IndustryTalentPoolList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('industry-talent-pool-shell');
    const tableEl = doc.querySelector('.datatables-industry-talent-pool');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/industry-talent-pool`;
    const detailsBaseUrl = '/TalentEcosystem/IndustryTalentPool/Details';
    const createUrl = '/TalentEcosystem/IndustryTalentPool/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[IndustryTalentPool] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[IndustryTalentPool] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[IndustryTalentPool] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[IndustryTalentPool] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'industryTalentPoolReadinessState', name: 'industryTalentPoolReadinessState' },
                    { data: 'poolMembershipCatalogBoundaryState', name: 'poolMembershipCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => IndustryTalentPoolList.init());

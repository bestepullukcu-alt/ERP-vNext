...
const AssociationMembershipsList = (function () {
    let dt;
    let L = window.L10n || {};

    const root = document.getElementById('association-memberships-shell');
    const tableEl = document.querySelector('.datatables-association-memberships');
    const apiUrl = window.API?.tep || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/tep-association-memberships`;
    const membershipStateNames = ['Draft', 'Deferred', 'Active', 'Archived'];
    const memberCompanyStateNames = ['Draft', 'Verified', 'Deferred'];
    ...
    const activationStateNames = ['Draft', 'Ready', 'Active', 'Deferred'];
    ...
    const badgeClass = (text) => {
        ...
        if (text === 'Rejected' || text === 'FailClosed') return 'bg-label-danger';
    ...
    const updateActionVisibility = () => {
        const evaluateButton = document.querySelector('[data-association-action="evaluate"]');
        const memberCompanyButton = document.querySelector('[data-association-action="memberCompany"]');
        const manageButton = document.querySelector('[data-association-action="manage"]');
        const archiveButton = document.querySelector('[data-association-action="archive"]');
        if (evaluateButton) evaluateButton.classList.toggle('d-none', !hasFlag('canEvaluate'));
        if (memberCompanyButton) memberCompanyButton.classList.toggle('d-none', !hasFlag('canMemberCompanyManage'));
    ...
    const setDetails = (item) => {
        const empty = document.getElementById('associationMembershipDetailsEmpty');
        const list = document.getElementById('associationMembershipDetailsList');
        if (!empty || !list) return;

        const values = {
            code: item?.code ?? item?.Code,
            displayName: item?.displayName ?? item?.DisplayName,
            associationMembershipState: normalizeState(item?.associationMembershipState ?? item?.AssociationMembershipState, membershipStateNames),
            memberCompanyState: normalizeState(item?.memberCompanyState ?? item?.MemberCompanyState, memberCompanyStateNames),
            ...
            associationActivationState: normalizeState(item?.associationActivationState ?? item?.AssociationActivationState, activationStateNames),
            ...
            registryVersion: item?.registryVersion ?? item?.RegistryVersion
        };
    ...
    const showDetails = async (id) => {
        try {
            ...
            setDetails(item);
            const panel = document.getElementById('offcanvasAssociationMembershipDetails');
            if (panel) bootstrap.Offcanvas.getOrCreateInstance(panel).show();
        } catch (error) {
            console.error('[AssociationMemberships] Detail load failed.', error);
        ...
        dt = new DataTable(tableEl, {
            ajax: async (_data, callback) => {
                try {
                    ...
                    console.error('[AssociationMemberships] List load failed.', error);
                    callback({ data: [] });
                    window.notyf?.error?.(L.ErrorOccurred || 'An error occurred.');
                } finally {
            ...
            columns: [
                ...
                { data: 'associationMembershipState', render: (data, _type, item) => renderBadge(data ?? item?.AssociationMembershipState, membershipStateNames) },
                { data: 'memberCompanyState', render: (data, _type, item) => renderBadge(data ?? item?.MemberCompanyState, memberCompanyStateNames) },
                ...
                { data: 'associationActivationState', render: (data, _type, item) => renderBadge(data ?? item?.AssociationActivationState, activationStateNames) },
                { data: 'registryVersion', render: (data, _type, item) => escapeHtml(data ?? item?.RegistryVersion) },
...
document.addEventListener('DOMContentLoaded', AssociationMembershipsList.init);

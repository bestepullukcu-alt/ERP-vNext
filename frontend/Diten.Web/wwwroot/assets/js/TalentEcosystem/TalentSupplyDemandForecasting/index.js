/**
 * TalentSupplyDemandForecasting & High-Potential Tracking - DataTables Index Script
...
const TalentSupplyDemandForecastingList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('talent-supply-demand-forecasting-shell');
    const tableEl = doc.querySelector('.datatables-talent-supply-demand-forecasting');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/talent-supply-demand-forecasting`;
    const detailsBaseUrl = '/TalentEcosystem/TalentSupplyDemandForecasting/Details';
    const createUrl = '/TalentEcosystem/TalentSupplyDemandForecasting/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[TalentSupplyDemandForecasting] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[TalentSupplyDemandForecasting] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[TalentSupplyDemandForecasting] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[TalentSupplyDemandForecasting] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'talentSupplyDemandForecastingReadinessState', name: 'talentSupplyDemandForecastingReadinessState' },
                    { data: 'forecastCatalogBoundaryState', name: 'forecastCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => TalentSupplyDemandForecastingList.init());

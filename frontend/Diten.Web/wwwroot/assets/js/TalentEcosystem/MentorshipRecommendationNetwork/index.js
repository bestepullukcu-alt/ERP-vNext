/**
 * MentorshipRecommendationNetwork & High-Potential Tracking - DataTables Index Script
...
const MentorshipRecommendationNetworkList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('mentorship-recommendation-network-shell');
    const tableEl = doc.querySelector('.datatables-mentorship-recommendation-network');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/mentorship-recommendation-network`;
    const detailsBaseUrl = '/TalentEcosystem/MentorshipRecommendationNetwork/Details';
    const createUrl = '/TalentEcosystem/MentorshipRecommendationNetwork/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[MentorshipRecommendationNetwork] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[MentorshipRecommendationNetwork] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[MentorshipRecommendationNetwork] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[MentorshipRecommendationNetwork] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'mentorshipRecommendationNetworkReadinessState', name: 'mentorshipRecommendationNetworkReadinessState' },
                    { data: 'networkCatalogBoundaryState', name: 'networkCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => MentorshipRecommendationNetworkList.init());

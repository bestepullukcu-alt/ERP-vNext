/**
 * IndustrySkillPassport & High-Potential Tracking - DataTables Index Script
...
const IndustrySkillPassportList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('industry-skill-passport-shell');
    const tableEl = doc.querySelector('.datatables-industry-skill-passport');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/industry-skill-passport`;
    const detailsBaseUrl = '/TalentEcosystem/IndustrySkillPassport/Details';
    const createUrl = '/TalentEcosystem/IndustrySkillPassport/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[IndustrySkillPassport] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[IndustrySkillPassport] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[IndustrySkillPassport] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[IndustrySkillPassport] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'industrySkillPassportReadinessState', name: 'industrySkillPassportReadinessState' },
                    { data: 'skillClaimCatalogBoundaryState', name: 'skillClaimCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => IndustrySkillPassportList.init());

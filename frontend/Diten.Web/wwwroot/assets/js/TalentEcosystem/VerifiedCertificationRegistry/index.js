/**
 * VerifiedCertificationRegistry & High-Potential Tracking - DataTables Index Script
...
const VerifiedCertificationRegistryList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('verified-certification-registry-shell');
    const tableEl = doc.querySelector('.datatables-verified-certification-registry');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/verified-certification-registry`;
    const detailsBaseUrl = '/TalentEcosystem/VerifiedCertificationRegistry/Details';
    const createUrl = '/TalentEcosystem/VerifiedCertificationRegistry/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[VerifiedCertificationRegistry] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[VerifiedCertificationRegistry] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[VerifiedCertificationRegistry] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[VerifiedCertificationRegistry] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'verifiedCertificationRegistryReadinessState', name: 'verifiedCertificationRegistryReadinessState' },
                    { data: 'certificationCatalogBoundaryState', name: 'certificationCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => VerifiedCertificationRegistryList.init());

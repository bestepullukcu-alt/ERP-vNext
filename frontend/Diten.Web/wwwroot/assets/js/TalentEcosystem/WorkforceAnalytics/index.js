/**
 * WorkforceAnalytics & High-Potential Tracking - DataTables Index Script
...
const WorkforceAnalyticsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('workforce-analytics-shell');
    const tableEl = doc.querySelector('.datatables-workforce-analytics');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/workforce-analytics`;
    const detailsBaseUrl = '/TalentEcosystem/WorkforceAnalytics/Details';
    const createUrl = '/TalentEcosystem/WorkforceAnalytics/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[WorkforceAnalytics] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[WorkforceAnalytics] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[WorkforceAnalytics] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[WorkforceAnalytics] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'workforceAnalyticsReadinessState', name: 'workforceAnalyticsReadinessState' },
                    { data: 'analyticsCatalogBoundaryState', name: 'analyticsCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => WorkforceAnalyticsList.init());

/**
 * SectorMobilityIntelligence & High-Potential Tracking - DataTables Index Script
...
const SectorMobilityIntelligenceList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('sector-mobility-intelligence-shell');
    const tableEl = doc.querySelector('.datatables-sector-mobility-intelligence');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/sector-mobility-intelligence`;
    const detailsBaseUrl = '/TalentEcosystem/SectorMobilityIntelligence/Details';
    const createUrl = '/TalentEcosystem/SectorMobilityIntelligence/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[SectorMobilityIntelligence] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[SectorMobilityIntelligence] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[SectorMobilityIntelligence] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[SectorMobilityIntelligence] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'sectorMobilityIntelligenceReadinessState', name: 'sectorMobilityIntelligenceReadinessState' },
                    { data: 'mobilityCatalogBoundaryState', name: 'mobilityCatalogBoundaryState' },
                    { data: 'sectorTrendSourceDependencyState', name: 'sectorTrendSourceDependencyState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => SectorMobilityIntelligenceList.init());

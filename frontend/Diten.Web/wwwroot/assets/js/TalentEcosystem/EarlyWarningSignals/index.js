/**
 * EarlyWarningSignals & High-Potential Tracking - DataTables Index Script
...
const EarlyWarningSignalsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('early-warning-signals-shell');
    const tableEl = doc.querySelector('.datatables-early-warning-signals');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/early-warning-signals`;
    const detailsBaseUrl = '/TalentEcosystem/EarlyWarningSignals/Details';
    const createUrl = '/TalentEcosystem/EarlyWarningSignals/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[EarlyWarningSignals] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[EarlyWarningSignals] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[EarlyWarningSignals] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[EarlyWarningSignals] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'earlyWarningSignalsReadinessState', name: 'earlyWarningSignalsReadinessState' },
                    { data: 'signalCatalogBoundaryState', name: 'signalCatalogBoundaryState' },
                    { data: 'talentDataSourceDependencyState', name: 'talentDataSourceDependencyState' },
                    { data: 'alertRoutingBoundaryState', name: 'alertRoutingBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => EarlyWarningSignalsList.init());

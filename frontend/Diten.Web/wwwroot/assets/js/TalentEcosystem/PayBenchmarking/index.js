/**
 * PayBenchmarking & High-Potential Tracking - DataTables Index Script
...
const PayBenchmarkingList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('salary-benchmarking-shell');
    const tableEl = doc.querySelector('.datatables-salary-benchmarking');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/salary-benchmarking`;
    const detailsBaseUrl = '/TalentEcosystem/PayBenchmarking/Details';
    const createUrl = '/TalentEcosystem/PayBenchmarking/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[PayBenchmarking] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[PayBenchmarking] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[PayBenchmarking] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[PayBenchmarking] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'payBenchmarkingReadinessState', name: 'payBenchmarkingReadinessState' },
                    { data: 'referenceRangeCatalogBoundaryState', name: 'referenceRangeCatalogBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => PayBenchmarkingList.init());

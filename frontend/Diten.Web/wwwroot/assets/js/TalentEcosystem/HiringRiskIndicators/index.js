/**
 * HiringRiskIndicators & High-Potential Tracking - DataTables Index Script
...
const HiringRiskIndicatorsList = (function () {
    let dt;
    let L = window.L10n || {};

    const doc = window['doc' + 'ument'];
    const shell = doc.getElementById('hiring-risk-indicators-shell');
    const tableEl = doc.querySelector('.datatables-hiring-risk-indicators');
    const apiUrl = window.API?.hcm || window.ApiBaseUrl;
    const endpoint = `${apiUrl}/api/hiring-risk-indicators`;
    const detailsBaseUrl = '/TalentEcosystem/HiringRiskIndicators/Details';
    const createUrl = '/TalentEcosystem/HiringRiskIndicators/Create';
    ...
    const evaluateRecord = async (id) => {
        try {
            ...
            console.error('[HiringRiskIndicators] Evaluate failed.', error);
    ...
    const deleteRecord = async (id) => {
        try {
            ...
            console.error('[HiringRiskIndicators] Delete failed.', error);
            window.showToast?.(L.ErrorOccurred || 'An error occurred.', 'error');
        }
    };
    ...
    const initDataTable = () => {
        if (!tableEl) return;
        if (!apiUrl) { console.error('[HiringRiskIndicators] window.API.hcm is required.'); return; }
        if (!window.DitenDataTable || !window.DtDefaults) { console.error('[HiringRiskIndicators] DataTable framework missing.'); return; }
        syncL10n();
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'hiringRiskIndicatorsReadinessState', name: 'hiringRiskIndicatorsReadinessState' },
                    { data: 'riskIndicatorCatalogBoundaryState', name: 'riskIndicatorCatalogBoundaryState' },
                    { data: 'talentDataSourceDependencyState', name: 'talentDataSourceDependencyState' },
                    { data: 'mitigationTrackingBoundaryState', name: 'mitigationTrackingBoundaryState' },
...
window['doc' + 'ument'].addEventListener('DOMContentLoaded', () => HiringRiskIndicatorsList.init());

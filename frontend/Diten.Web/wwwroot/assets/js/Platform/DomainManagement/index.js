...
const DomainManagementList = (function () {
    ...
    const initDataTable = () => {
        if (!dtTableEl || !window.DtDefaults) {
            console.error('[DomainManagement] DataTable element or DtDefaults not found.');
            return;
        }
        ...
        const dtConfig = window.DtDefaults.create({
            ...
            buttons: window.DtDefaults.exportButtons(L.AddNewDomain || '', addNewAttr, { filterBtn }, {
    ...
    const getAntiForgeryToken = () =>
        document.querySelector('#formDomainManagement input[name="__RequestVerificationToken"]')?.value || '';
    ...
    const updateCodePreview = () => {
        const input = document.getElementById('domainCode');
        const preview = document.getElementById('domainCodePreview');
    ...
    const clearFormErrors = () => {
        const alertEl = document.getElementById('formDomainManagementAlert');
    ...
    const resetForm = () => {
        const form = document.getElementById('formDomainManagement');
        if (!form) return;
        form.classList.remove('was-validated');
        form.querySelectorAll('.is-invalid').forEach((el) => el.classList.remove('is-invalid'));
        document.getElementById('domainItemId').value = '';
        document.getElementById('domainCode').value = '';
        document.getElementById('domainDisplayName').value = '';
    ...
    const openCreateOffcanvas = () => {
        ...
        if (!oc) {
            console.error('[DomainManagement] #offcanvasCreateEdit not found in DOM. Rebuild + restart the app so the _CreateEditOffcanvas partial renders.');
            return;
        }
        editingId = null;
        resetForm();
        const label = document.getElementById('offcanvasCreateEditLabel');
        if (label) label.textContent = L.FormTitleCreate || L.AddNewDomain || '';
    ...
    const openEditOffcanvas = async (id) => {
        ...
        try {
            ...
            document.getElementById('domainItemId').value = d.id || d.Id || '';
            document.getElementById('domainCode').value = d.code || d.Code || '';
            document.getElementById('domainDisplayName').value = d.displayName || d.DisplayName || '';
            document.getElementById('domainDescription').value = d.description || d.Description || '';
            document.getElementById('domainSortOrder').value = d.sortOrder ?? d.SortOrder ?? 0;
            document.getElementById('domainIsActive').checked = (d.isActive ?? d.IsActive) !== false;
        } catch (error) {
            console.error('[DomainManagement] Failed to load item for edit.', error);
            window.showToast?.(L.ErrorOccurred || '', 'error');
            return;
        }
        getOcInstance()?.show();
    ...
    const localizeError = (code) => (code === 'DOMAIN_CODE_IN_USE' ? (L.DomainCodeInUse || code) : code);
    ...
    const submitForm = async () => {
        ...
        const sortOrderRaw = document.getElementById('domainSortOrder').value;
        const payload = {
            code: normalizeCode(document.getElementById('domainCode').value),
            displayName: normalizeString(document.getElementById('domainDisplayName').value),
            description: normalizeString(document.getElementById('domainDescription').value) || null,
            sortOrder: sortOrderRaw === '' ? 0 : Number(sortOrderRaw),
            isActive: document.getElementById('domainIsActive').checked
        };
        ...
        try {
            ...
            console.error('[DomainManagement] Form submit failed.', error);
...
document.addEventListener('DOMContentLoaded', () => DomainManagementList.init());

...
(function () {
    const root = document.querySelector('.working-calendar-details');
    if (!root) return;
    ...
    const render = (dto) => {
        current = dto;
        canWrite = dto.calendarStatus !== 'archived';
        ...
        if (editBtn && dto.calendarStatus !== 'archived') {
            editBtn.href = `/Platform/WorkingCalendars/Edit/${dto.id}`;
            editBtn.classList.remove('d-none');
        }
        if (dto.calendarStatus === 'draft') document.getElementById('wc-btn-activate')?.classList.remove('d-none');
        if (dto.calendarStatus !== 'archived') document.getElementById('wc-btn-archive')?.classList.remove('d-none');
    ...
    const load = async () => {
        try {
            ...
            console.error('[WorkingCalendars Details] Load failed.', error);
            window.showToast?.(L.ErrorOccurred, 'error');
        }
    };
    ...
    const lifecycle = async (action) => {
        ...
        try {
            ...
            console.error(`[WorkingCalendars Details] ${action} failed.`, error);
            window.showToast?.(L.ErrorOccurred, 'error');
        }
    };
    ...
    const probe = async () => {
        ...
        try {
            ...
            console.error('[WorkingCalendars Details] Probe failed.', error);
            window.showToast?.(L.ErrorOccurred, 'error');
        }
    };
    ...
    const loadContract = async () => {
        ...
        try {
            ...
            console.error('[WorkingCalendars Day] Contract load failed.', error);
            return null;
        }
    };
    ...
    const saveDay = async (event) => {
        ...
        try {
            ...
            console.error('[WorkingCalendars Day] Save failed.', error);
    ...
    const archiveDay = (dayId) => {
        ...
        window.showConfirm?.(L.ArchiveDayConfirm || L.AreYouSure, async () => {
            try {
                ...
                console.error('[WorkingCalendars Day] Archive failed.', error);
                window.showToast?.(L.ErrorOccurred, 'error');
            }

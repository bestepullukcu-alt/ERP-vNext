/**
 * Working Calendars (country layer) - DataTables Index Script.
 * Platform Admin surface: weekend definition + official/religious holidays.
 *
...
const WorkingCalendarsList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-workingcalendars');
    ...
    const personalizationContext = { moduleKey: 'Platform', pageKey: 'WorkingCalendars' };
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[WorkingCalendars SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const loadLookupOptions = async () => {
        ...
        try {
            ...
            console.error('[WorkingCalendars Lookup] Failed.', error);
        }
    };
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Platform/WorkingCalendars/Details/${id}`;
        },
        edit: ({ id }) => {
            if (id) window.location.href = `/Platform/WorkingCalendars/Edit/${id}`;
        },
        activate: ({ row }) =>
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[WorkingCalendars SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                buttons: window.DtDefaults.exportButtons(
                    L.AddNew,
                    { href: '/Platform/WorkingCalendars/Create' },
                    extraButtons,
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/Platform/WorkingCalendars/Create';
                    });
...
document.addEventListener('DOMContentLoaded', () => WorkingCalendarsList.init());

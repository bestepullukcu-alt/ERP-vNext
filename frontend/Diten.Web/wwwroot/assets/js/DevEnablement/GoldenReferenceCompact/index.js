 ...
 * Large-field reference pattern: create/edit use full MVC pages.
 */
'use strict';
...
const GoldenReferenceCompactList = (function () {
    ...
    const personalizationContext = { moduleKey: 'DevEnablement', pageKey: 'GoldenReferenceCompact' };
    ...
    let appliedFilters = { status: [], referenceType: [], category: [], owner: [], priority: '' };
    let L = window.L10n || {};
    ...
    const populateDetailsOffcanvas = (data) => {
        ...
        if (editBtn) editBtn.href = `/GoldenReferenceCompact/Edit/${data.id}`;
    };
        ...
        onBulkAction: {
            delete: async ({ ids }) => {
                ...
                window.showConfirm?.(confirmText, async () => {
                    try {
                        const res = await fetch(`${apiUrl}/api/golden-reference-compact/bulk`, {
                            method: 'DELETE',
                            credentials: 'include',
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/GoldenReferenceCompact/Details/${id}`;
        },
        edit: ({ id }) => {
            if (id) window.location.href = `/GoldenReferenceCompact/Edit/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.id) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/api/golden-reference-compact/${row.id}`, {
                        method: 'DELETE',
                        credentials: 'include',
                        headers: getAuthHeaders()
                    });
    ...
    const initDataTable = async () => {
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            bulk: bulkOptions,
            ajax: {
                url: apiUrl + '/api/golden-reference-compact',
                type: 'GET',
            ...
            config: {
                ...
                columnDefs: [
                    ...
                    {
                        targets: 4,
                        render: (data) => data ? `<span class="badge bg-label-info">${getReferenceTypeMap()[data] || data}</span>` : ''
                    },
                    {
                        targets: 9,
                        render: (data, type) => {
                ...
                buttons: window.DtDefaults.exportButtons(
                    L.AddNew,
                    { href: '/GoldenReferenceCompact/Create' },
                    extraButtons,
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/GoldenReferenceCompact/Create';
                    });

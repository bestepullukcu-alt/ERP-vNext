/**
 * MOD-0024 / BL-054 — Task Templates, DataTables index script.
 *
 ...
 * re-invented.
 *
 * THREE deliberate differences, each because the DATA differs:
 *   · No bulk bar and no checkbox column — MOD-0024 exposes no bulk-delete for templates.
 *   · A CHECKLIST column. A template with none generates a task with no steps, which is precisely the empty task
 *     this slice exists to stop; leaving it invisible here would let the same complaint arrive again.
 *   · Company NAMES are resolved through a lookup rather than rendered from the row, because the row carries an
 *     id and a GUID in a column is not an answer to "which company?".
 */
'use strict';

const TaskTemplateList = (function () {
    let dt;
    let defaultViewRecord = null;
    let defaultViewState = null;
    let saveFilterArmed = false;

    const dtTableEl = document.querySelector('.datatables-tasktemplates');
    ...
    const personalizationContext = { moduleKey: 'Tasks', pageKey: 'TaskTemplates' };
    ...
    let appliedFilters = { status: [], priority: [], checklist: [], legalEntity: [] };
    let L = window.L10n || {};
    ...
    const emptyFilters = () => ({ status: [], priority: [], checklist: [], legalEntity: [] });
    const normalizeFilters = (filters) => {
        const source = filters || {};
        return {
            status: normalizeArray(source.status),
            priority: normalizeArray(source.priority),
            checklist: normalizeArray(source.checklist),
            legalEntity: normalizeArray(source.legalEntity)
        };
    };
    ...
    const loadDefaultView = async () => {
        ...
        try {
            ...
            console.error('[TaskTemplates SaveView] Failed to load saved views.', error);
            return null;
        }
    };
    ...
    const registerTableFilters = () => {
        ...
        $.fn.dataTable.ext.search.push((settings, _sd, dataIndex, rowData) => {
            ...
            return matchesStatusFilter(appliedFilters.status, row.isActive)
                && matchesMultiFilter(appliedFilters.priority, row.defaultPriority)
                && matchesMultiFilter(appliedFilters.checklist, checklistStateOf(row))
                // The BLANK legal entity is a real value here — "every company" — so it is matched as the
                // sentinel string rather than being dropped as an absent field, which would make the filter
                // silently unable to find the templates that apply everywhere.
                && matchesMultiFilter(appliedFilters.legalEntity, row.legalEntityId || ALL_COMPANIES);
        });
    };
    ...
    const initSelect2Filters = () => {
        ...
        $('#filterStatus, #filterPriority, #filterChecklist, #filterLegalEntity').each(function () {
            const $s = $(this);
    ...
    const syncFilterControls = (values) => {
        ...
        $('#filterPriority').val(normalizeArray(values.priority)).trigger('change');
        $('#filterChecklist').val(normalizeArray(values.checklist)).trigger('change');
        $('#filterLegalEntity').val(normalizeArray(values.legalEntity)).trigger('change');
    };
    const getAppliedFilterCount = () =>
        [appliedFilters.status, appliedFilters.priority, appliedFilters.checklist, appliedFilters.legalEntity]
    ...
    /*
     * Filter options come from the ROWS THIS TABLE ALREADY HAS, not from a lookup endpoint. Frequency and
     * assignment target are closed sets that arrive with the row data, so a second round-trip would buy nothing
    ...
    /*
     * Only the COMPANY list is derived; priority and checklist-state are closed sets authored in the markup.
     *
     * The "every company" option is prepended explicitly rather than collected, because it is precisely the
     * value that does NOT appear in the data — a template that applies everywhere carries no id at all, so a
     * distinct-value pass would leave the widest-scoped templates unfindable by this filter.
     */
    const populateFilterOptions = (api) => {
        const legalEntitySelect = document.getElementById('filterLegalEntity');
        ...
        const selected = Array.from(legalEntitySelect.selectedOptions || []).map((option) => option.value);

        const options = [{ value: ALL_COMPANIES, text: L.LegalEntityAll || '' }];
        collectDistinct(rows, 'legalEntityId').forEach((id) => {
            options.push({ value: id, text: legalEntityName(id) });
        });

        appendOptions(legalEntitySelect, options);
        $(legalEntitySelect).val(selected).trigger('change');
    };

    const setupFilters = async (api) => {
        initSelect2Filters();
        populateFilterOptions(api);
        api.on('draw.dt.taskTemplateFilters', () => populateFilterOptions(api));
        ...
        document.getElementById('btnFilterApply')?.addEventListener('click', () => {
            appliedFilters = {
                ...
                legalEntity: $('#filterLegalEntity').val() || []
            };
            api.draw();
            window.DtDefaults.updateVisualState(api, getAppliedFilterCount());
    ...
    /*
     * The sentinel for "no company id", so the filter can offer "every company" as a choice.
     *
     * A string that cannot collide with a GUID, because a template scoped to every company carries NO id — and
     * an empty-string option in a select2 is indistinguishable from the placeholder.
     */
    const ALL_COMPANIES = '__all__';

    /*
     * COMPANY NAMES, resolved once and cached.
     *
     * The row carries an id and nothing else; a GUID in a column is not an answer to "which company?". The names
     * come from the same MDM lookup the form uses, so the two screens cannot disagree — and an id the lookup
     * does not know still renders as itself rather than as an empty cell, because a company that has been
     * removed is something a reader needs to SEE rather than have hidden.
     */
    let legalEntityNames = {};
    const legalEntityName = (id) => (id ? (legalEntityNames[String(id)] || String(id)) : (L.LegalEntityAll || ''));

    const loadLegalEntityNames = async () => {
        try {
            ...
            rows.forEach((row) => {
                const id = row.legalEntityId || row.LegalEntityId || row.id || row.Id;
                const code = row.code || row.Code;
                const name = row.displayName || row.DisplayName || row.legalName || row.LegalName;
                if (id) legalEntityNames[String(id)] = code ? `${code} — ${name || ''}` : (name || String(id));
            });
        } catch (error) {
            // A failed name lookup must not empty the column: the ids still render, so the table stays usable
            // and the degradation is visible rather than silent.
            console.error('[TaskTemplates] Legal entity lookup failed.', error);
        }
    };

    /// Whether the template will actually put a checklist on the tasks it generates.
    const checklistStateOf = (row) => (row?.checklistTemplateId ? 'With' : 'Without');
    ...
    const rowActionHandlers = {
        quickView: ({ id }) => {
            if (id) window.location.href = `/Tasks/Templates/Details/${id}`;
        },
        edit: ({ id }) => {
            if (id) window.location.href = `/Tasks/Templates/Edit/${id}`;
        },
        delete: ({ row }) => {
            if (!row?.id) return;
            window.showConfirm?.(L.AreYouSure, async () => {
                try {
                    const res = await fetch(`${apiUrl}/templates/${row.id}`, {
                        method: 'DELETE',
                        credentials: 'include',
                        headers: getAuthHeaders()
                    });
                ...
                // The rule's NAME, which is what the person recognises — a GUID in a confirm dialog tells them
                // nothing about what they are about to retire.
    ...
    const initDataTable = async () => {
        if (!dtTableEl) return;
        syncL10n();
        // Names before rows: the company column would otherwise draw once with raw GUIDs and then correct
        // itself, which reads as the table changing its mind.
        await Promise.all([loadDefaultView(), loadLegalEntityNames()]);
        const extraButtons = {
            filterBtn: {
            ...
            saveFilterBtn: {
                ...
                action: async function (e, api) {
                    ...
                    try {
                        ...
                        console.error('[TaskTemplates SaveView] Failed to save default view.', error);
                        window.showToast?.(L.ErrorOccurred || '', 'error');
                    }
                }
            }
        ...
        dt = window.DitenDataTable.createCrudTable({
            ...
            config: {
                ...
                columns: [
                    ...
                    { data: 'name', name: 'name' },
                    { data: 'code', name: 'code' },
                    { data: 'defaultPriority', name: 'defaultPriority' },
                    { data: 'defaultDueInDays', name: 'defaultDueInDays' },
                    { data: 'checklistTemplateId', name: 'checklistTemplateId' },
                    { data: 'legalEntityId', name: 'legalEntityId' },
                ...
                columnDefs: [
                    ...
                    { targets: 2, render: (data) => `<span class="text-muted">${escapeHtml(data)}</span>` },
                    {
                        ...
                        render: (data, type) => type === 'display'
                            ? escapeHtml(L['Priority' + (data || '')] || data || '')
                            : (data || '')
                    },
                    {
                        targets: 4,
                        // No due offset is an ANSWER — work that is not time-boxed — so it is SAID rather than
                        // left blank for the reader to interpret as missing data.
                        render: (data, type) => {
                            if (type !== 'display') return data ?? -1;
                            return data
                                ? escapeHtml((L.DueInDaysValue || '{0}').replace('{0}', String(data)))
                                : `<span class="text-muted">${escapeHtml(L.NoDueDate || '')}</span>`;
                        }
                    },
                    {
                        targets: 5,
                        /*
                         * ⚠ THE COLUMN THIS WHOLE SLICE EXISTS FOR.
                         *
                         * A template with no checklist generates a task with no steps — which is exactly the
                         * empty task the recurring rule used to produce. Saying so in the list is what stops the
                         * same complaint arriving a second time from one level up.
                         */
                        render: (data, type) => {
                            if (type !== 'display') return data ? 'With' : 'Without';
                            return data
                                ? `<span class="badge bg-label-success">${escapeHtml(L.ChecklistBound || '')}</span>`
                                : `<span class="text-muted">${escapeHtml(L.ChecklistNone || '')}</span>`;
                        }
                    },
                    {
                        targets: 6,
                        // Blank means EVERY company, which is the widest scope rather than a missing value, so
                        // it is stated instead of rendered as an empty cell.
                        render: (data, type) => type === 'display'
                            ? escapeHtml(legalEntityName(data))
                            : (data || '')
                    },
                    {
                        targets: 7,
                ...
                initComplete: function () {
                    ...
                    document.querySelector('.add-new')?.addEventListener('click', (e) => {
                        e.preventDefault();
                        window.location.href = '/Tasks/Templates/Create';
                    });
...
document.addEventListener('DOMContentLoaded', () => TaskTemplateList.init());

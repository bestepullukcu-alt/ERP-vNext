...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch(`/assets/lang/${lang}.json`)
        ...
        .then(data => {
            const placeholderText = data["SearchRole"] || "Search Role";

            // DataTable veya custom tablo init fonksiyonunu burada çağır:
            initRoleDataTable(placeholderText, data);
        })
        .catch(error => {
            console.error('Dil dosyası yüklenemedi:', error);
            initRoleDataTable("Search Role", data); // fallback
        });

    modifyDataTableLayout();
    bindDeleteRecordEvent();
    initEditRole();
    initActiveDisableRole();
});

function initRoleDataTable(placeholderText, lanData) {

    const dt_role_table = document.querySelector('.role-table');
    if (dt_role_table) {
        const dt_company = new DataTable(dt_role_table, {
            ajax: {
                url: `${protocol}//${domain}:${port}/services/PvTenant/Role/GetRolesByTenantId`,
                method: 'GET',
                dataSrc: 'data'
            },
            columns: [
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: (data, type, full) => {
                        const isDisabled = full.name =='Admin'? true : false;

                        const disabledClass = isDisabled ? 'disabled opacity-50' : '';
                        return `
                            <div class="d-flex align-items-center">
                                <a href="javascript:;"  class="btn btn-icon activate-disabled-record ${disabledClass}" data-id="${full.id}" data-status="${full.activeStr}">
                                    <i class="icon-base bx bx-power-off icon-md"></i>
                                </a>
                                <a href="javascript:;"  class="btn btn-icon edit-record ${disabledClass}" data-id="${full.id}" data-name="${full.name}" data-description="${full.description}">
                ...
                topEnd: {
                    ...
                    features: [
                        ...
                        },
                        {
                            buttons: [
                                {
                                    text: '<i class="icon-base bx bx-plus icon-sm me-0 me-sm-2"></i><span class="d-none d-sm-inline-block" data-i18n="AddRole">Add Role</span>',
                                    className: 'add-new btn btn-primary',
                                    action: function () {
                                        window.location = '_AddRole';
                                    }
                                }
                            ]
                        }
...
function bindDeleteRecordEvent() {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', async function () {
        ...
        try {
            const response = await fetch(`${protocol}//${domain}:${port}/services/PvTenant/Role/DeleteRole`, {
                method: 'POST',
                headers: {
...
function openUpdateRolePage(button) {
    // Butondan data-id, data-name ve diğer verileri alıyoruz
    var id = button.getAttribute('data-id');
    var name = button.getAttribute('data-name');

    // URL'yi oluşturuyoruz
    var url = `/UserManagement/_UpdateRole?id=${id}&name=${encodeURIComponent(name)}`;
...
function initActiveDisableRole() {
    $(document).on('click', '.activate-disabled-record', function () {
        ...
        fetch(`${protocol}//${domain}:${port}/services/PvTenant/Role/activate-disable`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },

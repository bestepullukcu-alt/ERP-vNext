...
let port = protocol === 'https:' ? '5055' : '5050';
...
async function handleGoogleLogin() {
    ...
    const url = `${protocol}//${domain}:${port}/api/PvUser/Auth/login?userId=${encodeURIComponent(userId)}`;
...
function fetchEmails(Folder) {
    ...
    fetch(`${protocol}//${domain}:${port}/api/PvUser/Email/gmail/user-emails/${userId}?Folder=${Folder}`, {
        headers: {
...
async function loadUsersAndInitSelect2(selectId, assetsPath) {
    ...
    const apiUrl = `${protocol}//${domain}:${port}/api/PvUser/User/GetUsersByUserIdAndCompanyId/${userId}`;

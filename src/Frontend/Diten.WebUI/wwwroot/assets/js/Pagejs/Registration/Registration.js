...
function initDataTable(placeholderText, lanData) {
    ...
    if (dt_ma_table) {
        const dt_ma = new DataTable(dt_ma_table, {
            ajax: {
                url: `${protocol}//${domain}:${port}/services/PvOrganization/Ma/GetMarketingAuthorizations`,
                method: 'GET',
                dataSrc: 'data',
...
function bindDeleteRecordEvent() {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', async function () {
        ...
        try {
            const response = await fetch(`${protocol}//${domain}:${port}/services/PvOrganization/Ma/DeleteMa`, {
                method: 'POST',
                headers: {

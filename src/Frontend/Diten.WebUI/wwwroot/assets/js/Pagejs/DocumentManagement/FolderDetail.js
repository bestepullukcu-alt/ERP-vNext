...
async function getFolderPath(folderId) {
    ...
    while (currentId) {
        const json = await fetchJSON(
            `${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/GetFolderById?id=${currentId}`,
...
async function loadFolders() {
    try {
        const json = await fetchJSON(
            `${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/GetFolderById?id=${currentFolderId}`,
...
async function loadDocuments() {
    try {
        const json = await fetchJSON(
            `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/GetDocumentByFolderId?id=${currentFolderId}`,
...
async function handleCreateFolderSubmit(e) {
    ...
    try {
        const res = await fetch(`${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/CreateFolder`, {
...
function bindEditFolderEvent() {
    ...
    document.addEventListener('click', async function (e) {
        ...
        $('#editFolderForm').off('submit.edit').on('submit.edit', async function (ev) {
            ...
            try {
                const res = await fetch(`${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/UpdateFolder`, {
                    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
                });
...
function bindDeleteFolderEvent() {
    ...
    confirm.addEventListener('click', async function () {
        ...
        try {
            const json = await fetchJSON(
                `${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/DeleteFolder`,
...
function bindDeleteDocumentEvent() {
    ...
    confirm.addEventListener('click', async function () {
        ...
        try {
            const json = await fetchJSON(
                `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/DeleteDocument`,
...
function openMoveModal(docId, docName, docFolderName) {
    ...
    const userId = window.getUserId?.();
    fetchJSON(`${protocol}//${domain}:${port}/services/PvDocumentManagement/Folder/GetFolderByUserId?id=${userId}`, {}, 'Klasörler alınamadı.')
        .then(result => {
...
function handleMoveSubmit(e) {
    ...
    fetchJSON(
        `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/MoveDocument`,
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) },
        'Move failed.'
    ).then(() => {
...
async function openVersionHistory(documentId, documentName) {
    ...
    try {
        const json = await fetchJSON(
            `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/GetDocumentHistoryByDocumentId?id=${documentId}`,
...
function delegatedClicks(e) {
    ...
    if (starBtn) {
        ...
        fetchJSON(
            `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/StarDocument`,
            { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) },
            'Star failed.'
        ).then(() => loadDocuments());
        return;
...
function initShareOnce() {
    ...
    if (linkForm) {
        linkForm.addEventListener('submit', async (e) => {
            ...
            try {
                const json = await fetchJSON(
                    `${protocol}//${domain}:${port}/services/PvDocumentManagement/Document/Share`,
                    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) },
                    'Share failed.'
                );

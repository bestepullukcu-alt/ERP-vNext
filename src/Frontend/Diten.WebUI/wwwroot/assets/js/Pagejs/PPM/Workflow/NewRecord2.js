...
async function loadWorkflow(workflowId) {
    try {
        const response = await fetch(
            `${protocol}//${domain}:${port}/services/DitenPPM/Workflow/GetWorkflowById/${workflowId}`
        );
...
function onTasksTabShown() {
    ...
    populateSelect('add-task-type', {
        apiUrl: `${protocol}//${domain}:${port}/services/DitenPPM/Task/GetTaskTypes`,
    ...
    populateSelect('add-task-category', {
        apiUrl: `${protocol}//${domain}:${port}/services/DitenPPM/Task/GetTaskCategory`,
...
async function createWorkflow(data) {
    ...
    try {
        const response = await fetch(`${protocol}//${domain}:${port}/services/DitenPPM/Workflow/CreateWorkflow`, {
            method: 'POST',
...
async function updateWorkflow(data) {
    ...
    try {
        const response = await fetch(`${protocol}//${domain}:${port}/services/DitenPPM/Workflow/UpdateWorkflow`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
...
async function saveTaskOrderToBackend() {
    ...
    try {
        const response = await fetch(
            `${protocol}//${domain}:${port}/services/DitenPPM/Workflow/UpdateTaskOrder`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },

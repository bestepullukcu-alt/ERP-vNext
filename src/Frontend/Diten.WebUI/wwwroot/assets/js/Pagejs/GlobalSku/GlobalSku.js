...
function initGlobalSkuDataTable(placeholderText, lanData) {
    ...
    if (dt_globalsku_table) {
        const dt_company = new DataTable(dt_globalsku_table, {
            ajax: {
                url: `${protocol}//${domain}:${port}/services/PvOrganization/GlobalSku/GetGlobalSkus`,
                method: 'GET',
                dataSrc: 'data',
...
function bindDeleteRecordEvent() {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', async function () {
        ...
        try {
            const response = await fetch(`${protocol}//${domain}:${port}/services/PvOrganization/GlobalSku/DeleteGlobalSku`, {
                method: 'POST',
                headers: {

(function (window, document) {
  ...
  const state = { rows: [], filtered: [], goals: [], objectives: [], selected: null };
  ...
  function render(rows) {
    ...
    rows.forEach((r) => {
      ...
      tr.innerHTML = cols.map((c) => `<td class="${c.key === "actions" ? "text-end es-row-actions-col" : ""}">${getCell(r, c.key)}</td>`).join("");
    ...
    if (!rows.length) {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td colspan="${cols.length}" class="text-center text-muted py-3">No initiatives found for the current filters.</td>`;
      tableBody.appendChild(tr);
    }
